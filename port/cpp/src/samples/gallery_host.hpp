...
#define MAUI_GALLERY_PAGES(X)                                                                                          \
    ...
    X("slider", slider_page) X("stepper", stepper_page) X("progress_bar", progress_bar_page) X(                        \
        "activity_indicator", activity_indicator_page) X("box_view", box_view_page) X("date_picker", date_picker_page) \
        X("time_picker", time_picker_page) X("picker", picker_page) X("image_button", image_button_page) X(            \
            "refresh_view", refresh_view_page) X("absolute_layout", absolute_layout_page) X("grid", grid_page)         \
            X("flex_layout", flex_layout_page) X("stack_layout", stack_layout_page) X(                                 \
                "vertical_stack", vertical_stack_layout_page) X("horizontal_stack", horizontal_stack_layout_page)      \
                X("scroll_view", scroll_view_page) X("content_view", content_view_page) X("z_index", z_index_page) X(  \
                    "indicator", indicator_page) X("shapes_demo", shapes_demo_page) X("title_bar", title_bar_page)     \
                    X("clipping", clipping_page) X("templated_view", templated_view_page) X(                           \
                        "layout_is_enabled", layout_is_enabled_page) X("custom_layout",                                \
                                                                       custom_layout_page) X("hybrid_web_view",        \
                        ...
                        X("relative_layout", relative_layout_page) X("brushes", brushes_page) X(                       \
                            ...
                            X("styles", styles_page) X("triggers", triggers_page) X("behaviors", behaviors_page) X(    \
                                ...
                                X("semantics", semantics_page) X("focus", focus_page) X("dispatcher", dispatcher_page) \
                                    X("device", device_page) X("app_theme_binding", app_theme_binding_page)            \
                                        X("toolbar", toolbar_page) X("effects", effects_page) X(                       \
                                            "input_transparent", input_transparent_page) X("clip", clip_page)          \
                                            X("context_flyout", context_flyout_page) X("menu_bar", menu_bar_page) X(   \
                                                "navigation_gallery",                                                  \
                                                navigation_gallery_page) X("modal", modal_page)                        \
                                                X("application_control",                                               \
                                                  application_control_page) X("pointer_gesture", pointer_gesture_page) \
                                                    X("drag_drop", drag_drop_page) X("hit_testing", hit_testing_page)  \
