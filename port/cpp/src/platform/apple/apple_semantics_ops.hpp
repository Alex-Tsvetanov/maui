...
inline void install_hit_test_gate(Class cls) {
  ...
  IMP gate =
      imp_implementationWithBlock(^NSView *(NSView *view, NSPoint point) {
  ...
  objc_setAssociatedObject(cls, &k_hit_test_gate_installed_key, @YES,
...
inline void apply_semantics(NSView *view, const maui::core::semantics *value) {
  ...
  if (value->is_heading()) {
    ...
    view.accessibilityRoleDescription =
        NSAccessibilityRoleDescription(NSAccessibilityHeadingRole, nil);
  } else if ([view.accessibilityRole
                 isEqualToString:NSAccessibilityHeadingRole]) {
...
inline void apply_input_transparent(NSView *view, bool value) {
  ...
  objc_setAssociatedObject(view, &detail::k_input_transparent_key,
                           value ? @YES : nil,
