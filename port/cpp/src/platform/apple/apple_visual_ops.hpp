...
//   layer's backgroundColor (to_ns_color(...).CGColor); a LinearGradientPaint / RadialGradientPaint installs
//   a CAGradientLayer (axial / radial) as a sublayer of the backing layer, sized to the view bounds, with
//   colors[] + locations[] from the ordered stops (PaintExtensions.iOS.cs CreateCALayer / GetCAGradientLayer*);
//   a null paint clears both. The gradient sublayer is tagged (k_gradient_layer_name) so repeated calls
//   replace/remove it cleanly (switching paint kinds never leaves a stale gradient behind).
...
namespace maui::platform::apple
{
    ...
    // The name tagging the image-source background sublayer (an ImageSourcePaint fill) — the AppKit analog
    // of C#'s ViewExtensions.BackgroundLayerName. apply_background installs/removes it by this tag.
    ...
    inline NSArray* gradient_layer_colors(const std::vector<maui::graphics::gradient_stop>& stops)
    {
        ...
        for (std::size_t index = 0; index < stops.size(); index++)
        {
            ...
            if (stop_color == maui::graphics::color(0, 0, 0, 0) && has_neighbor) // Colors.Transparent
            {
                ...
                [colors addObject:(__bridge id)to_ns_color(borrowed).CGColor];
            }
            else
            {
                [colors addObject:(__bridge id)to_ns_color(stop_color).CGColor];
            }
        }
        return colors;
    }
    ...
    // Async): a named sublayer whose contents is the source's CGImage, sized to the view bounds, resize-
    // gravity (CALayer.GravityResize → kCAGravityResize). The base color is cleared (C# BackgroundColor =
    // Clear). A source that resolves to no image just leaves the (already-removed) layer absent. Returns
    // whether an image layer was installed (the caller skips the solid-color path when it was).
    //
    // LIFETIME: the whole install runs INSIDE the load completion, while the image_source_result still owns
    // the NSImage — `layer.contents = cgImage` retains the CGImage before the result is destroyed (a UAF if
    // the CGImage were extracted and used after the NSImage's CFRelease). The apple services invoke the
    // completion synchronously (a C++ move_only_function), so `installed` is observable on return.
    inline bool apply_image_source_background_layer(CALayer* layer, maui::core::i_image_source* source)
    {
        ...
        // Resolve against the process-wide registry (the no-DI seam; same as the loader's default path).
        ...
        service->load(*source, token, [&installed, layer](maui::core::image_source_result result) {
            ...
            NSImage* const image = (__bridge NSImage*)result.image();
            CGImageRef const cg_image = [image CGImageForProposedRect:nullptr context:nil hints:nil];
            if (cg_image == nullptr)
            {
                return;
            }
            ...
            image_layer.contents = (__bridge id)cg_image; // retains the CGImage while the NSImage is alive
    ...
    inline void apply_background(void* native, const maui::graphics::paint* p)
    {
        ...
        // ViewExtensions.UpdateBackgroundImageSourceAsync branch). Remove any stale gradient first, then
        // render the image; the base color is cleared by the helper. If the image layer is installed, the
        // solid/gradient paths below are skipped.
        if (const auto* const image_paint = dynamic_cast<const maui::core::image_source_paint*>(p))
        {
            ...
            // No image resolved → fall through to clear (the image layer was already removed by the helper).
