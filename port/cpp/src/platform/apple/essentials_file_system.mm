// file_system - Apple (AppKit / macOS) platform partial. Ported 1:1 from
// FileSystem.ios.tvos.watchos.macos.cs: cache/app-data resolve through NSSearchPath (the user
// domain's Caches / Library directories), and app-package files resolve under
// NSBundle.mainBundle.bundlePath + "Contents/Resources" (the MACCATALYST || MACOS branch of
// PlatformGetFullAppPackageFilePath), with '\' separators normalized. A missing package file
// throws std::runtime_error (the C# FileNotFoundException). Compiled as Objective-C++ with ARC
...
namespace maui::storage
{
    namespace
    {
        ...
        std::filesystem::path app_package_file_path(std::string_view filename)
        {
            ...
            root = root / "Contents" / "Resources"; // the macOS package layout
            return root / detail::normalize_app_package_path(filename);
        }

        class apple_file_system final : public i_file_system
        {
        public:
    ...
    namespace detail
    {
        ...
        {
            return std::make_shared<apple_file_system>();
        }
    } // namespace detail
} // namespace maui::storage
