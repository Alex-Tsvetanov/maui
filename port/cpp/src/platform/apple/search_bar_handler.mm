...
namespace maui::core
{
    ...
    namespace
    {
        void detach_trampolines(search_bar_platform& platform)
        {
            ...
            field.target = nil;
            field.action = nil;
            if (auto* const trampoline = (MauiSearchBarDelegate*)objc_getAssociatedObject(field, &k_delegate_key))
            {
    ...
    void search_bar_handler::map_text_color(search_bar_handler& handler, i_search_bar& view)
    {
        ...
        if (platform != nullptr)
        {
            ...
            as_search_field(platform->native).textColor = explicit_color != nil ? explicit_color : NSColor.textColor;
        }
    }
