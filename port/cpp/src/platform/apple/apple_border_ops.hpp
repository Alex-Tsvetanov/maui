...
namespace maui::platform::apple
{
                                    ...
                                    maui::graphics::rect bounds)
    {
        ...
        // is taken ONCE here and reused by the mask below AND the stroke path: deflating only the stroke
        // would shift its inner edge while the mask still cut at the undeflated outer edge, turning a pure
        // 0.5 DIP offset into a `thickness + 0.5` wide band. AppKit has no MAUI column of its own, so this
