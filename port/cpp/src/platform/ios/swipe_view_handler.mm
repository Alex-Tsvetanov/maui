...
// on a plain UIView and drives the machine from the programmatic open/close + the cross-platform
// synthetic offsets (the begin/swipe/end entry points a real pan recognizer can later call to add the
// drag visual — the same machine, the same notifications). The generic-IView property pushes mirror the
// ios content_page_handler.mm. Compiled as Objective-C++ with ARC only for the `ios` backend.
