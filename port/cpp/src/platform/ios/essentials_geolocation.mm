// geolocation - iOS (UIKit) platform partial. Ported from Geolocation.ios.macos.cs with the
// __IOS__ branches active:
//   - get_location_async: PausesLocationUpdatesAutomatically = NO (single update), the reduced-
//     accuracy flag from CLLocationManager.accuracyAuthorization (iOS 14+; the deployment floor is
//     far above), the request timeout via dispatch_after and the poll-style cancellation token.
//     (RequestTemporaryFullAccuracyAuthorization needs an Info.plist usage dictionary; the port
//     skips it like the rest of the Permissions subsystem - the host app owns authorization.)
//   - start_listening_foreground: PausesLocationUpdatesAutomatically = YES; failures (network /
//     Denied/Restricted authorization) stop listening FIRST, then raise listening_failed.
//   - CLLocation -> location per LocationExtensions.ToLocation (iOS branch: Course/Speed when
//     non-negative; IsFromMockProvider = simulator).
...
namespace
{
    ...
    bool reduced_accuracy(CLLocationManager* manager)
    {
        return manager.accuracyAuthorization == CLAccuracyAuthorizationReducedAccuracy;
    }

    // LocationExtensions.ToLocation(CLLocation, reducedAccuracy) - the __IOS__ branch.
    maui::devices::sensors::location to_location(CLLocation* platform_location, bool reduced)
    {
        ...
        result.reduced_accuracy = reduced;
        ...
        if (platform_location.course >= 0)
        {
            result.course = platform_location.course;
        }
        if (platform_location.speed >= 0)
        {
            result.speed = platform_location.speed;
        }
#if TARGET_OS_SIMULATOR
        result.is_from_mock_provider = true; // DeviceInfo.DeviceType == DeviceType.Virtual
#endif
...
@interface MauiIosSingleLocationListener : NSObject <CLLocationManagerDelegate>
@property(nonatomic, copy) void (^locationHandler)(CLLocation*);
@property(nonatomic) BOOL wasRaised;
@end

@implementation MauiIosSingleLocationListener
...
@interface MauiIosContinuousLocationListener : NSObject <CLLocationManagerDelegate>
@property(nonatomic, copy) void (^locationHandler)(CLLocation*);
@property(nonatomic, copy) void (^errorHandler)(maui::devices::sensors::geolocation_error);
@end

@implementation MauiIosContinuousLocationListener
- (void)locationManager:(CLLocationManager*)manager didUpdateLocations:(NSArray<CLLocation*>*)locations
{
    CLLocation* location = locations.lastObject;
    if (location != nil && self.locationHandler != nil)
...
namespace maui::devices::sensors
{
    namespace
    {
        ...
        class ios_geolocation final : public detail::geolocation_base
        {
        public:
            ~ios_geolocation() override
            {
                ios_geolocation::stop_listening_foreground();
            }
            ios_geolocation() = default;
            ios_geolocation(const ios_geolocation&) = delete;
            ios_geolocation(ios_geolocation&&) = delete;
            ios_geolocation& operator=(const ios_geolocation&) = delete;
            ios_geolocation& operator=(ios_geolocation&&) = delete;
            ...
            void get_last_known_location_async(location_callback on_complete) override
            {
                ...
                on_complete(cached != nil ? std::optional<location>(to_location(cached, reduced_accuracy(manager)))
                                          : std::nullopt);
            }
                                    ...
                                    location_callback on_complete) override
            {
                ...
                CLLocationManager* const manager = [[CLLocationManager alloc] init];
                MauiIosSingleLocationListener* const listener = [[MauiIosSingleLocationListener alloc] init];
                ...
                const bool reduced = reduced_accuracy(manager);
                ...
                listener.locationHandler = ^(CLLocation* platform_location) {
                  ...
                  *completed = true;
                  ...
                  {
                      (*shared_callback)(to_location(platform_location, reduced));
                  }
                };
            ...
            bool start_listening_foreground(const geolocation_listening_request& request) override
            {
                ...
                CLLocationManager* const manager = [[CLLocationManager alloc] init];
                MauiIosContinuousLocationListener* const listener = [[MauiIosContinuousLocationListener alloc] init];
                const bool reduced = reduced_accuracy(manager);
                listener.locationHandler = ^(CLLocation* platform_location) {
                  this->on_location_changed(to_location(platform_location, reduced));
                };
                ...
                manager.desiredAccuracy = to_platform_accuracy(request.desired_accuracy);
                manager.delegate = listener;
                manager.pausesLocationUpdatesAutomatically = YES; // allow pausing updates
                [manager startUpdatingLocation];
        ...
        private:
            ...
            MauiIosContinuousLocationListener* listening_listener_ = nil;
        };
    } // namespace
    ...
    namespace detail
    {
        ...
        {
            return std::make_shared<ios_geolocation>();
        }
    } // namespace detail
} // namespace maui::devices::sensors
