...
namespace maui::core
{
    ...
    maui::graphics::size search_bar_handler::get_desired_size(double width_constraint, double height_constraint) const
    {
        ...
        const auto_suggest_box box = as_auto_suggest_box(platform->native);
        // Clear the pinned size FIRST (see label/button/entry_handler.cpp's identical note): platform_arrange
        // stamps Width/Height on this element, and a FrameworkElement with an explicit Width/Height
        // measures to exactly that, so a later layout pass would just read back the PREVIOUS frame.
