...
namespace maui::core
{
    ...
    void label_handler::map_font(label_handler& handler, i_label& view)
    {
        ...
        block.FontSize(f.size() > 0 ? f.size() : maui::platform::windows::default_font_size());
    ...
    void label_handler::map_formatted_text(label_handler& handler, i_label& view)
    {
        ...
        for (const label_run& run_data : runs)
        {
            ...
            run.FontSize(f.size() > 0 ? f.size() : maui::platform::windows::default_font_size());
    ...
    maui::graphics::size label_handler::get_desired_size(double width_constraint, double height_constraint) const
    {
        ...
        // the pin. Pinned on the HOST (this element is the one platform_arrange sizes, exactly like
        // image_handler.cpp's identical host pin) rather than the TextBlock inside it: read i_label's own
        // width()/height() (this port's unset sentinel is NaN for both, same as C#'s NaN-is-unspecified
        // convention, so no translation is needed), pin the host's Width/Height to that, then widen
        // width_constraint/height_constraint by the same Math.Max rule.
        //
        ...
        // doing while the panel is a Canvas (814c4505ee).
