...
namespace
{
    ...
    // OPEN, NARROWED, NOT SOLVED — the cpp_xaml column paints NO selection chrome on `preselected_items`
    // while the cpp column paints it correctly on the SAME page, and MAUI paints it too. Measured
    // 2026-08-22 on the current captures (windows, light):
    //
    //     page                      cpp        cpp_xaml
    //     preselected_items         0.09%      0.64%   <- xaml shows NO ticks, no fill, no border
    //     multiple_bound_selection  0.00%      0.09%   <- xaml DOES paint, correctly
    //     preselected_item          0.00%      0.00%
    //     selection_synchronization 0.00%      0.00%
    //
    // So it is NOT "the loader never sets SelectedItems": multiple_bound_selection proves the direct-child
    // <CollectionView.SelectedItems><x:String> path reaches this handler and paints. Ruled out as staleness
    // the hard way — staged twin carries the new markup (5565 bytes), the regenerated bytes-mode TU
    // CONTAINS its bytes, maui_xaml.lib relinked 02:59, gallery_xaml.exe 03:09, capture after that.
    //
    ...
    // The same page is GREEN in cpp_xaml on ios, android and maccatalyst, so whatever it is, it is here.
