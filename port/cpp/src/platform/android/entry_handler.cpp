...
//     black — the unset-color-default family fix (see map_placeholder_color / editor_handler), so an empty
//     entry shows a gray placeholder matching MAUI, not solid black. An explicit PlaceholderColor still
//     overrides.
...
namespace maui::core
{
    ...
    void entry_platform::update_background(const maui::graphics::paint* value)
    {
        ...
        if (value != nullptr)
        {
            ...
            // otherwise it recolors the explicit background (e.g. a green Entry rendered gray).
    ...
    std::unique_ptr<entry_platform> entry_handler::create_platform_view()
    {
        ...
        // Tint the framework EditText's underline 9-patch to MAUI's rendered gray (#666666) — the same fix
        ...
        if (jclass csl_class = cache.find_class(env.get(), "android/content/res/ColorStateList"))
        {
            ...
            if (value_of != nullptr && set_bg_tint != nullptr)
            {
                const local_ref<jobject> tint{env.get(),
                                              env->CallStaticObjectMethod(csl_class, value_of, k_entry_underline_tint)};
    ...
    void entry_handler::map_placeholder_color(entry_handler& handler, i_entry& view)
    {
        ...
        if (env)
        {
            ...
            // the faithful stand-in for `!= null`, exactly as editor_handler::map_placeholder_color /
            // activity_indicator_handler::map_color do. On the UNSET branch, positively assert the measured
