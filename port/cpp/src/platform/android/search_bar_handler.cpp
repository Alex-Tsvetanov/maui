...
namespace maui::core
{
    ...
    std::unique_ptr<search_bar_platform> search_bar_handler::create_platform_view()
    {
        ...
        // (the trap the editor partial documents). Construct with defStyleAttr = 0 (no theme attr,
        // exactly what TextView(Context) does); fall back to the plain (Context) ctor so the widget is
        // never null. Single-line (the EditText default) — NO SetSingleLine(false) / multi-line knobs.
