...
namespace maui::core
{
    ...
    std::unique_ptr<date_picker_platform> date_picker_handler::create_platform_view()
    {
        ...
        // trap progress_bar's styled ctor + the picker / editor partials hit). Construct with defStyleAttr
        // = 0 (no theme attr, what TextView(Context) does); fall back to the plain (Context) ctor so the
