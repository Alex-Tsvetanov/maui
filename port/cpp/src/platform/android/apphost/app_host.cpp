...
#include "viewport.hpp" // display_size / drive_layout_viewport — shared with the C++&XAML apphost
...
namespace
{
    ...
    // layout pass that routes the safe area to the views that inset by it. Shared with the C++&XAML host —
    // see apphost/viewport.hpp; it used to be a line-for-line copy in each.
...
#undef MAUI_APPHOST_DISPATCH
    ...
    jobject mount_gallery(JNIEnv* env, const std::string& page_key, bool dark)
    {
        ...
        //     phone-ish size if the metrics could not be read. The bounds are the FULL, edge-to-edge window
        //     (the system bars overlay them); drive_layout_viewport routes the safe area to the views that
        //     inset by it.
