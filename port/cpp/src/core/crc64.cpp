// crc64_hash_string — CRC-64-Jones, ported byte-for-byte from Crc64HashAlgorithm.cs. See crc64.hpp.
...
namespace maui::core
{
    namespace
    {
        // The crc-64-jones lookup table (poly 0xad93d23594c935a9), verbatim from Crc64HashAlgorithm.Table.
        constexpr std::array<std::uint64_t, 256> crc64_table = {
        ...
        // Lowercase the X2 lookup gives uppercase hex; the C# lookup builds "X2" (uppercase) per byte.
        constexpr std::string_view hex_digits = "0123456789ABCDEF";
    } // namespace

    std::string crc64_hash_string(std::string_view input)
    {
        // HashCore: fold each UTF-8 byte through the table (crc init = ulong.MaxValue), counting length.
        std::uint64_t crc = ~std::uint64_t{0};
        std::uint64_t length = 0;
        for (const char ch : input)
        {
            const auto byte = static_cast<std::uint8_t>(ch);
            crc = crc64_table.at(static_cast<std::uint8_t>(crc ^ byte)) ^ (crc >> 8U);
            ++length;
        }

        // HashFinal: BitConverter.GetBytes(crc ^ length) — 8 LITTLE-ENDIAN bytes.
        const std::uint64_t final = crc ^ length;

        // ByteArrayToHexViaLookup32 over those 8 bytes, low byte first (little-endian order).
        std::string out;
        out.reserve(16);
        for (unsigned i = 0; i < 8; ++i)
        {
            const auto byte = static_cast<std::uint8_t>((final >> (8U * i)) & 0xFFU);
            out.push_back(hex_digits[byte >> 4U]);
