...
(examples/build-apple, plain executables opening a centered ~480x720 NSWindow) into the canonical
layout captures/maccatalyst/appkit_{cpp,xaml}/<key>_<theme>.png (the appkit_* framework columns that
build_comparison_json.py + gen_readme.py read). Sibling of capture_maccatalyst.py; kept separate so
the Catalyst harness is untouched.

Usage: python3 tools/parity/capture_appkit.py [--theme light|dark] [--dry-run] [key ...]
...
def main():
    ...
    if dry_run:
        for key in keys:
            print(cp.rel_capture(PLATFORM, "appkit_xaml", key, theme, "png"))
            if key not in NON_BUILDER:
                print(cp.rel_capture(PLATFORM, "appkit_cpp", key, theme, "png"))
        print("DRY_RUN_DONE", flush=True)
        return
    for key in keys:
        shot("appkit_xaml", key, theme, cp.capture_path(PLATFORM, "appkit_xaml", key, theme, "png"))
        if key not in NON_BUILDER:
            shot("appkit_cpp", key, theme, cp.capture_path(PLATFORM, "appkit_cpp", key, theme, "png"))
