...
//
// BYTE-DELIVERY vs HYDRATION. #embed is only how the *raw .xaml bytes reach the TU*; build_page itself
// consumes a fixed_string NTTP (already-materialized bytes) and hydrates it through the runtime loader —
// it never touches #embed. Those bytes can arrive either from #embed (the iOS/macOS/headless form) OR
// from a checked-in / generated byte-array literal (the Android bytes-mode form: the NDK's Clang 18 has
// no #embed, so examples/gallery_xaml/tools/gen_pages.py --embed-mode=bytes inlines the markup as a
// literal instead). So build_page's real requirement is MAUI_HAS_BUILD_PAGE (class-type NTTP + loader),
// NOT the #embed umbrella; gating build_page on #embed would wrongly reject the bytes-mode path on a
// toolchain that can hydrate the markup perfectly well.
...
// ---- build_page capability (byte-delivery-agnostic) --------------------------------------------------
// What build_page<VM, fixed_string> ACTUALLY needs: a class-type NTTP to carry the already-materialized
// markup bytes, plus the (always-linked) runtime loader to hydrate them. It does NOT need #embed — the
// bytes may equally come from a byte-array literal (Android bytes-mode). Gate the build_page static_assert
// on THIS, so the compile-time-XAML factory works wherever class-type NTTPs do, #embed or not.
#if MAUI_HAS_CLASS_NTTP
    #define MAUI_HAS_BUILD_PAGE 1
#else
    #define MAUI_HAS_BUILD_PAGE 0
#endif

// ---- the umbrella ------------------------------------------------------------------------------------
// The FULL "#embed-delivered compile-time XAML" story (the iOS/macOS path): #embed to pull the bytes in +
// class-type NTTPs to carry them. Kept for callers/tests that specifically assert the #embed path; the
// build_page static_assert deliberately uses the narrower MAUI_HAS_BUILD_PAGE instead (see above).
