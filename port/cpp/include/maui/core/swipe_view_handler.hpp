...
namespace maui::core
{
    ...
    struct swipe_view_platform : view_platform_base
    {
        ...
        // Microsoft.UI.Xaml.Controls.Canvas swipe host (defined in
        // src/platform/windows/swipe_view_handler.cpp). The host hosts the single swipe Content as its
        // child — the STATIC closed render (the live drag-to-reveal pan is the documented deviation: C#
        // Windows rides Microsoft.UI.Xaml.Controls.SwipeControl, deferred; the shared swipe_machine
        // still runs fully via the programmatic open/close + synthetic offsets — see the .cpp header).
