...
namespace maui::core
{
    struct search_bar_platform : view_platform_base
    {
        ...
        // Microsoft.UI.Xaml.Controls.AutoSuggestBox (defined in
        // src/platform/windows/search_bar_handler.cpp). Each override calls the view_platform_base body
        ...
        // dual-drive pattern). IsEnabled and Background have Windows-specific C# maps
        // (SearchBarHandler.Windows.MapIsEnabled / MapBackground → SearchBarExtensions). transform /
        ...
        // TextChanged / QuerySubmitted winrt::event_token values SearchBarHandler.Windows.cs
        // ConnectHandler installs (OnTextChanged — ProgrammaticChange skipped — / OnQuerySubmitted →
        // SearchButtonPressed). Revoked in on_disconnect_handler.
        std::int64_t text_changed_token = 0;
        std::int64_t query_submitted_token = 0;
#endif
    };
