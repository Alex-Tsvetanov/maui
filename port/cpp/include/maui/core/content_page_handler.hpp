...
namespace maui::core
{
    ...
    struct content_page_platform : view_platform_base
    {
        ...
        // Microsoft.UI.Xaml.Controls.Canvas content host (defined in
        // src/platform/windows/content_page_handler.cpp — the manual-frame container hosting the page's
        // single content child, C#'s ContentPanel translated to the port's Canvas layout model). Each
        ...
        // when one exists. is_enabled is intentionally NOT overridden: a Canvas is not a Control (C#'s
        // UpdateIsEnabled no-ops on it), matching the apple/ios/android twins. background pushes a
        // solid paint onto Panel.Background (ViewExtensions.UpdatePlatformViewBackground's Panel
        // branch); gradients keep the base mirror (deferred). transform / flow_direction / semantics /
        // shadow / clip / input_transparent keep the base mirrors in this first cut (deferred — see
        // the partial's header).
        void update_visibility(maui::core::visibility value) override;
        void update_opacity(double value) override;
        void update_automation_id(std::string_view value) override;
        void update_background(const maui::graphics::paint* value) override;
