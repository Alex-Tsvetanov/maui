...
| **Shapes/views don't honor `HorizontalOptions` inside a stack** — a `Start` shape stretches full-width; a `Fill` shape stays intrinsic. `shape::arrange` bypasses alignment-aware `compute_frame`. | 🔧 in progress | `path_aspect_gallery`, `auto_size_shapes` |
| **CollectionView embedded in a layout overlaps siblings / renders full-bleed** — an embedded CV reports a degenerate measured height, so the parent vertical layout assigns overlapping Y offsets. Standalone CV pages are fine. | 🔧 in progress | confirmed `multiple_bound_selection`, `scroll_to_group`; likely the `overlap` cluster: `collectionview`, `data_template_selector`, `empty_view`, `empty_view_swap`, `filter_selection`, `header_footer_grid`, `items_updating_scroll_mode`, `preselected_items`, `scroll_mode_test`, `selection_synchronization`, `single_bound_selection`, `some_empty_groups`, `switch_grouping` |
