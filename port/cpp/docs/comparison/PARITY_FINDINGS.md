...
| **Leaf-control native measure-vs-render** — real Entry/label (and likely button) report a desired height shorter than their native rendered height, so auto-sized containers paint past their measured slot (surfaces as the `scroll_to_group` residual). Broad native-handler fidelity concern. | ⬜ TODO — needs native investigation | `scroll_to_group` (residual) |
