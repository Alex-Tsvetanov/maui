...
| **CollectionView embedded in a layout overlaps siblings / renders full-bleed** — the cross-platform `collection_view_handler::get_desired_size` ported the obsolete `OnMeasure` screen-clamp instead of the content-size path, so an embedded CV reported full-screen (or 0 under the headless 0×0 display). | ⚠️ **PARTIAL** — fixed for headless + **macOS** (`61bdcc4595`, confirmed: `multiple_bound_selection` stacks cleanly on AppKit). **iOS native still overlaps** — the bounded `get_desired_size` does not flow into the iOS `UICollectionView` arrange (no `.mm` override; the native view isn't framed to the measured height). iOS-native fix in progress. | macOS-fixed; iOS-remaining: `multiple_bound_selection`, `scroll_to_group`, + the `overlap` cluster (`collectionview`, `data_template_selector`, `empty_view`, `empty_view_swap`, `filter_selection`, `header_footer_grid`, `items_updating_scroll_mode`, `preselected_items`, `scroll_mode_test`, `selection_synchronization`, `single_bound_selection`, `some_empty_groups`, `switch_grouping`) |
