...
| 112 | Chat Example | 🔴<br>L:diff<br>D:diff | — | <img src="csharp_ios_light/chat_example.png" height="360"> | <img src="cpp_ios_light/chat_example.png" height="360"> | <img src="csharp_ios_dark/chat_example.png" height="360"> | <img src="cpp_ios_dark/chat_example.png" height="360"> |
...
- **Light:** Root cause = iOS CV cell content-sizing (SHARED with varied_size_selector + the header_footer family). The bubble templates set the label's HorizontalOptions=Start/End (remote left/blue, local right/green) + Padding(10,6)/Margin(8,2), but the realized iOS CV cell STRETCHES its content to full cell width and ignores the content's HorizontalOptions — so the bubbles render as full-width colored bars instead of MAUI's content-width pills aligned by sender. Secondary, documented: the single-root templated cell can't carry the Border StrokeShape, so the bubbles are square-cornered (no rounded pill). Fixing the iOS CV cell to honor content HorizontalOptions + content-width sizing would address this whole cluster (deep/risky — touches every collection view).
- **Dark:** Same as light — bubbles render full-width (iOS CV cell stretches content, ignoring HorizontalOptions) instead of content-width pills aligned by sender; square-cornered (single-root cell, no StrokeShape). Shared CV cell-sizing root cause.
