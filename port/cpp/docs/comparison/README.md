...
- **Light:** The canvas DRAWING is correct (green triangle + yellow circle + translucent orange diagonal composited with transparency; the colored axes) — but the two GraphicsView canvases render TOO SMALL: MAUI shows two separate full-width (square) cream canvases with a gap; C++ shows them narrow (~70% width) and visually merged. Likely SHARED cause: an iOS leaf control with intrinsic content (GraphicsView here; Image on the clip/image pages) does NOT stretch to fill the parent width when HorizontalOptions=Fill — it stays at a smaller/content size. Worth a focused look at iOS leaf-control arrange honoring Fill width.
- **Dark:** Same as light — the composed shapes + axes draw correctly, but the GraphicsView canvases are too small / not filling width (the iOS leaf-control-Fill-width theme shared with the image/clip pages).
...
- **Light:** The card is a RoundRectangle Border (stroke 3, corner 10) with a black Shadow (offset 20,20, radius 40) and a light gradient fill. The iOS border_handler DOES push update_shadow (not the handler-drop class), so the missing drop-shadow is a shadow rendering/CLIPPING issue around the SwipeView container (the shadow extends beyond the border bounds and is clipped by the swipe content host, or apply_shadow doesn't render for a gradient+stroke border) — needs focused on-sim verification. Separately: 18 LEAF iOS handlers (button/label/entry/image/etc.) lack update_shadow (sibling of the Background-class gap) — a latent completeness gap, but no current diff page uses leaf-control shadows so it has no visible payoff yet.
- **Dark:** Same as light — Border has iOS shadow support; the missing drop-shadow is a swipe-container shadow-clip/render issue, not a handler-drop. (18 leaf handlers lack update_shadow = latent gap, no current failing page.)
