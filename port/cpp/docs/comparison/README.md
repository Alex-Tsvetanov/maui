...
- **Both themes:** FileSource (from_file('dotnet_bot.png')) renders only its purple background_brush; UriSource (campus.jpg) renders. DIAGNOSIS so far (inline, 2 background lldb agents died on infra timeouts): (1) file is present+valid in the installed bundle at the resolved path (mainBundle/dotnet_bot.png, 92532B 1200x694) — file-read prerequisite OK; (2) NOT a palette/format issue — an RGBA-converted copy ALSO renders purple; (3) NOT the std::filesystem::exists() gate — forcing the resolved path still renders purple; (4) routing is correct — i_file_image_source → load_file_source_sync → load_image_from_file → set_image_on_view (all verified in code; set_image_on_view is correct). So load_image_from_file returns nil even reading the present file, while the URI path's decode_image_bytes works for JPEG. NEXT (needs lldb): breakpoint load_image_from_file (image_handler.mm ~173-185) and print data.length after dataWithContentsOfFile + the decode_image_bytes result.loaded() — to tell whether the NSData read is nil or the CGImageSource decode of the file's PNG bytes fails (suspect a to_image_bytes/to_ns_data round-trip the URI path skips). FontImageSource also blank.
