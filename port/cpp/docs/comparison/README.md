...
**Progress: 49 / 172 🟢 matched** · 106 🟡 minor · 17 🔴 diff · 0 ⬜ pending
...
| 164 | Tabbed Flyout | ⬛<br>L:blank<br>D:blank | ⬛<br>L:cpp_blank<br>D:cpp_blank<br>_diff_<br><sub>gemini-3.1-flash-lite</sub> | <img src="csharp_ios_light/tabbed_flyout.png" height="360"> | <img src="cpp_ios_light/tabbed_flyout.png" height="360"> | <img src="csharp_ios_dark/tabbed_flyout.png" height="360"> | <img src="cpp_ios_dark/tabbed_flyout.png" height="360"> |
...
- **Light:** Real port bug (state application). In the 'All children are enabled' section both buttons are created {true,true} (set_is_enabled(true)), and the IsEnabled coercion is correct (explicit-true + enabled parent → is_enabled()==true), yet iOS renders the SECOND button's title gray (disabled appearance) while the first is blue (enabled); MAUI renders BOTH blue. So a default-enabled button's native enabled state isn't applied for the 2nd+ child (map_is_enabled is a no-op when value==default, so the native UIButton keeps a stale/disabled appearance for siblings). Other sections that SHOULD be disabled look right by coincidence. Needs runtime (lldb) confirmation of button[1].is_enabled() + when map_is_enabled runs at host time.
- **Dark:** Same as light — the 2nd button in the all-enabled section renders disabled-gray though both are set enabled (an is_enabled application/timing bug, not the coercion logic).
...
- **Light:** PENDING QUIRK RULING (wrapper-induced relative-sizing). The Ellipse correctly occupies HALF the available vertical space in BOTH — but the ASPECT differs: MAUI shows a wide-and-FLAT ellipse, C++ a near-CIRCLE. Cause: MAUI's harness wrapper crops the page much shorter (big black top/bottom bands), so its 'half' is short→flat; the port shows more of the page (modest padding) so its 'half' is taller→round. This sits between policy rulings: ruling #2 exempts the wrapper + says 'the port may show more of the page' (→ exempt), but also says the wrapper 'never changes a control's size' (→ port_diff). Flagged per ruling #3; kept as diff pending the user's ruling on whether wrapper-induced relative-sizing differences are exempt.
- **Dark:** Same as light — ellipse aspect differs (flat in MAUI / round in C++) because MAUI's harness crops the available height shorter; the ellipse fills half its (taller) available space in the port. Pending quirk ruling per policy #3.
...
- **Both themes:** ROOT CAUSE (diagnosed, effort-L fix): renders fully blank because the iOS gallery window-host (ios_gallery.mm ~line 99) treats page() as an i_view and grafts its native .view into rootViewController.view. But this page's root is a flyout_page (detail = a tabbed_page) — on iOS those are VIEW CONTROLLERS (UISplitViewController/UITabBarController), not plain views. Grafting their .view without installing the page's view-CONTROLLER as the window rootViewController (or proper child-VC containment) means UISplitVC/UITabBarVC child-VC lifecycle (viewDidLoad / child presentation) never fires, so nothing populates. Handlers ARE registered in both lists (not a registration miss). FIX: window host must detect a VC-backed root page (flyout/tabbed/navigation) and set it as the window rootViewController instead of grafting its view. Affects any VC-backed page used as a window root.
