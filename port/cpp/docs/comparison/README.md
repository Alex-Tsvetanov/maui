...
Pages rendered by **real shipped .NET MAUI** (standalone app) vs the **C++ port**, on **iOS and macOS/Mac Catalyst**. Compare layout/geometry (C# uses dark appearance on Mac Catalyst, the C++ gallery a light window).

**35 pages** on both platforms; the C++ macOS captures are post the AppKit top-left-origin flip fix.
