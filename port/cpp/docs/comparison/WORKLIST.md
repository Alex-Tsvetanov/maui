...
|---|---|---|---|---|---|---|
| 1 | windows | carousel_page | — | YEL | yes | OPEN (VM now reachable; prior finding: harness: NO MOTION EVIDENCE — the injected action moved NEITHER column; columns agree (carousel_page SSIM 0.9991). Fix the scenario coordinate, not the port) |
| 2 | windows | clip | — | YEL | yes | OPEN (VM now reachable; prior finding: needs-windows-vm: scroll EXTENT 42px too large. Changed-region bbox MAUI (8,32,261,750) vs port (8,32,261,792); same x/top, port ends 42px lower. Port stamps panel.Height() from view->content_size() (windows/scroll_view_handler.cpp:348-354) where MAUI's ScrollViewer measures content natively) |
| 3 | windows | clip_gallery | — | YEL | yes | OPEN (VM now reachable; prior finding: needs-windows-vm: NOT the clip extent bug — scroll extent MATCHES MAUI exactly (both bbox (20,32,313,792)). Port renders 2.3pp LESS lightgray (211,211,211): MAUI 6.6% vs port 4.3%, concentrated in rows 616-792. A clipped shape under-fills in the revealed lower rows) |
| 4 | windows | clip_views | — | YEL | yes | OPEN (VM now reachable; prior finding: needs-windows-vm: ROOT CAUSE FOUND. Focused Entry repaints its whole box (port 8604px/93% vs MAUI 506px/7%). Port skips EntryHandler.Windows.cs:58-59 MapBackground -> TextBoxExtensions.UpdateBackground's TextControlBackground resource-key dance and pushes Background generically; entry_handler.cpp:823-830 documents this as 'identical AT REST, diverges only in hover/focused/disabled') |
| 5 | windows | ios_scroll_view | — | YEL | yes | OPEN (VM now reachable; prior finding: harness: NO MOTION EVIDENCE — the injected action moved NEITHER column; columns agree (carousel_page SSIM 0.9991). Fix the scenario coordinate, not the port) |
| 6 | windows | path_gallery | — | YEL | yes | OPEN (VM now reachable; prior finding: needs-windows-vm: port's path content 32px WIDER (self-motion bbox MAUI (20,32,958,792) vs port (20,32,990,792); same top/bottom/left, differs on the HORIZONTAL axis). Distinct from clip (42px height extent) and clip_gallery (under-fill) — a Path/shape sizing difference) |
| 7 | windows | search_bar | — | YEL | yes | OPEN (VM now reachable; prior finding: harness: NO MOTION EVIDENCE — the injected action moved NEITHER column; columns agree (carousel_page SSIM 0.9991). Fix the scenario coordinate, not the port) |
| 8 | windows | semantics | — | YEL | yes | OPEN (VM now reachable; prior finding: harness: NO MOTION EVIDENCE — the injected action moved NEITHER column; columns agree (carousel_page SSIM 0.9991). Fix the scenario coordinate, not the port) |
| 9 | windows | slider | — | YEL | yes | OPEN (VM now reachable; prior finding: harness: NO MOTION EVIDENCE — the injected action moved NEITHER column; columns agree (carousel_page SSIM 0.9991). Fix the scenario coordinate, not the port) |
| 10 | windows | swipe_refresh | — | YEL | yes | OPEN (VM now reachable; prior finding: harness: NO MOTION EVIDENCE — the injected action moved NEITHER column; columns agree (carousel_page SSIM 0.9991). Fix the scenario coordinate, not the port) |
| 11 | windows | context_flyout | 52.78 | RED |  | OPEN (VM now reachable; prior finding: exemption-questioned: the context_flyout EXEMPT ruling was written for ANDROID (port rendered Chrome's first-run screen — live external content). On WINDOWS neither column shows browser content; palettes nearly match (MAUI 35.9/26.2 vs port 39.0/23.2) and 39.46% light / 59.94% dark differ over the whole frame (8,46,1016,792). Looks like LAYOUT, not live content. Needs a look before exempting) |
...
| 17 | ios | radio_content_properties | — | YEL | yes | BLOCKED needs-ios-diagnosis: same class as radio_button_content — STATIC (self-motion 0.0280% both, per-frame 1.31/1.31 identical pre/post). 1.38% light / 1.31% dark over the LEFT band x21-578 where the rings sit; histograms match (no delta >0.05pp) and vertical align finds NOTHING (0px, 3.18 unchanged). Edge-level, NOT the SF-symbol ring gap — that was already fixed: radio_button_handler.mm:186-188 draws the ring with UIGraphicsImageRenderer at 21pt/2pt/11pt per RadioButton.cs:552-563. Cause unknown |
| 18 | ios | scroll_view | — | YEL | yes | BLOCKED needs-ios-diagnosis: +6px SCROLL-POSITION offset, same class as ios/clip_gallery's +4px. Stills match at rest (0.03%), self-motion nearly equal (2.7838% vs 2.7432%), driven frame differs 2.42%; best vertical alignment +6px (mean|diff| 4.66 -> 2.90). Equal scroll distance, different final position — TWO ios pages now, so likely one shared scroll-offset cause |
| 19 | ios | selection_synchronization | — | RED | yes | OPEN (ref restored 89b30d0c62; port was always correct) |
| 20 | ios | swipe_refresh | — | RED | yes | BLOCKED harness-or-twin: MAUI self-motion is 0.0000% while the port reacts at 1.2897% across the sequence. The MAUI column never moved, so the pull did not happen THERE — one-sided NO-MOTION, not a port defect. Port pulling is correct (a69ce65275 fixed alwaysBounceVertical). Compare with 2ab7010972, which restored this twin's ScrollView because 'the pull it drives could not exist to be compared' |
