...
**155 example pages.** Two genuine framework gaps the sample work surfaced were root-caused + fixed with full-gate framework changes + TDD tests: **CollectionView struct-typed templated-cell rendering** (porting C#'s `TemplatedCell.Bind` to the AppKit/iOS handlers) and **`View.HorizontalOptions`/`VerticalOptions`** (made settable + honored at `view::arrange`, the C# `ComputeFrame` path).
...
## Feature subsystems (Core + UserInterface) (27)
...
## CollectionView + CarouselView galleries (27)
