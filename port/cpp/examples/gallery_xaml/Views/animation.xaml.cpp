// animation.xaml.cpp — implements examples::Views::animation_page() (declared in animation.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/animation.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.AnimationPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char animation_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/animation.xaml"
    };
    constexpr maui::fixed_string animation_xaml{animation_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(animation_xaml);
    ...
    static_assert(bytes_contain(animation_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(animation_xaml_bytes, "x:Class=\"MauiReference.Pages.AnimationPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> animation_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, animation_xaml>(options);
    }
} // namespace examples::Views
