...
// port/maui-reference/pages/filter_selection.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.FilterSelectionPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/filter_selection.xaml"
    };
    constexpr maui::fixed_string filter_selection_xaml{filter_selection_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(filter_selection_xaml);
    ...
    static_assert(bytes_contain(filter_selection_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(filter_selection_xaml_bytes, "x:Class=\"MauiReference.Pages.FilterSelectionPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> filter_selection_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, filter_selection_xaml>(options);
    }
} // namespace examples::Views
