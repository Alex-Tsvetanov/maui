// switch.xaml.cpp — implements examples::Views::switch_page() (declared in switch.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/switch.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.SwitchPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char switch_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/switch.xaml"
    };
    constexpr maui::fixed_string switch_xaml{switch_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(switch_xaml);
    ...
    static_assert(bytes_contain(switch_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(switch_xaml_bytes, "x:Class=\"MauiReference.Pages.SwitchPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> switch_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, switch_xaml>(options);
    }
} // namespace examples::Views
