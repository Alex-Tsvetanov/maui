...
// port/maui-reference/pages/items.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ItemsPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/items.xaml"
    };
    constexpr maui::fixed_string items_xaml{items_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(items_xaml);
    ...
    static_assert(bytes_contain(items_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(items_xaml_bytes, "x:Class=\"MauiReference.Pages.ItemsPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> items_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, items_xaml>(options);
    }
} // namespace examples::Views
