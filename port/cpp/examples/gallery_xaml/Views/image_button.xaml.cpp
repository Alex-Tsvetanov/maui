// image_button.xaml.cpp — implements examples::Views::image_button_page() (declared in image_button.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/image_button.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ImageButtonPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char image_button_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/image_button.xaml"
    };
    constexpr maui::fixed_string image_button_xaml{image_button_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(image_button_xaml);
    ...
    static_assert(bytes_contain(image_button_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(image_button_xaml_bytes, "x:Class=\"MauiReference.Pages.ImageButtonPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> image_button_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, image_button_xaml>(options);
    }
} // namespace examples::Views
