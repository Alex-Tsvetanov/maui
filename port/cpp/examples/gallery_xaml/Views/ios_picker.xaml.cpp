// ios_picker.xaml.cpp — implements examples::Views::ios_picker_page() (declared in ios_picker.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/ios_picker.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.IosPickerPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char ios_picker_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/ios_picker.xaml"
    };
    constexpr maui::fixed_string ios_picker_xaml{ios_picker_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(ios_picker_xaml);
    ...
    static_assert(bytes_contain(ios_picker_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(ios_picker_xaml_bytes, "x:Class=\"MauiReference.Pages.IosPickerPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> ios_picker_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, ios_picker_xaml>(options);
    }
} // namespace examples::Views
