...
// port/maui-reference/pages/swipe_view_margin.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.SwipeViewMarginPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/swipe_view_margin.xaml"
    };
    constexpr maui::fixed_string swipe_view_margin_xaml{swipe_view_margin_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(swipe_view_margin_xaml);
    ...
    static_assert(bytes_contain(swipe_view_margin_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(swipe_view_margin_xaml_bytes, "x:Class=\"MauiReference.Pages.SwipeViewMarginPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> swipe_view_margin_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, swipe_view_margin_xaml>(options);
    }
} // namespace examples::Views
