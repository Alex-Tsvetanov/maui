...
// port/maui-reference/pages/flex_layout.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.FlexLayoutPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/flex_layout.xaml"
    };
    constexpr maui::fixed_string flex_layout_xaml{flex_layout_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(flex_layout_xaml);
    ...
    static_assert(bytes_contain(flex_layout_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(flex_layout_xaml_bytes, "x:Class=\"MauiReference.Pages.FlexLayoutPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> flex_layout_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, flex_layout_xaml>(options);
    }
} // namespace examples::Views
