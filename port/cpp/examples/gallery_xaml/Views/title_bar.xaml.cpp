...
// port/maui-reference/pages/title_bar.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.TitleBarPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/title_bar.xaml"
    };
    constexpr maui::fixed_string title_bar_xaml{title_bar_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(title_bar_xaml);
    ...
    static_assert(bytes_contain(title_bar_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(title_bar_xaml_bytes, "x:Class=\"MauiReference.Pages.TitleBarPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> title_bar_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, title_bar_xaml>(options);
    }
} // namespace examples::Views
