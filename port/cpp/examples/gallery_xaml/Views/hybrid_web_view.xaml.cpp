// hybrid_web_view.xaml.cpp — implements examples::Views::hybrid_web_view_page() (declared in hybrid_web_view.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/hybrid_web_view.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.HybridWebViewPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char hybrid_web_view_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/hybrid_web_view.xaml"
    };
    constexpr maui::fixed_string hybrid_web_view_xaml{hybrid_web_view_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(hybrid_web_view_xaml);
    ...
    static_assert(bytes_contain(hybrid_web_view_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(hybrid_web_view_xaml_bytes, "x:Class=\"MauiReference.Pages.HybridWebViewPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> hybrid_web_view_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, hybrid_web_view_xaml>(options);
    }
} // namespace examples::Views
