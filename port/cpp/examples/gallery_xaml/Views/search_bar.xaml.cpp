...
// port/maui-reference/pages/search_bar.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.SearchBarPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/search_bar.xaml"
    };
    constexpr maui::fixed_string search_bar_xaml{search_bar_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(search_bar_xaml);
    ...
    static_assert(bytes_contain(search_bar_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(search_bar_xaml_bytes, "x:Class=\"MauiReference.Pages.SearchBarPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> search_bar_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, search_bar_xaml>(options);
    }
} // namespace examples::Views
