// ios_pan_gesture.xaml.cpp — implements examples::Views::ios_pan_gesture_page() (declared in ios_pan_gesture.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/ios_pan_gesture.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.IosPanGesturePage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char ios_pan_gesture_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/ios_pan_gesture.xaml"
    };
    constexpr maui::fixed_string ios_pan_gesture_xaml{ios_pan_gesture_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(ios_pan_gesture_xaml);
    ...
    static_assert(bytes_contain(ios_pan_gesture_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(ios_pan_gesture_xaml_bytes, "x:Class=\"MauiReference.Pages.IosPanGesturePage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> ios_pan_gesture_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, ios_pan_gesture_xaml>(options);
    }
} // namespace examples::Views
