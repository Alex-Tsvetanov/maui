...
// port/maui-reference/pages/frame.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.FramePage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/frame.xaml"
    };
    constexpr maui::fixed_string frame_xaml{frame_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(frame_xaml);
    ...
    static_assert(bytes_contain(frame_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(frame_xaml_bytes, "x:Class=\"MauiReference.Pages.FramePage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> frame_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, frame_xaml>(options);
    }
} // namespace examples::Views
