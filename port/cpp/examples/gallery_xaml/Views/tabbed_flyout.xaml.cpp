...
// port/maui-reference/pages/tabbed_flyout.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.TabbedFlyoutPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/tabbed_flyout.xaml"
    };
    constexpr maui::fixed_string tabbed_flyout_xaml{tabbed_flyout_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(tabbed_flyout_xaml);
    ...
    static_assert(bytes_contain(tabbed_flyout_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(tabbed_flyout_xaml_bytes, "x:Class=\"MauiReference.Pages.TabbedFlyoutPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> tabbed_flyout_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, tabbed_flyout_xaml>(options);
    }
} // namespace examples::Views
