...
// port/maui-reference/pages/picker.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.PickerPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/picker.xaml"
    };
    constexpr maui::fixed_string picker_xaml{picker_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(picker_xaml);
    ...
    static_assert(bytes_contain(picker_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(picker_xaml_bytes, "x:Class=\"MauiReference.Pages.PickerPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> picker_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, picker_xaml>(options);
    }
} // namespace examples::Views
