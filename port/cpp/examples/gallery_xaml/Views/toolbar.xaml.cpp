// toolbar.xaml.cpp — implements examples::Views::toolbar_page() (declared in toolbar.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/toolbar.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ToolbarPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char toolbar_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/toolbar.xaml"
    };
    constexpr maui::fixed_string toolbar_xaml{toolbar_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(toolbar_xaml);
    ...
    static_assert(bytes_contain(toolbar_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(toolbar_xaml_bytes, "x:Class=\"MauiReference.Pages.ToolbarPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> toolbar_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, toolbar_xaml>(options);
    }
} // namespace examples::Views
