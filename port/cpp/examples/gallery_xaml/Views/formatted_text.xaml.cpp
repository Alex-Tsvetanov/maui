...
// port/maui-reference/pages/formatted_text.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.FormattedTextPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/formatted_text.xaml"
    };
    constexpr maui::fixed_string formatted_text_xaml{formatted_text_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(formatted_text_xaml);
    ...
    static_assert(bytes_contain(formatted_text_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(formatted_text_xaml_bytes, "x:Class=\"MauiReference.Pages.FormattedTextPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> formatted_text_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, formatted_text_xaml>(options);
    }
} // namespace examples::Views
