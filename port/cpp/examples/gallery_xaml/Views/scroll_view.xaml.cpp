...
// port/maui-reference/pages/scroll_view.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ScrollViewPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/scroll_view.xaml"
    };
    constexpr maui::fixed_string scroll_view_xaml{scroll_view_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(scroll_view_xaml);
    ...
    static_assert(bytes_contain(scroll_view_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(scroll_view_xaml_bytes, "x:Class=\"MauiReference.Pages.ScrollViewPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> scroll_view_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, scroll_view_xaml>(options);
    }
} // namespace examples::Views
