...
// port/maui-reference/pages/filter_collection.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.FilterCollectionPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/filter_collection.xaml"
    };
    constexpr maui::fixed_string filter_collection_xaml{filter_collection_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(filter_collection_xaml);
    ...
    static_assert(bytes_contain(filter_collection_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(filter_collection_xaml_bytes, "x:Class=\"MauiReference.Pages.FilterCollectionPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> filter_collection_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, filter_collection_xaml>(options);
    }
} // namespace examples::Views
