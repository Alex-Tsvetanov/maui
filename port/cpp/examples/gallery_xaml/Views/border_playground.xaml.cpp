...
// port/maui-reference/pages/border_playground.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.BorderPlaygroundPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/border_playground.xaml"
    };
    constexpr maui::fixed_string border_playground_xaml{border_playground_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(border_playground_xaml);
    ...
    static_assert(bytes_contain(border_playground_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(border_playground_xaml_bytes, "x:Class=\"MauiReference.Pages.BorderPlaygroundPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> border_playground_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, border_playground_xaml>(options);
    }
} // namespace examples::Views
