// editor.xaml.cpp — implements examples::Views::editor_page() (declared in editor.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/editor.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.EditorPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char editor_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/editor.xaml"
    };
    constexpr maui::fixed_string editor_xaml{editor_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(editor_xaml);
    ...
    static_assert(bytes_contain(editor_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(editor_xaml_bytes, "x:Class=\"MauiReference.Pages.EditorPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> editor_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, editor_xaml>(options);
    }
} // namespace examples::Views
