// app_theme_binding.xaml.cpp — implements examples::Views::app_theme_binding_page() (declared in
// app_theme_binding.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/app_theme_binding.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.AppThemeBindingPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char app_theme_binding_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/app_theme_binding.xaml"
    };
    constexpr maui::fixed_string app_theme_binding_xaml{app_theme_binding_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(app_theme_binding_xaml);
    ...
    static_assert(bytes_contain(app_theme_binding_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(app_theme_binding_xaml_bytes, "x:Class=\"MauiReference.Pages.AppThemeBindingPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> app_theme_binding_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, app_theme_binding_xaml>(options);
    }
} // namespace examples::Views
