...
// port/maui-reference/pages/grid_grouping.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.GridGroupingPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/grid_grouping.xaml"
    };
    ...
    static_assert(bytes_contain(grid_grouping_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(grid_grouping_xaml_bytes, "x:Class=\"MauiReference.Pages.GridGroupingPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> grid_grouping_page(const maui::xaml::xaml_load_options& options)
    {
        auto page = maui::build_page<maui::no_view_model, grid_grouping_xaml>(options);
