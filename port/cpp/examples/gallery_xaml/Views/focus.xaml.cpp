// focus.xaml.cpp — implements examples::Views::focus_page() (declared in focus.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/focus.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.FocusPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char focus_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/focus.xaml"
    };
    constexpr maui::fixed_string focus_xaml{focus_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(focus_xaml);
    ...
    static_assert(bytes_contain(focus_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(focus_xaml_bytes, "x:Class=\"MauiReference.Pages.FocusPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> focus_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, focus_xaml>(options);
    }
} // namespace examples::Views
