...
// port/maui-reference/pages/input_transparent.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.InputTransparentPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/input_transparent.xaml"
    };
    constexpr maui::fixed_string input_transparent_xaml{input_transparent_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(input_transparent_xaml);
    ...
    static_assert(bytes_contain(input_transparent_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(input_transparent_xaml_bytes, "x:Class=\"MauiReference.Pages.InputTransparentPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> input_transparent_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, input_transparent_xaml>(options);
    }
} // namespace examples::Views
