...
// port/maui-reference/pages/border_layout.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.BorderLayoutPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/border_layout.xaml"
    };
    constexpr maui::fixed_string border_layout_xaml{border_layout_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(border_layout_xaml);
    ...
    static_assert(bytes_contain(border_layout_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(border_layout_xaml_bytes, "x:Class=\"MauiReference.Pages.BorderLayoutPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> border_layout_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, border_layout_xaml>(options);
    }
} // namespace examples::Views
