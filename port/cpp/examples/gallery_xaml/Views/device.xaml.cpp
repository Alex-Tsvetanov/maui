// device.xaml.cpp — implements examples::Views::device_page() (declared in device.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/device.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.DevicePage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char device_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/device.xaml"
    };
    constexpr maui::fixed_string device_xaml{device_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(device_xaml);
    ...
    static_assert(bytes_contain(device_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(device_xaml_bytes, "x:Class=\"MauiReference.Pages.DevicePage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> device_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, device_xaml>(options);
    }
} // namespace examples::Views
