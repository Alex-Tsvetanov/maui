...
// port/maui-reference/pages/dispatcher.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.DispatcherPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/dispatcher.xaml"
    };
    constexpr maui::fixed_string dispatcher_xaml{dispatcher_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(dispatcher_xaml);
    ...
    static_assert(bytes_contain(dispatcher_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(dispatcher_xaml_bytes, "x:Class=\"MauiReference.Pages.DispatcherPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> dispatcher_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, dispatcher_xaml>(options);
    }
} // namespace examples::Views
