...
// port/maui-reference/pages/semantics.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.SemanticsPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/semantics.xaml"
    };
    constexpr maui::fixed_string semantics_xaml{semantics_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(semantics_xaml);
    ...
    static_assert(bytes_contain(semantics_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(semantics_xaml_bytes, "x:Class=\"MauiReference.Pages.SemanticsPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> semantics_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, semantics_xaml>(options);
    }
} // namespace examples::Views
