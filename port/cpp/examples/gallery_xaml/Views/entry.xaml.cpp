...
// port/maui-reference/pages/entry.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.EntryPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/entry.xaml"
    };
    constexpr maui::fixed_string entry_xaml{entry_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(entry_xaml);
    ...
    static_assert(bytes_contain(entry_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(entry_xaml_bytes, "x:Class=\"MauiReference.Pages.EntryPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> entry_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, entry_xaml>(options);
    }
} // namespace examples::Views
