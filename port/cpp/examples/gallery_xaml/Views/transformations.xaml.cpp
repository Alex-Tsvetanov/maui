...
// port/maui-reference/pages/transformations.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.TransformationsPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/transformations.xaml"
    };
    constexpr maui::fixed_string transformations_xaml{transformations_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(transformations_xaml);
    ...
    static_assert(bytes_contain(transformations_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(transformations_xaml_bytes, "x:Class=\"MauiReference.Pages.TransformationsPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> transformations_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, transformations_xaml>(options);
    }
} // namespace examples::Views
