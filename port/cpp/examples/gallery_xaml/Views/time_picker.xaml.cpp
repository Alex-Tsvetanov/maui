// time_picker.xaml.cpp — implements examples::Views::time_picker_page() (declared in time_picker.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/time_picker.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.TimePickerPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char time_picker_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/time_picker.xaml"
    };
    constexpr maui::fixed_string time_picker_xaml{time_picker_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(time_picker_xaml);
    ...
    static_assert(bytes_contain(time_picker_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(time_picker_xaml_bytes, "x:Class=\"MauiReference.Pages.TimePickerPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> time_picker_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, time_picker_xaml>(options);
    }
} // namespace examples::Views
