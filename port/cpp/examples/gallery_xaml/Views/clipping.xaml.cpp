// clipping.xaml.cpp — implements examples::Views::clipping_page() (declared in clipping.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/clipping.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ClippingPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char clipping_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/clipping.xaml"
    };
    constexpr maui::fixed_string clipping_xaml{clipping_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(clipping_xaml);
    ...
    static_assert(bytes_contain(clipping_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(clipping_xaml_bytes, "x:Class=\"MauiReference.Pages.ClippingPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> clipping_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, clipping_xaml>(options);
    }
} // namespace examples::Views
