// clip_views.xaml.cpp — implements examples::Views::clip_views_page() (declared in clip_views.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/clip_views.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ClipViewsPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char clip_views_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/clip_views.xaml"
    };
    constexpr maui::fixed_string clip_views_xaml{clip_views_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(clip_views_xaml);
    ...
    static_assert(bytes_contain(clip_views_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(clip_views_xaml_bytes, "x:Class=\"MauiReference.Pages.ClipViewsPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> clip_views_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, clip_views_xaml>(options);
    }
} // namespace examples::Views
