...
// port/maui-reference/pages/menu_bar.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.MenuBarPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/menu_bar.xaml"
    };
    constexpr maui::fixed_string menu_bar_xaml{menu_bar_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(menu_bar_xaml);
    ...
    static_assert(bytes_contain(menu_bar_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(menu_bar_xaml_bytes, "x:Class=\"MauiReference.Pages.MenuBarPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> menu_bar_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, menu_bar_xaml>(options);
    }
} // namespace examples::Views
