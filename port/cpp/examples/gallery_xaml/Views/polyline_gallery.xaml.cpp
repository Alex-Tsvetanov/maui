// polyline_gallery.xaml.cpp — implements examples::Views::polyline_gallery_page() (declared in
// polyline_gallery.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/polyline_gallery.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.PolylineGalleryPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char polyline_gallery_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/polyline_gallery.xaml"
    };
    constexpr maui::fixed_string polyline_gallery_xaml{polyline_gallery_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(polyline_gallery_xaml);
    ...
    static_assert(bytes_contain(polyline_gallery_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(polyline_gallery_xaml_bytes, "x:Class=\"MauiReference.Pages.PolylineGalleryPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> polyline_gallery_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, polyline_gallery_xaml>(options);
    }
} // namespace examples::Views
