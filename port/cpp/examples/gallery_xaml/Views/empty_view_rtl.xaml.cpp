...
// port/maui-reference/pages/empty_view_rtl.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.EmptyViewRtlPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/empty_view_rtl.xaml"
    };
    constexpr maui::fixed_string empty_view_rtl_xaml{empty_view_rtl_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(empty_view_rtl_xaml);
    ...
    static_assert(bytes_contain(empty_view_rtl_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(empty_view_rtl_xaml_bytes, "x:Class=\"MauiReference.Pages.EmptyViewRtlPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> empty_view_rtl_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, empty_view_rtl_xaml>(options);
    }
} // namespace examples::Views
