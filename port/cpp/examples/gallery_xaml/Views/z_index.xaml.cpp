...
// port/maui-reference/pages/z_index.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ZIndexPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/z_index.xaml"
    };
    constexpr maui::fixed_string z_index_xaml{z_index_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(z_index_xaml);
    ...
    static_assert(bytes_contain(z_index_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(z_index_xaml_bytes, "x:Class=\"MauiReference.Pages.ZIndexPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> z_index_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, z_index_xaml>(options);
    }
} // namespace examples::Views
