...
// port/maui-reference/pages/preselected_items.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.PreselectedItemsPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/preselected_items.xaml"
    };
    constexpr maui::fixed_string preselected_items_xaml{preselected_items_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(preselected_items_xaml);
    ...
    static_assert(bytes_contain(preselected_items_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(preselected_items_xaml_bytes, "x:Class=\"MauiReference.Pages.PreselectedItemsPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> preselected_items_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, preselected_items_xaml>(options);
    }
} // namespace examples::Views
