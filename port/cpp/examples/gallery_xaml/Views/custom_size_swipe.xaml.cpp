...
// port/maui-reference/pages/custom_size_swipe.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.CustomSizeSwipePage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/custom_size_swipe.xaml"
    };
    constexpr maui::fixed_string custom_size_swipe_xaml{custom_size_swipe_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(custom_size_swipe_xaml);
    ...
    static_assert(bytes_contain(custom_size_swipe_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(custom_size_swipe_xaml_bytes, "x:Class=\"MauiReference.Pages.CustomSizeSwipePage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> custom_size_swipe_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, custom_size_swipe_xaml>(options);
    }
} // namespace examples::Views
