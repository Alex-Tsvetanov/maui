// custom_layout.xaml.cpp — implements examples::Views::custom_layout_page() (declared in custom_layout.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/custom_layout.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.CustomLayoutPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char custom_layout_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/custom_layout.xaml"
    };
    constexpr maui::fixed_string custom_layout_xaml{custom_layout_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(custom_layout_xaml);
    ...
    static_assert(bytes_contain(custom_layout_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(custom_layout_xaml_bytes, "x:Class=\"MauiReference.Pages.CustomLayoutPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> custom_layout_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, custom_layout_xaml>(options);
    }
} // namespace examples::Views
