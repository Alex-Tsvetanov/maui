// table_view.xaml.cpp — implements examples::Views::table_view_page() (declared in table_view.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/table_view.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.TableViewPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char table_view_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/table_view.xaml"
    };
    constexpr maui::fixed_string table_view_xaml{table_view_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(table_view_xaml);
    ...
    static_assert(bytes_contain(table_view_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(table_view_xaml_bytes, "x:Class=\"MauiReference.Pages.TableViewPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> table_view_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, table_view_xaml>(options);
    }
} // namespace examples::Views
