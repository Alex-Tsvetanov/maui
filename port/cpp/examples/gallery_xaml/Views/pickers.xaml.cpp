...
// port/maui-reference/pages/pickers.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.PickersPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/pickers.xaml"
    };
    constexpr maui::fixed_string pickers_xaml{pickers_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(pickers_xaml);
    ...
    static_assert(bytes_contain(pickers_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(pickers_xaml_bytes, "x:Class=\"MauiReference.Pages.PickersPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> pickers_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, pickers_xaml>(options);
    }
} // namespace examples::Views
