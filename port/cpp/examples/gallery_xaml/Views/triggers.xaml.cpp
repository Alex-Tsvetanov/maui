// triggers.xaml.cpp — implements examples::Views::triggers_page() (declared in triggers.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/triggers.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.TriggersPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char triggers_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/triggers.xaml"
    };
    constexpr maui::fixed_string triggers_xaml{triggers_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(triggers_xaml);
    ...
    static_assert(bytes_contain(triggers_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(triggers_xaml_bytes, "x:Class=\"MauiReference.Pages.TriggersPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> triggers_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, triggers_xaml>(options);
    }
} // namespace examples::Views
