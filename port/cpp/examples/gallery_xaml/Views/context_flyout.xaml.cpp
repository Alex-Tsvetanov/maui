// context_flyout.xaml.cpp — implements examples::Views::context_flyout_page() (declared in context_flyout.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/context_flyout.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ContextFlyoutPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char context_flyout_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/context_flyout.xaml"
    };
    constexpr maui::fixed_string context_flyout_xaml{context_flyout_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(context_flyout_xaml);
    ...
    static_assert(bytes_contain(context_flyout_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(context_flyout_xaml_bytes, "x:Class=\"MauiReference.Pages.ContextFlyoutPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> context_flyout_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, context_flyout_xaml>(options);
    }
} // namespace examples::Views
