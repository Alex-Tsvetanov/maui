...
// port/maui-reference/pages/web_view.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.WebViewPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/web_view.xaml"
    };
    constexpr maui::fixed_string web_view_xaml{web_view_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(web_view_xaml);
    ...
    static_assert(bytes_contain(web_view_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(web_view_xaml_bytes, "x:Class=\"MauiReference.Pages.WebViewPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> web_view_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, web_view_xaml>(options);
    }
} // namespace examples::Views
