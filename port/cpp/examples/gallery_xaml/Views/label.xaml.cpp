// label.xaml.cpp — implements examples::Views::label_page() (declared in label.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/label.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.LabelPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char label_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/label.xaml"
    };
    constexpr maui::fixed_string label_xaml{label_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(label_xaml);
    ...
    static_assert(bytes_contain(label_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(label_xaml_bytes, "x:Class=\"MauiReference.Pages.LabelPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> label_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, label_xaml>(options);
    }
} // namespace examples::Views
