// ios_search_bar.xaml.cpp — implements examples::Views::ios_search_bar_page() (declared in ios_search_bar.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/ios_search_bar.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.IosSearchBarPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char ios_search_bar_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/ios_search_bar.xaml"
    };
    constexpr maui::fixed_string ios_search_bar_xaml{ios_search_bar_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(ios_search_bar_xaml);
    ...
    static_assert(bytes_contain(ios_search_bar_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(ios_search_bar_xaml_bytes, "x:Class=\"MauiReference.Pages.IosSearchBarPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> ios_search_bar_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, ios_search_bar_xaml>(options);
    }
} // namespace examples::Views
