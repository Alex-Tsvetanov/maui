...
// port/maui-reference/pages/scroll_to_group.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ScrollToGroupPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/scroll_to_group.xaml"
    };
    ...
    static_assert(bytes_contain(scroll_to_group_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(scroll_to_group_xaml_bytes, "x:Class=\"MauiReference.Pages.ScrollToGroupPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> scroll_to_group_page(const maui::xaml::xaml_load_options& options)
    {
        auto page = maui::build_page<maui::no_view_model, scroll_to_group_xaml>(options);
