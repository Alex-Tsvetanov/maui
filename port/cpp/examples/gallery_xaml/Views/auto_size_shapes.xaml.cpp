...
// port/maui-reference/pages/auto_size_shapes.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.AutoSizeShapesPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/auto_size_shapes.xaml"
    };
    constexpr maui::fixed_string auto_size_shapes_xaml{auto_size_shapes_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(auto_size_shapes_xaml);
    ...
    static_assert(bytes_contain(auto_size_shapes_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(auto_size_shapes_xaml_bytes, "x:Class=\"MauiReference.Pages.AutoSizeShapesPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> auto_size_shapes_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, auto_size_shapes_xaml>(options);
    }
} // namespace examples::Views
