...
// port/maui-reference/pages/visual_states.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.VisualStatesPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/visual_states.xaml"
    };
    constexpr maui::fixed_string visual_states_xaml{visual_states_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(visual_states_xaml);
    ...
    static_assert(bytes_contain(visual_states_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(visual_states_xaml_bytes, "x:Class=\"MauiReference.Pages.VisualStatesPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> visual_states_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, visual_states_xaml>(options);
    }
} // namespace examples::Views
