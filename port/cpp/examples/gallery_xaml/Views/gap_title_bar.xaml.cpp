// gap_title_bar.xaml.cpp — implements examples::Views::gap_title_bar_page() (declared in gap_title_bar.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/gap_title_bar.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.GapTitleBarPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char gap_title_bar_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/gap_title_bar.xaml"
    };
    constexpr maui::fixed_string gap_title_bar_xaml{gap_title_bar_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(gap_title_bar_xaml);
    ...
    static_assert(bytes_contain(gap_title_bar_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(gap_title_bar_xaml_bytes, "x:Class=\"MauiReference.Pages.GapTitleBarPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> gap_title_bar_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, gap_title_bar_xaml>(options);
    }
} // namespace examples::Views
