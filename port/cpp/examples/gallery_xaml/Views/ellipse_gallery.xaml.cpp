// ellipse_gallery.xaml.cpp — implements examples::Views::ellipse_gallery_page() (declared in ellipse_gallery.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/ellipse_gallery.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.EllipseGalleryPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char ellipse_gallery_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/ellipse_gallery.xaml"
    };
    constexpr maui::fixed_string ellipse_gallery_xaml{ellipse_gallery_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(ellipse_gallery_xaml);
    ...
    static_assert(bytes_contain(ellipse_gallery_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(ellipse_gallery_xaml_bytes, "x:Class=\"MauiReference.Pages.EllipseGalleryPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> ellipse_gallery_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, ellipse_gallery_xaml>(options);
    }
} // namespace examples::Views
