...
// port/maui-reference/pages/some_empty_groups.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.SomeEmptyGroupsPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/some_empty_groups.xaml"
    };
    ...
    static_assert(bytes_contain(some_empty_groups_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(some_empty_groups_xaml_bytes, "x:Class=\"MauiReference.Pages.SomeEmptyGroupsPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> some_empty_groups_page(const maui::xaml::xaml_load_options& options)
    {
        auto page = maui::build_page<maui::no_view_model, some_empty_groups_xaml>(options);
        ...
        {
            list->set_items_source(examples::ViewModels::some_teams());
        }
