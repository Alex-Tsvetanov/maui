...
// port/maui-reference/pages/date_picker.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.DatePickerPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/date_picker.xaml"
    };
    constexpr maui::fixed_string date_picker_xaml{date_picker_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(date_picker_xaml);
    ...
    static_assert(bytes_contain(date_picker_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(date_picker_xaml_bytes, "x:Class=\"MauiReference.Pages.DatePickerPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> date_picker_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, date_picker_xaml>(options);
    }
} // namespace examples::Views
