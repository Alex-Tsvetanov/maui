// grid_layout.xaml.cpp — implements examples::Views::grid_layout_page() (declared in grid_layout.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/grid_layout.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.GridLayoutPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char grid_layout_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/grid_layout.xaml"
    };
    constexpr maui::fixed_string grid_layout_xaml{grid_layout_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(grid_layout_xaml);
    ...
    static_assert(bytes_contain(grid_layout_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(grid_layout_xaml_bytes, "x:Class=\"MauiReference.Pages.GridLayoutPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> grid_layout_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, grid_layout_xaml>(options);
    }
} // namespace examples::Views
