// gap_toolbar.xaml.cpp — implements examples::Views::gap_toolbar_page() (declared in gap_toolbar.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/gap_toolbar.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.GapToolbarPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char gap_toolbar_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/gap_toolbar.xaml"
    };
    constexpr maui::fixed_string gap_toolbar_xaml{gap_toolbar_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(gap_toolbar_xaml);
    ...
    static_assert(bytes_contain(gap_toolbar_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(gap_toolbar_xaml_bytes, "x:Class=\"MauiReference.Pages.GapToolbarPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> gap_toolbar_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, gap_toolbar_xaml>(options);
    }
} // namespace examples::Views
