...
// port/maui-reference/pages/drag_drop.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.DragDropPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/drag_drop.xaml"
    };
    constexpr maui::fixed_string drag_drop_xaml{drag_drop_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(drag_drop_xaml);
    ...
    static_assert(bytes_contain(drag_drop_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(drag_drop_xaml_bytes, "x:Class=\"MauiReference.Pages.DragDropPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> drag_drop_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, drag_drop_xaml>(options);
    }
} // namespace examples::Views
