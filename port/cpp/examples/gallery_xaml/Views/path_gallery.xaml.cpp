...
// port/maui-reference/pages/path_gallery.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.PathGalleryPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/path_gallery.xaml"
    ...
    static_assert(bytes_contain(path_gallery_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(path_gallery_xaml_bytes, "x:Class=\"MauiReference.Pages.PathGalleryPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> path_gallery_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, path_gallery_xaml>(options);
    }
} // namespace examples::Views
