// header_footer.xaml.cpp — implements examples::Views::header_footer_page() (declared in header_footer.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/header_footer.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.HeaderFooterPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char header_footer_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/header_footer.xaml"
    };
    constexpr maui::fixed_string header_footer_xaml{header_footer_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(header_footer_xaml);
    ...
    static_assert(bytes_contain(header_footer_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(header_footer_xaml_bytes, "x:Class=\"MauiReference.Pages.HeaderFooterPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> header_footer_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, header_footer_xaml>(options);
    }
} // namespace examples::Views
