...
// port/maui-reference/pages/controls_stack.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ControlsStackPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/controls_stack.xaml"
    };
    constexpr maui::fixed_string controls_stack_xaml{controls_stack_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(controls_stack_xaml);
    ...
    static_assert(bytes_contain(controls_stack_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(controls_stack_xaml_bytes, "x:Class=\"MauiReference.Pages.ControlsStackPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> controls_stack_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, controls_stack_xaml>(options);
    }
} // namespace examples::Views
