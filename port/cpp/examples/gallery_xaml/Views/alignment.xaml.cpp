// alignment.xaml.cpp — implements examples::Views::alignment_page() (declared in alignment.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/alignment.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.AlignmentPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char alignment_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/alignment.xaml"
    };
    constexpr maui::fixed_string alignment_xaml{alignment_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(alignment_xaml);
    ...
    static_assert(bytes_contain(alignment_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(alignment_xaml_bytes, "x:Class=\"MauiReference.Pages.AlignmentPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> alignment_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, alignment_xaml>(options);
    }
} // namespace examples::Views
