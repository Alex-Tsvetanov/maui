// behaviors.xaml.cpp — implements examples::Views::behaviors_page() (declared in behaviors.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/behaviors.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.BehaviorsPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char behaviors_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/behaviors.xaml"
    };
    constexpr maui::fixed_string behaviors_xaml{behaviors_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(behaviors_xaml);
    ...
    static_assert(bytes_contain(behaviors_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(behaviors_xaml_bytes, "x:Class=\"MauiReference.Pages.BehaviorsPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> behaviors_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, behaviors_xaml>(options);
    }
} // namespace examples::Views
