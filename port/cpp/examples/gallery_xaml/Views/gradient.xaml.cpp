...
// port/maui-reference/pages/gradient.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.GradientPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/gradient.xaml"
    };
    constexpr maui::fixed_string gradient_xaml{gradient_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(gradient_xaml);
    ...
    static_assert(bytes_contain(gradient_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(gradient_xaml_bytes, "x:Class=\"MauiReference.Pages.GradientPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> gradient_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, gradient_xaml>(options);
    }
} // namespace examples::Views
