...
// port/maui-reference/pages/update_path_data.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.UpdatePathDataPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/update_path_data.xaml"
    };
    constexpr maui::fixed_string update_path_data_xaml{update_path_data_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(update_path_data_xaml);
    ...
    static_assert(bytes_contain(update_path_data_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(update_path_data_xaml_bytes, "x:Class=\"MauiReference.Pages.UpdatePathDataPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> update_path_data_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, update_path_data_xaml>(options);
    }
} // namespace examples::Views
