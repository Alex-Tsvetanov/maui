...
// port/maui-reference/pages/layout_is_enabled.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.LayoutIsEnabledPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/layout_is_enabled.xaml"
    };
    constexpr maui::fixed_string layout_is_enabled_xaml{layout_is_enabled_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(layout_is_enabled_xaml);
    ...
    static_assert(bytes_contain(layout_is_enabled_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(layout_is_enabled_xaml_bytes, "x:Class=\"MauiReference.Pages.LayoutIsEnabledPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> layout_is_enabled_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, layout_is_enabled_xaml>(options);
    }
} // namespace examples::Views
