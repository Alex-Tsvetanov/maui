// staggered_layout.xaml.cpp — implements examples::Views::staggered_layout_page() (declared in
// staggered_layout.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/staggered_layout.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.StaggeredLayoutPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char staggered_layout_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/staggered_layout.xaml"
    };
    constexpr maui::fixed_string staggered_layout_xaml{staggered_layout_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(staggered_layout_xaml);
    ...
    static_assert(bytes_contain(staggered_layout_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(staggered_layout_xaml_bytes, "x:Class=\"MauiReference.Pages.StaggeredLayoutPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> staggered_layout_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, staggered_layout_xaml>(options);
    }
} // namespace examples::Views
