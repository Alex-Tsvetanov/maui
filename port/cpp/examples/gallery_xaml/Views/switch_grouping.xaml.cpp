...
// port/maui-reference/pages/switch_grouping.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.SwitchGroupingPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/switch_grouping.xaml"
    };
    ...
    static_assert(bytes_contain(switch_grouping_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(switch_grouping_xaml_bytes, "x:Class=\"MauiReference.Pages.SwitchGroupingPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> switch_grouping_page(const maui::xaml::xaml_load_options& options)
    {
        auto page = maui::build_page<maui::no_view_model, switch_grouping_xaml>(options);
