...
// port/maui-reference/pages/styles.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.StylesPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/styles.xaml"
    };
    constexpr maui::fixed_string styles_xaml{styles_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(styles_xaml);
    ...
    static_assert(bytes_contain(styles_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(styles_xaml_bytes, "x:Class=\"MauiReference.Pages.StylesPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> styles_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, styles_xaml>(options);
    }
} // namespace examples::Views
