...
// port/maui-reference/pages/pointer_gesture.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.PointerGesturePage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/pointer_gesture.xaml"
    };
    constexpr maui::fixed_string pointer_gesture_xaml{pointer_gesture_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(pointer_gesture_xaml);
    ...
    static_assert(bytes_contain(pointer_gesture_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(pointer_gesture_xaml_bytes, "x:Class=\"MauiReference.Pages.PointerGesturePage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> pointer_gesture_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, pointer_gesture_xaml>(options);
    }
} // namespace examples::Views
