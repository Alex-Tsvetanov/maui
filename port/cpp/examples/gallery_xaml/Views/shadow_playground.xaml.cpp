...
// port/maui-reference/pages/shadow_playground.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ShadowPlaygroundPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/shadow_playground.xaml"
    };
    constexpr maui::fixed_string shadow_playground_xaml{shadow_playground_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(shadow_playground_xaml);
    ...
    static_assert(bytes_contain(shadow_playground_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(shadow_playground_xaml_bytes, "x:Class=\"MauiReference.Pages.ShadowPlaygroundPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> shadow_playground_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, shadow_playground_xaml>(options);
    }
} // namespace examples::Views
