...
// port/maui-reference/pages/cv_visual_states.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.CvVisualStatesPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/cv_visual_states.xaml"
    };
    constexpr maui::fixed_string cv_visual_states_xaml{cv_visual_states_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(cv_visual_states_xaml);
    ...
    static_assert(bytes_contain(cv_visual_states_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(cv_visual_states_xaml_bytes, "x:Class=\"MauiReference.Pages.CvVisualStatesPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> cv_visual_states_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, cv_visual_states_xaml>(options);
    }
} // namespace examples::Views
