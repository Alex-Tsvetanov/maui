...
// port/maui-reference/pages/invalidate_brush.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.InvalidateBrushPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/invalidate_brush.xaml"
    };
    constexpr maui::fixed_string invalidate_brush_xaml{invalidate_brush_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(invalidate_brush_xaml);
    ...
    static_assert(bytes_contain(invalidate_brush_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(invalidate_brush_xaml_bytes, "x:Class=\"MauiReference.Pages.InvalidateBrushPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> invalidate_brush_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, invalidate_brush_xaml>(options);
    }
} // namespace examples::Views
