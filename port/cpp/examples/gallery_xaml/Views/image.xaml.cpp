...
// port/maui-reference/pages/image.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ImagePage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/image.xaml"
    };
    constexpr maui::fixed_string image_xaml{image_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(image_xaml);
    ...
    static_assert(bytes_contain(image_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(image_xaml_bytes, "x:Class=\"MauiReference.Pages.ImagePage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> image_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, image_xaml>(options);
    }
} // namespace examples::Views
