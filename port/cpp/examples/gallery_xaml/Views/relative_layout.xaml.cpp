...
// port/maui-reference/pages/relative_layout.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.RelativeLayoutPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/relative_layout.xaml"
    };
    constexpr maui::fixed_string relative_layout_xaml{relative_layout_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(relative_layout_xaml);
    ...
    static_assert(bytes_contain(relative_layout_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(relative_layout_xaml_bytes, "x:Class=\"MauiReference.Pages.RelativeLayoutPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> relative_layout_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, relative_layout_xaml>(options);
    }
} // namespace examples::Views
