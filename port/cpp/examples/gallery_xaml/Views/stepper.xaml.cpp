// stepper.xaml.cpp — implements examples::Views::stepper_page() (declared in stepper.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/stepper.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.StepperPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char stepper_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/stepper.xaml"
    };
    constexpr maui::fixed_string stepper_xaml{stepper_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(stepper_xaml);
    ...
    static_assert(bytes_contain(stepper_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(stepper_xaml_bytes, "x:Class=\"MauiReference.Pages.StepperPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> stepper_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, stepper_xaml>(options);
    }
} // namespace examples::Views
