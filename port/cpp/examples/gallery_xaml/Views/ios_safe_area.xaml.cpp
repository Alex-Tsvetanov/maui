// ios_safe_area.xaml.cpp — implements examples::Views::ios_safe_area_page() (declared in ios_safe_area.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/ios_safe_area.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.IosSafeAreaPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char ios_safe_area_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/ios_safe_area.xaml"
    };
    constexpr maui::fixed_string ios_safe_area_xaml{ios_safe_area_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(ios_safe_area_xaml);
    ...
    static_assert(bytes_contain(ios_safe_area_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(ios_safe_area_xaml_bytes, "x:Class=\"MauiReference.Pages.IosSafeAreaPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> ios_safe_area_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, ios_safe_area_xaml>(options);
    }
} // namespace examples::Views
