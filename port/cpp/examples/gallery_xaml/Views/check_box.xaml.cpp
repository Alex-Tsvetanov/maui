// check_box.xaml.cpp — implements examples::Views::check_box_page() (declared in check_box.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/check_box.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.CheckBoxPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char check_box_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/check_box.xaml"
    };
    constexpr maui::fixed_string check_box_xaml{check_box_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(check_box_xaml);
    ...
    static_assert(bytes_contain(check_box_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(check_box_xaml_bytes, "x:Class=\"MauiReference.Pages.CheckBoxPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> check_box_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, check_box_xaml>(options);
    }
} // namespace examples::Views
