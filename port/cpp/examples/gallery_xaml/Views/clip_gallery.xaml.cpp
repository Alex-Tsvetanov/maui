...
// port/maui-reference/pages/clip_gallery.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ClipGalleryPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/clip_gallery.xaml"
    };
    constexpr maui::fixed_string clip_gallery_xaml{clip_gallery_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(clip_gallery_xaml);
    ...
    static_assert(bytes_contain(clip_gallery_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(clip_gallery_xaml_bytes, "x:Class=\"MauiReference.Pages.ClipGalleryPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> clip_gallery_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, clip_gallery_xaml>(options);
    }
} // namespace examples::Views
