...
// port/maui-reference/pages/line_join_gallery.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.LineJoinGalleryPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/line_join_gallery.xaml"
    };
    constexpr maui::fixed_string line_join_gallery_xaml{line_join_gallery_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(line_join_gallery_xaml);
    ...
    static_assert(bytes_contain(line_join_gallery_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(line_join_gallery_xaml_bytes, "x:Class=\"MauiReference.Pages.LineJoinGalleryPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> line_join_gallery_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, line_join_gallery_xaml>(options);
    }
} // namespace examples::Views
