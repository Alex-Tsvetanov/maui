// effects.xaml.cpp — implements examples::Views::effects_page() (declared in effects.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/effects.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.EffectsPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char effects_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/effects.xaml"
    };
    constexpr maui::fixed_string effects_xaml{effects_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(effects_xaml);
    ...
    static_assert(bytes_contain(effects_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(effects_xaml_bytes, "x:Class=\"MauiReference.Pages.EffectsPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> effects_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, effects_xaml>(options);
    }
} // namespace examples::Views
