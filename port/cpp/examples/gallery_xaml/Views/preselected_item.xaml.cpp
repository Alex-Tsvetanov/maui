...
// port/maui-reference/pages/preselected_item.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.PreselectedItemPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/preselected_item.xaml"
    };
    constexpr maui::fixed_string preselected_item_xaml{preselected_item_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(preselected_item_xaml);
    ...
    static_assert(bytes_contain(preselected_item_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(preselected_item_xaml_bytes, "x:Class=\"MauiReference.Pages.PreselectedItemPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> preselected_item_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, preselected_item_xaml>(options);
    }
} // namespace examples::Views
