// collectionview.xaml.cpp — implements examples::Views::collectionview_page() (declared in collectionview.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/collectionview.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.CollectionviewPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char collectionview_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/collectionview.xaml"
    };
    constexpr maui::fixed_string collectionview_xaml{collectionview_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(collectionview_xaml);
    ...
    static_assert(bytes_contain(collectionview_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(collectionview_xaml_bytes, "x:Class=\"MauiReference.Pages.CollectionviewPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> collectionview_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, collectionview_xaml>(options);
    }
} // namespace examples::Views
