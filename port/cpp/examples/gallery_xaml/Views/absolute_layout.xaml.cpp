...
// port/maui-reference/pages/absolute_layout.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.AbsoluteLayoutPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/absolute_layout.xaml"
    };
    constexpr maui::fixed_string absolute_layout_xaml{absolute_layout_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(absolute_layout_xaml);
    ...
    static_assert(bytes_contain(absolute_layout_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(absolute_layout_xaml_bytes, "x:Class=\"MauiReference.Pages.AbsoluteLayoutPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> absolute_layout_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, absolute_layout_xaml>(options);
    }
} // namespace examples::Views
