// containers.xaml.cpp — implements examples::Views::containers_page() (declared in containers.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/containers.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ContainersPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char containers_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/containers.xaml"
    };
    constexpr maui::fixed_string containers_xaml{containers_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(containers_xaml);
    ...
    static_assert(bytes_contain(containers_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(containers_xaml_bytes, "x:Class=\"MauiReference.Pages.ContainersPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> containers_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, containers_xaml>(options);
    }
} // namespace examples::Views
