// carousel_view.xaml.cpp — implements examples::Views::carousel_view_page() (declared in carousel_view.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/carousel_view.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.CarouselViewPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char carousel_view_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/carousel_view.xaml"
    };
    constexpr maui::fixed_string carousel_view_xaml{carousel_view_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(carousel_view_xaml);
    ...
    static_assert(bytes_contain(carousel_view_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(carousel_view_xaml_bytes, "x:Class=\"MauiReference.Pages.CarouselViewPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> carousel_view_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, carousel_view_xaml>(options);
    }
} // namespace examples::Views
