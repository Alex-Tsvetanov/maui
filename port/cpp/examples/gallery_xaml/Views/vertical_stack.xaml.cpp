// vertical_stack.xaml.cpp — implements examples::Views::vertical_stack_page() (declared in vertical_stack.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/vertical_stack.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.VerticalStackPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char vertical_stack_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/vertical_stack.xaml"
    };
    constexpr maui::fixed_string vertical_stack_xaml{vertical_stack_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(vertical_stack_xaml);
    ...
    static_assert(bytes_contain(vertical_stack_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(vertical_stack_xaml_bytes, "x:Class=\"MauiReference.Pages.VerticalStackPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> vertical_stack_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, vertical_stack_xaml>(options);
    }
} // namespace examples::Views
