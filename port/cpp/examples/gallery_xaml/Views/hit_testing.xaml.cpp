...
// port/maui-reference/pages/hit_testing.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.HitTestingPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/hit_testing.xaml"
    };
    constexpr maui::fixed_string hit_testing_xaml{hit_testing_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(hit_testing_xaml);
    ...
    static_assert(bytes_contain(hit_testing_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(hit_testing_xaml_bytes, "x:Class=\"MauiReference.Pages.HitTestingPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> hit_testing_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, hit_testing_xaml>(options);
    }
} // namespace examples::Views
