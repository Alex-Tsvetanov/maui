...
// port/maui-reference/pages/progress_bar.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.ProgressBarPage) — and build_page hydrates it through the runtime
...
#embed "../../../../maui-reference/pages/progress_bar.xaml"
    };
    constexpr maui::fixed_string progress_bar_xaml{progress_bar_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(progress_bar_xaml);
    ...
    static_assert(bytes_contain(progress_bar_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(progress_bar_xaml_bytes, "x:Class=\"MauiReference.Pages.ProgressBarPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> progress_bar_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, progress_bar_xaml>(options);
    }
} // namespace examples::Views
