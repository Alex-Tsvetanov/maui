// borderless.xaml.cpp — implements examples::Views::borderless_page() (declared in borderless.xaml.hpp).
//
// This TU OWNS the compile-time XAML: the COMPILER embeds the CANONICAL SHARED page
// port/maui-reference/pages/borderless.xaml — the exact same bytes real .NET MAUI compiles via XamlC +
// code-behind (MauiReference.Pages.BorderlessPage) — and build_page hydrates it through the runtime
...
namespace
{
    ...
    constexpr unsigned char borderless_xaml_bytes[] = {
#embed "../../../../maui-reference/pages/borderless.xaml"
    };
    constexpr maui::fixed_string borderless_xaml{borderless_xaml_bytes};

    MAUI_XAML_REJECT_EVENT_ATTRIBUTES(borderless_xaml);
    ...
    static_assert(bytes_contain(borderless_xaml_bytes, "<ContentPage"));
    static_assert(bytes_contain(borderless_xaml_bytes, "x:Class=\"MauiReference.Pages.BorderlessPage\""));
} // namespace

namespace examples::Views
{
    std::unique_ptr<maui::controls::content_page> borderless_page(const maui::xaml::xaml_load_options& options)
    {
        // unique_ptr<page_impl<no_view_model>> upcasts to unique_ptr<content_page> on return.
        return maui::build_page<maui::no_view_model, borderless_xaml>(options);
    }
} // namespace examples::Views
