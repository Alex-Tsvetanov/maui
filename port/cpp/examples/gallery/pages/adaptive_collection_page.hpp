...
// directly. The collection_view starts in the < 600 state (linear), matching a freshly-appeared narrow page
// before the first SizeChanged fires. (The shared XAML is a bare CollectionView, so the page content IS the
// list — an earlier twin added a layout-kind readout label that diverged from MAUI; it is dropped.)
//
...
namespace maui::samples
{
    ...
    public:
                  ...
                  "Item 1", "Item 2", "Item 3", "Item 4", "Item 5", "Item 6", "Item 7", "Item 8"}))
        {
            ...
            // The shared adaptive_collection.xaml wraps the CollectionView in a bare Grid (no readout label) —
            // ContentPage > Grid > CollectionView. The adaptive layout swap (apply_width/use_grid/use_linear)
            // still drives list_.set_items_layout; the earlier readout label diverged from MAUI and is dropped.
            grid_.add(list_);
