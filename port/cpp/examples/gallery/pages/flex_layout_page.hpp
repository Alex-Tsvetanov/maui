...
namespace maui::samples
{
    ...
    public:
        flex_layout_page()
        {
            ...
            // The shared flex_layout.xaml sets NO explicit TextColor on these labels, so — like the loader
            // (xaml) column, which matches MAUI in BOTH themes — they are left UNSET here. The iOS/maccatalyst
            // label handler resolves an unset text color to UIColor.labelColor (the DYNAMIC system label color:
            // dark in light appearance, WHITE in dark), so the labels read on the colored FlexLayout regions
            // exactly as MAUI does in both themes. (An earlier twin pinned a FIXED dark #3B3B3B here, which was
            // right in light but stayed dark — near-invisible — under dark appearance where MAUI goes white.)
