...
| M-android parity | Android VISUAL-PARITY board (MAUI ┃ C++ ┃ C++&XAML, light+dark) | ✅ **158 green / 12 yellow / 2 red** on BOTH the `cpp` and `xaml` columns of all 172 pages (`docs/comparison/README.md`, scored by `tools/parity/pixel_score.py`: green = SSIM≥0.98 AND ≤1.0% differing pixels, worst-of-both-themes, status bar cropped). Both reds are EXEMPT live external content (`context_flyout` = a live bing.com WebView, `hybrid_web_view` = a browser error page). The 12 yellows are each measured to a documented FLOOR, not a to-do: the 6-page **border family** (MAUI strokes at a fractional pixel offset the port's dp→px path conversion cannot reproduce — a canvas AA stroke was implemented and measured TWICE, independently, and regressed both times; `border` additionally carries a 33px MAUI safe-area-vs-centering offset on non-filling centered content), `date_picker`/`time_picker` (~0.7%, sub-pixel picker-field height rounding), `clip_views`/`ios_blur_effect` (sub-pixel AA, blocked by SSIM not diff%), `layout_is_enabled` (1.55%, right-column button text measure), `activity_indicator` (a live indeterminate spinner caught at different rotations — unsynchronizable). **LANDED 2026-07-27 (`2d5733e14c`):** the AndroidX AppCompat + Google Material AARs are now LINKED (`tools/parity/android-aar-lib.sh` stages them out of the local NuGet cache — the same AARs MAUI builds against — and supplies what AGP would: classes.jar on the javac classpath + into d8, each res/ tree aapt2-compiled and merged, an R.java per `--extra-packages`, cached under `build/android/aardeps`). `aapt2 link` runs BEFORE javac (it emits those R classes) and the APK carries native multidex (AppCompat+Material exceed the 64K method limit). The Activity theme is MAUI's OWN `Maui.MainTheme` reproduced from the oracle on the real `Theme.MaterialComponents.DayNight`, so widget DEFAULTS now resolve the way MAUI's do. Full-board recapture (734 captures, 4 serial passes): ZERO regressions; picker 0.79->0.29%, date_picker 0.78->0.57% light, time_picker 0.71->0.53% light, stepper 0.03->0.00%. **Still open on top of it:** swapping the stand-in widgets for MAUI's real ones (`AppCompatEditText` for the picker family, `MaterialButton`) — now unblocked, and the likeliest route to the pickers' remaining DARK-theme SSIM gate (0.967/0.970). |
