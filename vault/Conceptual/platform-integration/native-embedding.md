...
ms_date: "10/18/2024"
source: "https://learn.microsoft.com/dotnet/maui/platform-integration/native-embedding?view=net-maui-10.0"
---
...
Typically, a .NET Multi-platform App UI (.NET MAUI) app includes pages that contain layouts, such as [[Grid (Controls)|Grid]], and layouts that contain views, such as [[Button (Controls)|Button]]. Pages, layouts, and views all derive from [[Element|Element]]. Native embedding enables any .NET MAUI controls that derive from [[Element|Element]] to be consumed in .NET for Android, .NET for iOS, .NET for Mac Catalyst, and WinUI native apps.
...
- Create a [[MauiApp|MauiApp]] object.
- Create a [[MauiContext|MauiContext]] object from the [[MauiApp|MauiApp]] object. The [[MauiContext|MauiContext]] object will be used to obtain a native view from the .NET MAUI view.
...
In this example, the [[MauiApp|MauiApp]] object is created using lazy initialization. The `UseMauiEmbedding` extension method is invoked on the [[MauiAppBuilder|MauiAppBuilder]] object. Therefore, your native app project should include a reference to the .NET MAUI class library project you created that contains this extension method. A [[MauiContext|MauiContext]] object is then created from the [[MauiApp|MauiApp]] object, with a `bool` determining where the context is scoped from. The [[MauiContext|MauiContext]] object will be used when converting .NET MAUI controls to native types.
...
A .NET MAUI view can then be created and converted to a native view with the `ToPlatformEmbedded` extension method, which requires the [[MauiContext|MauiContext]] object as an argument.
...
In this example, the [[MauiApp|MauiApp]] object is created as a shared, static instance. When this object is created, `MauiProgram.CreateMauiApp` is called which in turn calls the `UseMauiEmbedding` extension method on the [[MauiAppBuilder|MauiAppBuilder]] object. Therefore, your native app project should include a reference to the .NET MAUI class library project you created that contains your `MauiProgram` class and your .NET MAUI UI. A [[MauiContext|MauiContext]] object is then created from the [[MauiApp|MauiApp]] object, that's scoped to the [[MauiApp|MauiApp]] object. The [[MauiContext|MauiContext]] object will be used when converting .NET MAUI controls to native types.
...
- Create a [[MauiApp|MauiApp]] object.
- Create a [[MauiContext|MauiContext]] object with the `CreateEmbeddedWindowContext` method. The [[MauiContext|MauiContext]] object will be used to obtain a native view from the .NET MAUI view.
...
In this example, the [[MauiApp|MauiApp]] object is created as a shared, static instance. When this object is created, `MauiProgram.CreateMauiApp` is called which in turn calls the `UseMauiEmbedding` extension method on the [[MauiAppBuilder|MauiAppBuilder]] object. Therefore, your native app project should include a reference to the .NET MAUI class library project you created that contains your `MauiProgram` class and your .NET MAUI UI. A [[MauiContext|MauiContext]] object is then created with the `CreateEmbeddedWindowContext` method, that's scoped to the window. The [[MauiContext|MauiContext]] object will be used when converting .NET MAUI controls to native types.
...
In this example, a [[ContentView (Controls)|ContentView]]-derived object is converted to an Android `View` object.
...
> [!NOTE]
> The `ToPlatformEmbedded` extension method is in the `Embedding` namespace. Therefore your native app project should include a `using` statement for that namespace.
...
In this example, a [[ContentView (Controls)|ContentView]]-derived object is converted to a `UIView` object and then width and height constraints are set on it to allow interaction.
...
In this example, a [[ContentView (Controls)|ContentView]]-derived object is converted to a `UIView` object.
...
In this example, a [[ContentPage|ContentPage]]-derived object is converted to a `UIViewController`.
...
In this example, a [[ContentView (Controls)|ContentView]]-derived object is converted to a `FrameworkElement` object. The `FrameworkElement` object can then be set as the content of a WinUI page.
