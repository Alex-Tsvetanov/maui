...
ms_date: "02/02/2023"
source: "https://learn.microsoft.com/dotnet/maui/platform-integration/device/sensors?view=net-maui-10.0"
---
...
Sensor speed sets the speed in which a sensor will return data to your app. When you start a sensor, you provide the desired sensor speed with the [[SensorSpeed|SensorSpeed]] enumeration:

- [[SensorSpeed.Fastest|Fastest]]\
Get the sensor data as fast as possible (not guaranteed to return on UI thread).

- [[SensorSpeed.Game|Game]]\
Rate suitable for games (not guaranteed to return on UI thread).

- [[SensorSpeed.UI|UI]]\
Rate suitable for general user interface.

- [[SensorSpeed.Default|Default]]\
...
In .NET 8, [[SensorSpeed|SensorSpeed]] intervals are identical across all platforms:

- [[SensorSpeed.Default|Default]] uses an interval of 200ms.
- [[SensorSpeed.UI|UI]] uses an interval of 60ms.
- [[SensorSpeed.Game|Game]] uses an interval of 20ms.
- [[SensorSpeed.Fastest|Fastest]] uses an interval of 5ms.

### Sensor event handlers

Event handlers added to sensors with either the [[SensorSpeed.Game|Game]] or [[SensorSpeed.Fastest|Fastest]] speeds **aren't** guaranteed to run on the UI thread. If the event handler needs to access user-interface elements, use the [[main-thread|`MainThread.BeginInvokeOnMainThread`]] method to run that code on the UI thread.
...
The [[IAccelerometer|IAccelerometer]] interface provides access to the sensor, and is available through the [[Accelerometer.Default|Accelerometer.Default]] property. Both the `IAccelerometer` interface and `Accelerometer` class are contained in the `Microsoft.Maui.Devices.Sensors` namespace.
...
To start monitoring the accelerometer sensor, call the `IAccelerometer.Start%2A` method. .NET MAUI sends accelerometer data changes to your app by raising the [[IAccelerometer.ReadingChanged|IAccelerometer.ReadingChanged]] event. Use the `IAccelerometer.Stop%2A` method to stop monitoring the sensor. You can detect the monitoring state of the accelerometer with the `IAccelerometer.IsMonitoring%2A` property, which will be `true` if the accelerometer was started and is currently being monitored.
...
The [[IBarometer|IBarometer]] interface provides access to the sensor, and is available through the [[Barometer.Default|Barometer.Default]] property. Both the `IBarometer` interface and `Barometer` class are contained in the `Microsoft.Maui.Devices.Sensors` namespace.

To start monitoring the barometer sensor, call the `IBarometer.Start%2A` method. .NET MAUI sends air pressure readings to your app by raising the [[IBarometer.ReadingChanged|IBarometer.ReadingChanged]] event. Use the `IBarometer.Stop%2A` method to stop monitoring the sensor. You can detect the monitoring state of the barometer with the `IBarometer.IsMonitoring%2A` property, which will be `true` if the barometer is currently being monitored.
...
This API uses [CMAltimeter](https://developer.apple.com/documentation/coremotion/cmaltimeter#//apple_ref/occ/cl/CMAltimeter) to monitor pressure changes, which is a hardware feature that was added to iPhone 6 and newer devices. A [[FeatureNotSupportedException|FeatureNotSupportedException]] will be thrown on devices that don't support the altimeter, the sensor used to report air pressure.

[[SensorSpeed|SensorSpeed]] isn't used by this API, as it isn't supported on iOS.
...
The [[ICompass|ICompass]] interface provides access to the sensor, and is available through the [[Compass.Default|Compass.Default]] property. Both the `ICompass` interface and `Compass` class are contained in the `Microsoft.Maui.Devices.Sensors` namespace.

To start monitoring the compass sensor, call the `ICompass.Start%2A` method. .NET MAUI raises the [[ICompass.ReadingChanged|ICompass.ReadingChanged]] event when the compass heading changes. Use the `ICompass.Stop%2A` method to stop monitoring the sensor. You can detect the monitoring state of the compass with the `ICompass.IsMonitoring%2A` property, which will be `true` if the compass is currently being monitored.
...
> [!NOTE]
> If your app needs to gather accelerometer sensor data using the [[SensorSpeed.Fastest|Fastest]] sensor speed, you must declare the `HIGH_SAMPLING_RATE_SENSORS` permission. For more information, see [Accelerometer](#accelerometer).
...
To start monitoring the accelerometer sensor, call the `IAccelerometer.Start%2A` method. When a shake is detected, the [[IAccelerometer.ShakeDetected|IAccelerometer.ShakeDetected]] event is raised.  Use the `IAccelerometer.Stop%2A` method to stop monitoring the sensor. You can detect the monitoring state of the accelerometer with the `IAccelerometer.IsMonitoring` property, which will be `true` if the accelerometer was started and is currently being monitored.

It's recommended to use [[SensorSpeed.Game|Game]] or faster for the [[SensorSpeed|SensorSpeed]].
...
The [[IGyroscope|IGyroscope]] interface provides access to the sensor, and is available through the [[Gyroscope.Default|Gyroscope.Default]] property. Both the `IGyroscope` interface and `Gyroscope` class are contained in the `Microsoft.Maui.Devices.Sensors` namespace.
...
To start monitoring the gyroscope sensor, call the `IGyroscope.Start%2A` method. .NET MAUI sends gyroscope data changes to your app by raising the [[IGyroscope.ReadingChanged|IGyroscope.ReadingChanged]] event. The data provided by this event is measured in rad/s (radian per second). Use the `IGyroscope.Stop%2A` method to stop monitoring the sensor. You can detect the monitoring state of the gyroscope with the `IGyroscope.IsMonitoring%2A` property, which will be `true` if the gyroscope was started and is currently being monitored.
...
The [[IMagnetometer|IMagnetometer]] interface provides access to the sensor, and is available through the [[Magnetometer.Default|Magnetometer.Default]] property. Both the `IMagnetometer` interface and `Magnetometer` class are contained in the `Microsoft.Maui.Devices.Sensors` namespace.
...
To start monitoring the magnetometer sensor, call the `IMagnetometer.Start%2A` method. .NET MAUI sends magnetometer data changes to your app by raising the [[IMagnetometer.ReadingChanged|IMagnetometer.ReadingChanged]] event. The data provided by this event is measured in $µT$ (microteslas). Use the `IMagnetometer.Stop%2A` method to stop monitoring the sensor. You can detect the monitoring state of the magnetometer with the `IMagnetometer.IsMonitoring%2A` property, which will be `true` if the magnetometer was started and is currently being monitored.
...
> [!NOTE]
> This sensor isn't used for determining if the device's video display is in portrait or landscape mode. Use the `DeviceDisplay.Current.MainDisplayInfo.Orientation` property instead. For more information, see [[display|Device display information]].

The [[IOrientationSensor|IOrientationSensor]] interface provides access to the sensor, and is available through the [[OrientationSensor.Default|OrientationSensor.Default]] property. Both the `IOrientationSensor` interface and `OrientationSensor` class are contained in the `Microsoft.Maui.Devices.Sensors` namespace.

To start monitoring the orientation sensor, call the `IOrientationSensor.Start%2A` method. .NET MAUI sends orientation data changes to your app by raising the [[IOrientationSensor.ReadingChanged|IOrientationSensor.ReadingChanged]] event. Use the `IOrientationSensor.Stop%2A` method to stop monitoring the sensor. You can detect the monitoring state of the orientation with the `IOrientationSensor.IsMonitoring%2A` property, which will be `true` if the orientation was started and is currently being monitored.
