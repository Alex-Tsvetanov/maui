...
ms_date: "10/03/2023"
source: "https://learn.microsoft.com/dotnet/maui/platform-integration/device/flashlight?view=net-maui-10.0"
---
...
This article describes how you can use the .NET Multi-platform App UI (.NET MAUI) [[IFlashlight|IFlashlight]] interface. With this interface, you can toggle the device's camera flash on and off, to emulate a flashlight.

The default implementation of the `IFlashlight` interface is available through the [[Flashlight (Devices).Default|Flashlight.Default]] property. Both the `IFlashlight` interface and `Flashlight` class are contained in the `Microsoft.Maui.Devices` namespace.
...
The flashlight can be turned on and off through the [[IFlashlight.TurnOnAsync|TurnOnAsync]] and [[IFlashlight.TurnOffAsync|TurnOffAsync]] methods. The following code example ties the flashlight's on or off state to a [[Switch (Controls)|Switch]] control:

:::code language="csharp" source="../snippets/shared_1/DeviceDetailsPage.xaml.cs" id="flashlight":::

In addition, the `IsSupportedAsync%2A` method can be invoked to check if a flashlight is available on the device, prior to calling the [[IFlashlight.TurnOnAsync|TurnOnAsync]] method.
