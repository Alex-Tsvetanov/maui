...
ms_date: "03/23/2026"
source: "https://learn.microsoft.com/dotnet/maui/platform-integration/device/geolocation?view=net-maui-10.0"
---
...
This article describes how you can use the .NET Multi-platform App UI (.NET MAUI) [[IGeolocation|IGeolocation]] interface. This interface provides APIs to retrieve the device's current geolocation coordinates.

The default implementation of the `IGeolocation` interface is available through the [[Geolocation.Default|Geolocation.Default]] property. Both the `IGeolocation` interface and `Geolocation` class are contained in the `Microsoft.Maui.Devices.Sensors` namespace.
...
If you're going to request full accuracy with the [[GeolocationRequest.RequestFullAccuracy|GeolocationRequest.RequestFullAccuracy]] property, add the following dictionary to the _Platforms/iOS/Info.plist_ and _Platforms/MacCatalyst/Info.plist_ files:
...
Depending on the device, not all location values may be available. For example, the [[Location.Altitude|Altitude]] property might be `null`, have a value of 0, or have a positive value indicating the meters above sea level. Other values that may not be present include the [[Location.Speed|Speed]] and [[Location.Course|Course]] properties.

## Get the current location

While checking for the [last known location](#get-the-last-known-location) of the device may be quicker, it can be inaccurate. Use the `GetLocationAsync%2A` method to query the device for the current location. You can configure the accuracy and timeout of the query. It's best to the method overload that uses the [[GeolocationRequest|GeolocationRequest]] and `CancellationToken` parameters, since it may take some time to get the device's location.
...
Not all location values may be available, depending on the device. For example, the [[Location.Altitude|Altitude]] property might be `null`, have a value of 0, or have a positive value indicating the meters above sea level. Other values that may not be present include [[Location.Speed|Speed]] and [[Location.Course|Course]].

> [!WARNING]
> `GetLocationAsync%2A` can return `null` in some scenarios. This indicates that the underlying platform is unable to obtain the current location.
...
To check to see if the app is currently listening for location changes, there's a [[Geolocation.IsListeningForeground|IsListeningForeground]] property you can query. You can control the accuracy of the location change detection by passing one of the enum values in [[GeolocationAccuracy|GeolocationAccuracy]] to the [[GeolocationListeningRequest|GeolocationListeningRequest]] constructor. Once you're ready to start listening for location changes you should call the `StartListeningForegroundAsync%2A` method. This method starts listening for location updates and raises the [[Geolocation.LocationChanged|LocationChanged]] event when the location changes, provided that the app is in the foreground. The [[GeolocationLocationChangedEventArgs|GeolocationLocationChangedEventArgs]] object that accompanies this event has a [[GeolocationLocationChangedEventArgs.Location|Location]] property, of type [[Location|Location]], that represents the new location that's been detected.
...
Error handling can be implemented by registering an event handler for the [[Geolocation.ListeningFailed|ListeningFailed]] event. The [[GeolocationListeningFailedEventArgs|GeolocationListeningFailedEventArgs]] object that accompanies this event has an [[GeolocationListeningFailedEventArgs.Error|Error]] property, of type [[GeolocationError|GeolocationError]], that indicates why listening failed. When the [[Geolocation.ListeningFailed|ListeningFailed]] event is raised, listening for further location changes stops and no further [[Geolocation.LocationChanged|LocationChanged]] events are raised.

To stop listening for location changes, call the `StopListeningForeground%2A` method:
...
> [!NOTE]
> The `StopListeningForeground%2A` method has no effect when the app isn't listening for location changes.


## Check if location services are enabled

The [[Geolocation|Geolocation]] class has a read-only `IsEnabled` property that can be used to determine if location services have been enabled on the device.
...
Some devices may return a mock location from the provider or by an application that provides mock locations. You can detect this by using the [[Location.IsFromMockProvider|IsFromMockProvider]] on any [[Location|Location]]:

:::code language="csharp" source="../snippets/shared_1/SensorsPage.xaml.cs" id="geolocation_ismock":::

## Distance between two locations

The `CalculateDistance%2A` method calculates the distance between two geographic locations. This calculated distance doesn't take roads or other pathways into account, and is merely the shortest distance between the two points along the surface of the Earth. This calculation is known as the _great-circle distance_ calculation.
...
The `Double)` constructor accepts the latitude and longitude arguments, respectively. Positive latitude values are north of the equator, and positive longitude values are east of the Prime Meridian. Use the final argument to `CalculateDistance` to specify miles or kilometers. The [[UnitConverters|UnitConverters]] class also defines `KilometersToMiles%2A` and `MilesToKilometers%2A` methods for converting between the two units.
...
> [!NOTE]
> Some Android devices without a barometric sensor return `0.0` for [[Location.Altitude|Altitude]] when no altitude data is available. This is indistinguishable from actual sea-level elevation. Treat `0.0` as an unknown value rather than sea level, and validate `location.Altitude != 0` before using altitude data in your app.

The [[Location.ReducedAccuracy|Location.ReducedAccuracy]] property is only used by iOS and returns `false` on all other platforms.
