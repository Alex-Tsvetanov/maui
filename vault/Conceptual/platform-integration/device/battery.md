...
ms_date: "02/02/2023"
source: "https://learn.microsoft.com/dotnet/maui/platform-integration/device/battery?view=net-maui-10.0"
---
...
This article describes how you can use the .NET Multi-platform App UI (.NET MAUI) [[IBattery|IBattery]] interface to check the device's battery information and monitor for changes. This interface also provides information about the device's energy-saver status, which indicates if the device is running in a low-power mode.

The default implementation of the `IBattery` interface is available through the [[Battery (Devices).Default|Battery.Default]] property. Both the `IBattery` interface and `Battery` class are contained in the `Microsoft.Maui.Devices` namespace.
...
The battery status can be checked by accessing the `Battery.Default` property, which is the default implementation of the `IBattery` interface. This interface defines various properties to provide information about the state of the battery, such as [[IBattery.ChargeLevel|ChargeLevel]] to read how much battery is left. The `ChargeLevel` property returns a value between **0.0** and **1.0**, indicating the battery's charge level from empty to full, respectively.

The [[IBattery.BatteryInfoChanged|BatteryInfoChanged]] event is also available, and is raised when the state of the battery changes. The following example demonstrates how to use the monitor the `BatteryInfoChanged` event and report the battery status to [[Label (Controls)|Label]] controls:
...
The energy-saver status of the device can be read by accessing the [[IBattery.EnergySaverStatus|EnergySaverStatus]] property, which is either [[EnergySaverStatus.On|On]], [[EnergySaverStatus.Off|Off]], or [[EnergySaverStatus.Unknown|Unknown]]. If the status is `On`, the application should avoid background processing or other activities that may consume a lot of power.

The battery raises the [[IBattery.EnergySaverStatusChanged|EnergySaverStatusChanged]] event when the battery enters or leaves energy-saver mode.
...
The [[IBattery.PowerSource|PowerSource]] property returns a [[BatteryPowerSource|BatteryPowerSource]] enumeration that indicates how the device is being charged, if at all. If it's not being charged, the status is [[BatteryPowerSource.Battery|BatteryPowerSource.Battery]]. The [[BatteryPowerSource.AC|BatteryPowerSource.AC]], [[BatteryPowerSource.Usb|BatteryPowerSource.Usb]], and [[BatteryPowerSource.Wireless|BatteryPowerSource.Wireless]] values indicate that the battery is being charged.

The following code example sets the text of a [[Label (Controls)|Label]] control based on power source.
...
- Only returns [[BatteryPowerSource.AC|BatteryPowerSource.AC]] or [[BatteryPowerSource.Battery|BatteryPowerSource.Battery]] for [[IBattery.PowerSource|PowerSource]].
