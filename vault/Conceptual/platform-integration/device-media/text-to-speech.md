...
ms_date: "03/07/2025"
source: "https://learn.microsoft.com/dotnet/maui/platform-integration/device-media/text-to-speech?view=net-maui-10.0"
---
...
This article describes how you can use the .NET Multi-platform App UI (.NET MAUI) [[ITextToSpeech|ITextToSpeech]] interface. This interface enables an application to utilize the built-in text-to-speech engines to speak back text from the device. You can also use it to query for available languages.

The default implementation of the `ITextToSpeech` interface is available through the [[TextToSpeech.Default|TextToSpeech.Default]] property. Both the `ITextToSpeech` interface and `TextToSpeech` class are contained in the `Microsoft.Maui.Media` namespace.
...
Text-to-speech works by calling the `SpeakAsync%2A` method with the text to speak, as the following code example demonstrates:
...
To control the volume, pitch, and locale of the voice, use the [[SpeechOptions|SpeechOptions]] class. Pass an instance of that class to the `CancellationToken)` method. The [[ITextToSpeech.GetLocalesAsync|GetLocalesAsync]] method retrieves a collection of the locales provided by the operating system.
...
To control the volume, pitch, rate, and locale of the voice, use the [[SpeechOptions|SpeechOptions]] class. Pass an instance of that class to the `CancellationToken)` method. The [[ITextToSpeech.GetLocalesAsync|GetLocalesAsync]] method retrieves a collection of the locales provided by the operating system.
