...
ms_date: "04/02/2025"
source: "https://learn.microsoft.com/dotnet/maui/platform-integration/communication/authentication?view=net-maui-10.0"
---
...
This article describes how you can use the .NET Multi-platform App UI (.NET MAUI) the [[IWebAuthenticator|IWebAuthenticator]] interface. This interface lets you start browser-based authentication flows, which listen for a callback to a specific URL registered to the app.

The default implementation of the `IWebAuthenticator` interface is available through the [[WebAuthenticator.Default|WebAuthenticator.Default]] property. Both the `IWebAuthenticator` interface and `WebAuthenticator` class are contained in the `Microsoft.Maui.Authentication` namespace.
...
If you're interested in using your own web service for authentication, it's possible to use [[WebAuthenticator|WebAuthenticator]] to implement the client-side functionality.
...
To access the [[WebAuthenticator|WebAuthenticator]] functionality the following platform-specific setup is required.
...
> [!CAUTION]
> At the moment, [[WebAuthenticator|WebAuthenticator]] isn't working on Windows. For more information, see [GitHub issue #2702](https://github.com/dotnet/maui/issues/2702).
...
The API consists mainly of a single method, `AuthenticateAsync%2A`, which takes two parameters:
...
> [!NOTE]
> There's also an overload of the `AuthenticateAsync%2A` method that enables an authentication flow to be cancelled programatically with a `CancellationToken` argument.


The result is a `[[WebAuthenticatorResult|WebAuthenticatorResult]], which includes any query parameters parsed from the callback URI:
...
The [[WebAuthenticator|WebAuthenticator]] API takes care of launching the url in the browser and waiting until the callback is received:

![](media/authentication/web-authenticator.png)

If the user cancels the flow at any point, a `TaskCanceledException` is thrown.

### Private authentication session

iOS 13 introduced an ephemeral web browser API for developers to launch the authentication session as private. This enables developers to request that no shared cookies or browsing data is available between authentication sessions and will be a fresh login session each time. This is available through the [[WebAuthenticatorOptions|WebAuthenticatorOptions]] parameter passed to the `AuthenticateAsync%2A` method:
...
For iOS 13 and higher, call the `AppleSignInAuthenticator.AuthenticateAsync%2A` method. This uses the native Apple Sign in APIs so your users get the best experience possible on these devices. For example, you can write your shared code to use the correct API at runtime:
...
It's possible to use the [[WebAuthenticator|WebAuthenticator]] API with any web back-end service. To use it with an ASP.NET core app, configure the web app with the following steps:
