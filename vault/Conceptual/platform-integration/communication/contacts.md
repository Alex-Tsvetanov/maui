...
ms_date: "02/02/2023"
source: "https://learn.microsoft.com/dotnet/maui/platform-integration/communication/contacts?view=net-maui-10.0"
---
...
This article describes how you can use the .NET Multi-platform App UI (.NET MAUI) [[IContacts|IContacts]] interface to select a contact and read information about it.

The default implementation of the `IContacts` interface is available through the [[Contacts.Default|Default]] property. Both the `IContacts` interface and `Contacts` class are contained in the `Microsoft.Maui.ApplicationModel.Communication` namespace.
...
You can request the user to pick a contact by calling the [[IContacts.PickContactAsync|PickContactAsync]] method. A contact dialog will appear on the device allowing the user to select a contact. If the user doesn't select a contact, `null` is returned.

:::code language="csharp" source="../snippets/shared_1/CommsPage.xaml.cs" id="contact_select":::

## Get all contacts

The `GetAllAsync%2A` method returns a collection of contacts.
...
- The `cancellationToken` parameter in the `GetAllAsync%2A` method isn't supported.
...
- The `cancellationToken` parameter in the `GetAllAsync%2A` method isn't supported.
- The iOS platform doesn't support the [[Contact.DisplayName|DisplayName]] property natively, thus, the `DisplayName` value is constructed as "[[Contact.GivenName|GivenName]] [[Contact.FamilyName|FamilyName]]".
