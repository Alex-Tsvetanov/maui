...
ms_date: "12/16/2024"
source: "https://learn.microsoft.com/dotnet/maui/platform-integration/storage/file-picker?view=net-maui-10.0"
---
...
Enable iCloud capabilities. For more information, see [[capabilities|Capabilities]].

Mac Catalyst apps that are released to the Mac App Store require Apple's App Sandbox to be enabled. The App Sandbox restricts access to system resources and user data in Mac apps, to contain damage if an app becomes compromised. For more information about enabling the App Sandbox, see [[publish-app-store#add-entitlements|Add entitlements]].
...
For more information about adding entitlements to Mac Catalyst apps, see [[entitlements|Mac Catalyst entitlements]]. For more information about managed file access, see [Enable managed file access](https://developer.apple.com/documentation/xcode/configuring-the-macos-app-sandbox/#Enable-managed-file-access) on developer.apple.com.
...
> [!WARNING]
> On Android, `FileResult.FullPath` may return a `content://` URI rather than a filesystem path. Passing a `content://` URI directly to `File.OpenRead()` or similar APIs will fail with a `FileNotFoundException`. Always use `OpenReadAsync%2A` to read the file contents:
>
> ```csharp
