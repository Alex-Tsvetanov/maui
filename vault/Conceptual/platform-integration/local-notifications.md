...
ms_date: "06/18/2024"
source: "https://learn.microsoft.com/dotnet/maui/platform-integration/local-notifications?view=net-maui-10.0"
---
...
The `NotificationManagerService` class should be placed in your app's *Platforms > Android* folder. Alternatively, multi-targeting can be performed based on your own filename and folder criteria, rather than using the *Platforms > Android* folder. For more information, see [[configure-multi-targeting|Configure multi-targeting]].
...
For more information about .NET MAUI permissions, see [[permissions|Permissions]].
...
The `NotificationManagerService` class should be placed in your app's *Platforms > iOS*  or *Platforms > Mac Catalyst* folder. Alternatively, multi-targeting can be performed based on your own filename and folder criteria, rather than using the *Platforms* folders. For more information, see [[configure-multi-targeting|Configure multi-targeting]].
...
Each `NotificationManagerService` implementation should be registered against the `INotificationManagerService` interface, so that the operations the interface exposes can be called from cross-platform code. This can be achieved by registering the types with the [[MauiAppBuilder.Services|Services]] property of the [[MauiAppBuilder|MauiAppBuilder]] object in the `CreateMauiApp` method in the `MauiProgram` class:
...
For more information about dependency injection in .NET MAUI, see [[dependency-injection|Dependency injection]].
...
For more information about resolving registered types, see [[dependency-injection#resolution|Resolution]].
...
![](media/local-notifications/android-notification-area.png)

When you swipe down on the status bar the notification drawer opens:

![](media/local-notifications/android-notification-drawer.png)
...
![](media/local-notifications/ios-notification.png)



On Mac Catalyst, incoming notifications are automatically received by Notification Centre:

![](media/local-notifications/mac-notification.png)
