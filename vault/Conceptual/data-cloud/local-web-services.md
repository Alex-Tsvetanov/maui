...
tags:
  ...
  - area/data-cloud
ms_date: "10/07/2024"
source: "https://learn.microsoft.com/dotnet/maui/data-cloud/local-web-services?view=net-maui-10.0"
---
...
> [!TIP]
> If you're using .NET 10 or later, consider using [[aspire-integration|Aspire integration]] to simplify connecting to local web services. Aspire automatically handles platform-specific networking configuration, service discovery, and development tunnels, eliminating much of the manual configuration described in this article.
...
> [!NOTE]
> When running a .NET MAUI app in the iOS simulator from Windows, the app is displayed in the [[remote-simulator|remote iOS simulator for Windows]]. However, the app is running on the paired Mac. Therefore, there's no localhost access to a web service running in Windows for an iOS app running on a Mac.
...
In the code that defines the URL of your local web service in your .NET MAUI app, ensure that the web service URL specifies the HTTP scheme, and the correct hostname. The [[DeviceInfo|DeviceInfo]] class can be used to detect the platform the app is running on. The correct hostname can then be set as follows:
...
For more information about the [[DeviceInfo|DeviceInfo]] class, see [[information|Device information]].
...
In the code that defines the URL of your local web service in your .NET MAUI app, ensure that the web service URL specifies the HTTPS scheme, and the correct hostname. The [[DeviceInfo|DeviceInfo]] class can be used to detect the platform the app is running on. The correct hostname can then be set as follows:
