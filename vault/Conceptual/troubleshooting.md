...
tags:
  ...
  - area/general
ms_date: "01/02/2025"
source: "https://learn.microsoft.com/dotnet/maui/troubleshooting?view=net-maui-10.0"
---
...
01. Run the `dotnet --version` command.
    ...
    If the result starts with `7.0` and includes the value `-rc`, you're running in the context of a .NET 7 release candidate, which fully supports .NET MAUI, and includes .NET MAUI templates. You may just need to install the .NET MAUI workload. Review the [[installation|Installation instructions]] and ensure that the **.NET Multi-platform App UI development** workload is enabled.
...
When using the [[contacts|`Contacts` API]], you may see the following error related to iOS and macOS:
...
2. Either use `xcode-select --switch ...` to select the system's version of Xcode, or set the `DEVELOPER_DIR` environment variable to the path of Xcode. For more information, see [[cli|Build with a specific version of Xcode]].

## Diagnose issues in Blazor Hybrid apps

[[BlazorWebView (Maui)|BlazorWebView]] has built-in logging that can help you diagnose problems in your Blazor Hybrid app. There are two steps to enable this logging:
...
For more information, see [[blazorwebview#diagnosing-issues|Diagnosing issues in Blazor Hybrid apps]].
