...
tags:
  ...
  - area/android
ms_date: "08/27/2024"
source: "https://learn.microsoft.com/dotnet/maui/android/emulator/device-manager?view=net-maui-10.0"
---
...
> [!IMPORTANT]
> Enable hardware acceleration for the Android devices. For more information, see [[hardware-acceleration|Hardware Acceleration for Emulator Performance]].
...
You use the Android Device Manager to create and configure an Android Virtual Devices (AVD) that run in the [[debug-on-emulator|Android Emulator]]. Each AVD is an emulator configuration that simulates a physical Android device. This makes it possible to run and test your app in a variety of configurations that simulate different physical Android devices.

![](media/device-manager/win/01-devices-dialog-sml.png)
...
- The following packages must be installed:
  ...
  ![](media/device-manager/win/02-sdk-tools.png)

When you install the **.NET Multi-Platform App UI development** workload in Visual Studio, everything is installed for you. For more information on setting up .NET MAUI with Visual Studio, see [[first-app|Build your first app]].
...
![](media/device-manager/win/03-tools-menu.png)
...
![](media/device-manager/win/08-new-button.png)
...
01. Give the device a new name. In the following example, the new device is named _Pixel 3a - API 31_.
    ...
    ![](media/device-manager/win/09-device-props-sml.png)
...
01. Select a processor type for this virtual device with the **Processor** box.
    ...
    It's recommended that you choose **x86_64** and enable [[hardware-acceleration|hardware acceleration]].
...
01. Use the property list to change some of the most commonly modified properties. To make changes to properties, see [[device-properties|Editing Android Virtual Device Properties]].

01. Add any additional properties that you need to explicitly set with the **Add Property** box at the bottom of the window:

    ![](media/device-manager/win/16-add-property-menu.png)

    You can also define a custom property by selecting **Custom...**.

01. Press the **Create** button to create the new device:

    ![](media/device-manager/win/17-create-button.png)
...
01. The Android Device Manager adds the new device to the list of installed virtual devices while displaying a **Creating** progress indicator during device creation:
    ...
    ![](media/device-manager/win/19-creating-the-device.png)
...
![](media/device-manager/win/21-edit-button.png)
...
![](media/device-manager/win/23-device-editing-sml.png)

After you've made the necessary configuration changes, press the **Save** button. For more information about changing virtual device properties, see [[device-properties|Editing Android Virtual Device Properties]].

### Additional options

Additional options for working with devices are available from the **Additional Options** (&hellip;) pull-down menu:

![](media/device-manager/win/24-overflow-menu.png)
...
- **Factory Reset** &ndash; Resets the selected device to its default settings, erasing any user changes made to the internal state of the device while it was running. This action also erases the current [[debug-on-emulator#fast-boot|Fast Boot]] snapshot if it exists. This change doesn't alter modifications that you make to the virtual device during creation and editing. A dialog box will appear with the reminder that this reset cannot be undone &ndash; press **Factory Reset** to confirm the reset.
...
![](media/device-manager/win/29-sdk-error.png)
...
01. Open one of the log files and search for an entry describing **Config file path**. For example:
    ...
    ![](media/device-manager/win/31-config-file-path.png)
...
![](media/device-manager/win/32-sdk-instance-error.png)
...
01. Delete the _snapshot.pb_ file:
    ...
    ![](media/device-manager/win/33-delete-snapshot.png)
