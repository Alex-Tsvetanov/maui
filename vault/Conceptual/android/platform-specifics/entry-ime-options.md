...
tags:
  ...
  - area/android
ms_date: "04/05/2022"
source: "https://learn.microsoft.com/dotnet/maui/android/platform-specifics/entry-ime-options?view=net-maui-10.0"
---

# Entry input method editor options on Android

This .NET Multi-platform App UI (.NET MAUI) Android platform-specific sets the input method editor (IME) options for the soft keyboard for an [[Entry (Controls)|Entry]]. This includes setting the user action button in the bottom corner of the soft keyboard, and the interactions with the [[Entry (Controls)|Entry]]. It's consumed in XAML by setting the `Entry.ImeOptions` attached property to a value of the `ImeFlags` enumeration:
...
The result is that a specified `ImeFlags` value is applied to the soft keyboard for the [[Entry (Controls)|Entry]], which sets the input method editor options:

![](media/entry-ime-options/entry-imeoptions.png)
