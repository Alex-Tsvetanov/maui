...
tags:
  ...
  - area/macios
ms_date: "02/20/2024"
source: "https://learn.microsoft.com/dotnet/maui/macios/universal-links?view=net-maui-10.0"
---
...
The associated domains entitlement specifies a list of domains that the app is associated with. This entitlement should be added to the *Entitlements.plist* file in your app. For more information about adding an entitlement on iOS, see [[entitlements|Entitlements]]. For more information about adding an entitlement on Mac Catalyst, see [[entitlements|Entitlements]].
...
1. On the **Edit your App ID Configuration** page, enable the **Associated Domains** capability and then select the **Save** button:
    ...
    ![](media/universal-links/associated-domains-capability.png)
...
When a user activates a universal link, iOS and Mac Catalyst launch your app and send it an `NSUserActivity` object. This object can be queried to determine how your app launched, and to determine what action to take. This should be performed in the `FinishedLaunching` and `ContinueUserActivity` lifecycle delegates. The `FinishedLaunching` delegate is invoked when the app has launched, and the `ContinueUserActivity` delegate is invoked when the app is running or suspended. For more information about lifecycle delegates, see [[app-lifecycle#platform-lifecycle-events|Platform lifecycle events]].

To respond to an iOS lifecycle delegate being invoked, call the `ConfigureLifecycleEvents%2A` method on the [[MauiAppBuilder|MauiAppBuilder]] object in the `CreateMauiapp` method of your `MauiProgram` class. Then, on the [[ILifecycleBuilder|ILifecycleBuilder]] object, call the `AddiOS` method and specify the `Action` that registers a handler for the required delegate:
...
When iOS opens your app as a result of a universal link, the `NSUserActivity` object will have an `ActivityType` property with a value of `BrowsingWeb`. The activity object's `WebPageUrl` property will contain the URL that the user wants to access. The URL can be passed to your `App` class with the `SendOnAppLinkRequestReceived%2A` method.
...
In your `App` class, override the `OnAppLinkRequestReceived%2A` method to receive and process the URL:
...
In the example above, the `OnAppLinkRequestReceived%2A` override displays the app link URL. In practice, the app link should take users directly to the content represented by the URL, without any prompts, logins, or other interruptions. Therefore, the `OnAppLinkRequestReceived%2A` override is the location from which to invoke navigation to the content represented by the URL.
