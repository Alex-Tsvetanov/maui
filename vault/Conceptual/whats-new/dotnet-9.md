...
tags:
  ...
  - area/whats-new
ms_date: "01/31/2025"
source: "https://learn.microsoft.com/dotnet/maui/whats-new/dotnet-9?view=net-maui-10.0"
---
...
.NET MAUI 9 requires minimum deployment targets of iOS 12.2, and Mac Catalyst 15.0 (macOS 12.0). Android and Windows minimum deployment targets remain the same. For more information, see [[supported-platforms|Supported platforms for .NET MAUI apps]].
...
For more information, see [[hybridwebview|HybridWebView]].

### Titlebar for Mac Catalyst and Windows

The [[TitleBar|TitleBar]] control provides the ability to add a custom title bar to your app on Mac Catalyst and Windows:

![](media/dotnet-9/titlebar-overview.png)

A [[TitleBar|TitleBar]] can be set as the value of the [[Window.TitleBar|Window.TitleBar]] property on any [[TitleBar|TitleBar]]:
...
![](media/dotnet-9/titlebar-full.png)

For more information, see [[titlebar|TitleBar]].
...
The binding mode for `IsVisible` and `IsEnabled` on a [[BackButtonBehavior|BackButtonBehavior]] in a Shell app is now `BindingMode.OneWay` instead of `BindingMode.OneTime`. This enables you to more easily control the behavior of the back button at runtime, with data bindings:
...
The default behavior for hosting content in a [[BlazorWebView (Maui)|BlazorWebView]] has changed to `0.0.0.1`. The internal `0.0.0.0` address used to host content no longer works and results in the [[BlazorWebView (Maui)|BlazorWebView]] not loading any content and rendering as an empty rectangle.
...
By default, [[BlazorWebView (Maui)|BlazorWebView]] now fires and forgets the async disposal of the underlying `WebViewManager`. This reduces the potential for disposal deadlocks to occur on Android.

> [!WARNING]
> This fire-and-forget default behavior means that disposal can return before all objects are disposed, which can cause behavioral changes in your app. The items that are disposed are partially Blazor's own internal types, but also app-defined types such as scoped services used within the [[BlazorWebView (Maui)|BlazorWebView]] portion of your app.

To opt out of this behavior, you should configure your app to block on dispose via an `AppContext` switch in your `MauiProgram` class:
...
If your app is configured to block on dispose via this switch, [[BlazorWebView (Maui)|BlazorWebView]] performs async-over-sync disposal, which means that it blocks the thread until the async disposal is complete. However, this can cause deadlocks if the disposal needs to run code on the same thread (because the thread is blocked while waiting).

### Buttons on iOS

[[Button (Controls)|Button]] controls on iOS now respect spacing, padding, border width, and margins more accurately than in previous releases. A large image in a [[Button (Controls)|Button]] will now be resized to the maximum size, taking into account the spacing, padding, border width, and margins. However, if a [[Button (Controls)|Button]] contains text and an image it might not be possible to fit all the content inside the button, and so you should size your image manually to achieve your desired layout.
...
In .NET MAUI 9, the [[ContentPage.HideSoftInputOnTapped|HideSoftInputOnTapped]] property is also supported on Mac Catalyst, as well and Android and iOS.

### Focus behavior on Windows

In .NET MAUI 8 on Windows, the [[VisualElement (Controls).Focused|VisualElement.Focused]] event, the [[VisualElement (Controls).Unfocused|VisualElement.Unfocused]] event, and the [[VisualElement (Controls).IsFocused|VisualElement.IsFocused]] property are applied to an element and its children:
...
In this example on .NET 8, the `OnFocused` event handler is executed on Windows when the [[VerticalStackLayout|VerticalStackLayout]], [[Entry (Controls)|Entry]], or [[Editor|Editor]] gains focus.

.NET MAUI 9 changes this behavior on Windows to be identical to the other platforms. Therefore, the [[VisualElement (Controls).Focused|VisualElement.Focused]] event, the [[VisualElement (Controls).Unfocused|VisualElement.Unfocused]] event, and the [[VisualElement (Controls).IsFocused|VisualElement.IsFocused]] property only apply to an element. Therefore, when the previous example runs on .NET 9, the `OnFocused` event handler isn't executed because only input controls can gain focus.

### Soft keyboard input support

.NET MAUI 9 adds new soft keyboard input support for `Password`, `Date`, and `Time`. These can be enabled on [[Editor|Editor]] and [[Entry (Controls)|Entry]] controls:
...
The [[TextAlignment|TextAlignment]] enumeration adds a `Justify` member that can be used to align text in text controls. For example, you can horizontally align text in a [[Label (Controls)|Label]] with `HorizontalTextAlignment.Justify`:
...
[[TimePicker (Controls)|TimePicker]] gains a [[TimePicker (Controls).TimeSelected|TimeSelected]] event, which is raised when the selected time changes. The [[TimeChangedEventArgs|TimeChangedEventArgs]] object that accompanies the `TimeSelected` event has `NewTime` and `OldTime` properties, which specify the new and old time, respectively.

### WebView

[[WebView (Controls)|WebView]] adds a `ProcessTerminated` event that's raised when a [[WebView (Controls)|WebView]] process ends unexpectedly. The `WebViewProcessTerminatedEventArgs` object that accompanies this event defines platform-specific properties that indicate why the process failed.

## Compiled bindings in code

Bindings written in code typically use string paths that are resolved at runtime with reflection, and the overhead of doing this varies from platform to platform. .NET MAUI 9 introduces an additional `SetBinding%2A` extension method that defines bindings using a `Func` argument instead of a  string path:
...
In addition, .NET MAUI 9 adds a `BindingBase.Create%2A` method that sets the binding directly on the object with a `Func`, and returns the binding object instance:
...
In a Shell app, you no longer need to register your pages with the dependency injection container unless you want to influence the lifetime of the page relative to the container with the `AddSingleton`, `AddTransient`, or `AddScoped` methods. For more information about these methods, see [[dependency-injection#dependency-lifetime|Dependency lifetime]].

## Handler disconnection

When implementing a custom control using handlers, every platform handler implementation is required to implement the [[IElementHandler.DisconnectHandler|DisconnectHandler]] method, to perform any native view cleanup such as unsubscribing from events. However, prior to .NET MAUI 9, the [[IElementHandler.DisconnectHandler|DisconnectHandler]] implementation is intentionally not invoked by .NET MAUI. Instead, you'd have to invoke it yourself when choosing to cleanup a control, such as when navigating backwards in an app.

In .NET MAUI 9, handlers automatically disconnect from their controls when possible, such as when navigating backwards in an app. In some scenarios you might not want this behavior. Therefore, .NET MAUI 9 adds a [[HandlerProperties.DisconnectPolicyProperty|`HandlerProperties.DisconnectPolicy`]] attached property for controlling when handlers are disconnected from their controls. This property requires a [[HandlerDisconnectPolicy|HandlerDisconnectPolicy]] argument, with the enumeration defining the following values:

- `Automatic`, which indicates that handlers will be disconnected automatically. This is the default value of the [[HandlerProperties.DisconnectPolicyProperty|`HandlerProperties.DisconnectPolicy`]] attached property.
- `Manual`, which indicates that handlers will have to be disconnected manually by invoking the [[IElementHandler.DisconnectHandler|DisconnectHandler]] implementation.
...
For more information, see [[nativeaot|Native AOT deployment on iOS and Mac Catalyst]].
...
![](media/dotnet-9/syncfusion-sample-pages.png)
...
In .NET MAUI 9, a stand-alone XAML [[ResourceDictionary|ResourceDictionary]] (which isn't backed by a code-behind file) defaults to having its XAML compiled. To opt out of this behavior, specify `<?xaml-comp compile="false" ?>` after the XML header.

## Trimming

Full trimming is now supported by setting the `$(TrimMode)` MSBuild property to `full`. For more information, see [[trimming|Trim a .NET MAUI app]].
...
When debugging and deploying a new .NET MAUI project to Windows, the default behavior in .NET MAUI 9 is to deploy an unpackaged app. For more information, see [[setup|Deploy and debug your .NET MAUI app on Windows]].
...
All classes that implement [[IMarkupExtension{T}|IMarkupExtension]], `IMarkupExtension`1`, [[IValueProvider|IValueProvider]], and [[IExtendedTypeConverter|IExtendedTypeConverter]] need to be annotated with either the [[RequireServiceAttribute|RequireServiceAttribute]] or [[AcceptEmptyServiceProviderAttribute|AcceptEmptyServiceProviderAttribute]]. This is required due to a XAML compiler optimization introduced in .NET MAUI 9 that enables the generation of more efficient code, which helps reduce the app size and improve runtime performance.
...
For more information, see [[xcsync|Xcode sync]].
...
The [[Frame|Frame]] control is marked as obsolete in .NET MAUI 9, and will be completely removed in a future release. The [[Border|Border]] control should be used in its place.

When replacing a [[Frame|Frame]] with a [[Border|Border]], the [[Frame.BorderColor|Frame.BorderColor]] property value should become the [[Border.Stroke|Border.Stroke]] property value, and the [[Frame.CornerRadius|Frame.CornerRadius]] property value should become part of the [[Border.StrokeShape|Border.StrokeShape]] property value. In addition, it may be necessary to duplicate the `Margin` value as the `Padding` value.

The following example shows equivalent [[Frame|Frame]] and [[Border|Border]] elements in XAML:
...
For more information see [[border|Border]].

### MainPage

Instead of defining the first page of your app using the [[Application (Controls).MainPage|MainPage]] property on an [[Application (Controls)|Application]] object, you should set the [[Window.Page|Page]] property on a [[Window|Window]] to the first page of your app. This is what happens internally in .NET MAUI when you set the [[Application (Controls).MainPage|MainPage]] property, so there's no behavior change introduced by the [[Application (Controls).MainPage|MainPage]] property being marked as obsolete.

The following example shows setting the [[Window.Page|Page]] property on a [[Window|Window]], via the `CreateWindow` override:
...
Code that accesses the `Application.Current.MainPage` property should now access the `Application.Current.Windows[0].Page` property for apps with a single window. For apps with multiple windows, use the `Application.Current.Windows` collection to identify the correct window and then access the `Page` property. In addition, each element features a `Window` property, that's accessible when the element is part of the current window, from which the `Page` property can be accessed (`Window.Page`). Platform code can retrieve the app's [[IWindow|IWindow]] object with the `Microsoft.Maui.Platform.GetWindow` extension method.

While the [[Application (Controls).MainPage|MainPage]] property is retained in .NET MAUI 9 it will be completely removed in a future release.

### Compatibility layouts

The compatibility layout classes in the `Compatibility` namespace have been obsoleted.

### Legacy measure calls

The following [[VisualElement (Controls)|VisualElement]] measure methods have been obsoleted:

- `VisualElement.OnMeasure%2A`
- `Controls.MeasureFlags)`

These are legacy measure methods that don't function correctly with .NET MAUI layout expectations.

As a replacement, the `Double,System.Double)` method has been introduced. This method returns the minimum size that an element needs in order to be displayed on a device. Margins are excluded from the measurement, but are returned with the size. This is the preferred method to call when measuring a view.

In addition, the [[SizeRequest|SizeRequest]] struct is obsoleted. Instead, [[Size|Size]] should be used.
...
When debugging and deploying a new .NET MAUI project to Windows, the default behavior in .NET 9 is to deploy an unpackaged app. To adopt this behavior, see [[setup#convert-a-packaged-net-maui-windows-app-to-unpackaged|Convert a packaged .NET MAUI Windows app to unpackaged]].
...
XAML markup extensions will need to be annotated with either the [[RequireServiceAttribute|RequireServiceAttribute]] or [[AcceptEmptyServiceProviderAttribute|AcceptEmptyServiceProviderAttribute]]. This is required due to a XAML compiler optimization that enables the generation of more efficient code, which helps reduce the app size and improve runtime performance. For more information, see [Service providers](~/xaml/markup-extensions/create.md?view=net-maui-9&preserve-view=true#service-providers).
...
You can opt into compiling binding expressions that are declared in code, to take advantage of better runtime performance. For more information, see [[compiled-bindings#compiled-bindings-in-code|Compiled bindings in code]].

### Adopt full trimming

You can adopt into using full trimming, to reduce the overall size of your app, by setting the `$(TrimMode)` MSBuild property to `full`. For more information, see [[trimming|Trim a .NET MAUI app]].

### Adopt NativeAOT deployment on supported platforms

You can opt into Native AOT deployment on iOS and Mac Catalyst. Native AOT deployment produces a .NET MAUI app that's been ahead-of-time (AOT) compiled to native code. For more information, see [[nativeaot|Native AOT deployment on iOS and Mac Catalyst]].
...
For more information about Android asset packs, see [[asset-packs|Android asset packs]].
