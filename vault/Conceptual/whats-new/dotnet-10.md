...
tags:
  ...
  - area/whats-new
ms_date: "11/11/2025"
source: "https://learn.microsoft.com/dotnet/maui/whats-new/dotnet-10?view=net-maui-10.0"
---
...
.NET MAUI in .NET 9 included two optional handlers on iOS and Mac Catalyst that brought performance and stability improvements to [[CollectionView|CollectionView]] and [[CarouselView|CarouselView]]. In .NET 10, these are the default handlers for [[CollectionView|CollectionView]] and [[CarouselView|CarouselView]].
...
On Android, the [[Editor|Editor]] and [[Entry (Controls)|Entry]] views change their native views from `AppCompatEditText` to `MauiAppCompatEditText`, which adds support for the `SelectionChanged` event.

### HybridWebView

[[HybridWebView|HybridWebView]] gains an `InvokeJavaScriptAsync%2A` overload that invokes a specified JavaScript method without specifying any information about the return type. For more information, see [Invoke JavaScript methods that don't return a value](~/user-interface/controls/hybridwebview.md?view=net-maui-10.0&preserve-view=true#invoke-javascript-methods-that-dont-return-a-value).
...
> [!NOTE]
> [[Cell (Controls)|Cell]] hasn't been deprecated because it's currently used for source generation. However, it should be considered deprecated.

### Page

The `IsBusy` property is marked obsolete. In .NET 10 we recommend using [[ActivityIndicator|ActivityIndicator]] (or a similar UX) for page or view-specific busy states and avoiding long-lived global busy indicators. For guidance, see [ContentPage](~/user-interface/pages/contentpage.md?view=net-maui-10.0&preserve-view=true).

### Picker

Programmatically control picker state with new Open/Close API.

In addition, [[DatePicker (Controls)|DatePicker]] and [[TimePicker (Controls)|TimePicker]] support nullable selection in .NET 10. This enables clearing values and binding to nullable types:

- [[DatePicker (Controls).Date|Date]] is now `DateTime`; [[DatePicker (Controls).MinimumDate|MinimumDate]] and [[DatePicker (Controls).MaximumDate|MaximumDate]] are also nullable.
- [[TimePicker (Controls).Time|Time]] is now `TimeSpan`.
...
[[SearchBar (Controls)|SearchBar]] gains a `SearchIconColor` bindable property that sets the color of the search icon:
...
[[SearchBar (Controls)|SearchBar]] also gains a `ReturnType` bindable property, of type [[ReturnType|ReturnType]], that specifies the appearance of the return button. The default value of this property is `Search`.

For more information, see [SearchBar](~/user-interface/controls/searchbar.md?view=net-maui-10.0&preserve-view=true).

### Switch

[[Switch (Controls)|Switch]] gains an `OffColor` bindable property that sets the color of the switch when it's in the off state:
...
When videos are hosted in a [[WebView (Controls)|WebView]] on Android, they can now be played fullscreen by including `allowfullscreen` in the `iframe`.
...
[[SearchHandler|SearchHandler]] adds APIs to programmatically show or hide the soft keyboard, making it easier to control focus and user input flow:
...
For more information, see [[page-popover|Display a modal page as a popover on iOS and Mac Catalyst]].

### Geolocation

The [[Geolocation|Geolocation]] class gains a read-only `IsEnabled` property that can be used to determine if location services have been enabled on the device.
...
The [[SpeechOptions|SpeechOptions]] class gains a `Rate` property that controls the speech rate when using [[TextToSpeech|TextToSpeech]] functionality. For more information, see [Text-to-Speech settings](~/platform-integration/device-media/text-to-speech.md?view=net-maui-10.0&preserve-view=true#settings).

### Web authentication

The [[WebAuthenticator|WebAuthenticator]] gains a `AuthenticateAsync%2A` method overload that enables an authentication flow to be cancelled programatically with a `CancellationToken` argument.
...
For more information, see [[safe-area|Safe area layout]].
...
The [[FontImageExtension|FontImageExtension]] XAML markup extension has been deprecated. Instead, the [[FontImageSource|FontImageSource]] type should be used:
...
For more information, see [[fonts#display-font-icons|Display font icons]].
...
- For command line `dotnet build` invocations, `ZipArchive` is used to create *.aab* and *.apk* files. This should result in faster build times.
- For Visual Studio builds, [dotnet/android-libzipsharp](https://github.com/dotnet/android-libzipsharp) is still used because the .NET Framework version of `Compression` can't be used.
...
Previously, `NSUrlSessionHandler` would initialize the `NSUrlSessionConfiguration.TLSMinimumSupportedProtocol` value from the `ServicePointManager.SecurityProtocol` property, but the `ServicePointManager` class is now deprecated. Therefore, you'll have to set the `NSUrlSessionConfiguration.TLSMinimumSupportedProtocol` value before creating the `NSUrlSessionHandler`:
