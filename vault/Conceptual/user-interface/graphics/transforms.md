...
tags:
  ...
  - area/user-interface
ms_date: "09/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/graphics/transforms?view=net-maui-10.0"
---
...
.NET Multi-platform App UI (.NET MAUI) graphics supports traditional graphics transforms, which are implemented as methods on the [[ICanvas|ICanvas]] object. Mathematically, transforms alter the coordinates and sizes that you specify in [[ICanvas|ICanvas]] drawing methods, when the graphical objects are rendered.
...
The .NET MAUI [[VisualElement (Controls)|VisualElement]] class also supports the following transform properties: [[VisualElement (Controls).TranslationX|TranslationX]], [[VisualElement (Controls).TranslationY|TranslationY]], [[VisualElement (Controls).Scale|Scale]], [[VisualElement (Controls).Rotation|Rotation]], [[VisualElement (Controls).RotationX|RotationX]], and [[VisualElement (Controls).RotationY|RotationY]]. However, there are several differences between `Graphics` transforms and [[VisualElement (Controls)|VisualElement]] transforms:

- `Graphics` transforms are methods, while [[VisualElement (Controls)|VisualElement]] transforms are properties. Therefore, `Graphics` transforms perform an operation while [[VisualElement (Controls)|VisualElement]] transforms set a state. `Graphics` transforms apply to subsequently drawn graphics objects, but not to graphics objects that are drawn before the transform is applied. In contrast, a [[VisualElement (Controls)|VisualElement]] transform applies to a previously rendered element as soon as the property is set. Therefore, `Graphics` transforms are cumulative as methods are called, while [[VisualElement (Controls)|VisualElement]] transforms are replaced when the property is set with another value.
- `Graphics` transforms are applied to the [[ICanvas|ICanvas]] object, while [[VisualElement (Controls)|VisualElement]] transforms are applied to a [[VisualElement (Controls)|VisualElement]] derived object.
- `Graphics` transforms are relative to the upper-left corner of the [[ICanvas|ICanvas]], while [[VisualElement (Controls)|VisualElement]] transforms are relative to the upper-left corner of the [[VisualElement (Controls)|VisualElement]] to which they're applied.
...
The `Translate%2A` method requires `x` and `y` arguments of type `float` that cause subsequently drawn graphic objects to be shifted horizontally and vertically. Negative `x` values move an object to the left, while positive values move an object to the right. Negative `y` values move up an object, while positive values move down an object.

A common use of the translate transform is for rendering a graphical object that has been originally created using coordinates that are convenient for drawing. The following example creates a [[PathF|PathF]] object for an 11-pointed star:
...
The center of the star is at (0,0), and the points of the star are on a circle surrounding that point. Each point is a combination of sine and cosine values of an angle that increases by 5/11ths of 360 degrees. The radius of the circle is set as 100. If the [[PathF|PathF]] object is displayed without any transforms, the center of the star will be positioned at the upper-left corner of the [[ICanvas|ICanvas]], and only a quarter of it will be visible. Therefore, a translate transform is used to shift the star horizontally and vertically to (150,150):

![](media/transforms/translate.png)
...
The `Scale%2A` method requires `x` and `y` arguments of type `float` that let you specify different values for horizontal and vertical scaling, otherwise known as *anisotropic* scaling. The values of `x` and `y` have a significant impact on the resulting scaling:
...
The following example demonstrates the `Scale%2A` method:
...
In this example, ".NET MAUI" is displayed inside a rounded rectangle stroked with a dashed line. The same graphical objects drawn after the `Scale%2A` call increase in size proportionally:

![](media/transforms/scale.png)
...
Order matters when you combine `Translate%2A` and `Scale%2A` calls. If the `Translate%2A` call comes after the `Scale%2A` call, the translation factors are scaled by the scaling factors. If the `Translate%2A` call comes before the `Scale%2A` call, the translation factors aren't scaled.
...
There are two `Rotate%2A` overloads. The first requires a `degrees` argument of type `float` that defines the rotation angle, and centers the rotation around the upper-left corner of the canvas (0,0). The following example demonstrates this `Rotate%2A` method:
...
![](media/transforms/rotate.png)

Alternatively, graphical objects can be rotated centered around a specific point. To rotate around a specific point, use the `Rotate%2A` overload that accepts `degrees`, `x`, and `y` arguments of type `float`:
...
In this example, the `Translate%2A` and `Scale%2A` calls apply globally to the clock, and so are called before the `Rotate%2A` method.

There are 60 marks of two different sizes that are drawn in a circle around the clock. The `FillCircle%2A` call draws that circle at (0,-90), which relative to the center of the clock corresponds to 12:00. The `Rotate%2A` call increments the rotation angle by 6 degrees after every tick mark. The `angle` variable is used solely to determine if a large circle or a small circle is drawn. Finally, the current time is obtained and rotation degrees are calculated for the hour, minute, and second hands. Each hand is drawn in the 12:00 position so that the rotation angle is relative to that position:

![](media/transforms/clock.png)
...
An affine transformation matrix has its final column equal to (0,0,1), so only members in the first two columns need to be specified. Therefore, the 3x3 matrix is represented by the `Matrix3x2` struct, from the `Numerics` namespace, which is a collection of three rows and two columns of `float` values.
...
The only [[ICanvas|ICanvas]] method that accepts a `Matrix3x2` argument is the `ConcatenateTransform%2A` method, which combines multiple transforms into a single transform. The following example shows how to use this method to transform a [[PathF|PathF]] object:
...
In this example, the [[PathF|PathF]] object is scaled and sheared on the x-axis, and translated on the x-axis and the y-axis.
