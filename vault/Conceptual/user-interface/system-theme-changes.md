...
tags:
  ...
  - area/user-interface
ms_date: "09/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/system-theme-changes?view=net-maui-10.0"
---
...
.NET Multi-platform App UI (.NET MAUI) apps can respond to system theme changes by consuming resources with the [[AppThemeBindingExtension|`AppThemeBinding`]] markup extension, and the `SetAppThemeColor` and `SetAppTheme<T>`  extension methods.
...
![](media/system-theme-changes/main-page-both-themes.png)

## Define and consume theme resources

Resources for light and dark themes can be consumed with the [[AppThemeBindingExtension|`AppThemeBinding`]] markup extension, and the `SetAppThemeColor` and `SetAppTheme<T>` extension methods. With these approaches, resources are automatically applied based on the value of the current system theme. In addition, objects that consume these resources are automatically updated if the system theme changes while an app is running.

### AppThemeBinding markup extension

The [[AppThemeBindingExtension|`AppThemeBinding`]] markup extension enables you to consume a resource, such as an image or color, based on the current system theme.

The [[AppThemeBindingExtension|`AppThemeBinding`]] markup extension is supported by the [[AppThemeBindingExtension|AppThemeBindingExtension]] class, which defines the following properties:
...
> [!NOTE]
> The XAML parser allows the [[AppThemeBindingExtension|AppThemeBindingExtension]] class to be abbreviated as `AppThemeBinding`.

The `Default` property is the content property of [[AppThemeBindingExtension|AppThemeBindingExtension]]. Therefore, for XAML markup expressions expressed with curly braces, you can eliminate the `Default=` part of the expression provided that it's the first argument.

The following XAML example shows how to use the [[AppThemeBindingExtension|`AppThemeBinding`]] markup extension:
...
In this example, the text color of the first [[Label (Controls)|Label]] is set to green when the device is using its light theme, and is set to red when the device is using its dark theme. Similarly, the [[Image (Controls)|Image]] displays a different image file based upon the current system theme.

Resources defined in a [[ResourceDictionary|ResourceDictionary]] can be consumed in an [[AppThemeBindingExtension|`AppThemeBinding`]] with the [[StaticResourceExtension|`StaticResource`]] markup extension:
...
In this example, the background color of the [[Grid (Controls)|Grid]] and the [[Button (Controls)|Button]] style changes based on whether the device is using its light theme or dark theme.

In addition, resources defined in a [[ResourceDictionary|ResourceDictionary]] can also be consumed in an [[AppThemeBindingExtension|`AppThemeBinding`]] with the [[DynamicResourceExtension|`DynamicResource`]] markup extension:
...
.NET MAUI includes `SetAppThemeColor` and `SetAppTheme<T>` extension methods that enable [[VisualElement (Controls)|VisualElement]] objects to respond to system theme changes.

The `SetAppThemeColor` method enables [[Color|Color]] objects to be specified that will be set on a target property based on the current system theme:
...
In this example, the text color of the [[Label (Controls)|Label]] is set to green when the device is using its light theme, and is set to red when the device is using its dark theme.
...
In this example, the [[Image (Controls)|Image]] displays `lightlogo.png` when the device is using its light theme, and `darklogo.png` when the device is using its dark theme.
