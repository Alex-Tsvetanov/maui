...
tags:
  ...
  - area/user-interface
ms_date: "05/03/2024"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/handlers/customize?view=net-maui-10.0"
---
...
- `PrependToMapping%2A`, which modifies the mapper for a handler before the .NET MAUI control mappings have been applied.
- `ModifyMapping%2A`, which modifies an existing mapping.
- `AppendToMapping%2A`, which modifies the mapper for a handler after the .NET MAUI control mappings have been applied.
...
Each handler class exposes the native view for the cross-platform control via its [[IElementHandler.PlatformView|PlatformView]] property. This property can be accessed to set native view properties, invoke native view methods, and subscribe to native view events. In addition, the cross-platform control implemented by the handler is exposed via its [[IElementHandler.VirtualView|VirtualView]] property.
...
The .NET MAUI [[Entry (Controls)|Entry]] view is a single-line text input control, that implements the [[IEntry|IEntry]] interface. The [[EntryHandler|EntryHandler]] maps the [[Entry (Controls)|Entry]] view to the following native views for each platform:
...
The following diagrams shows how the [[Entry (Controls)|Entry]] view is mapped to its native views via the [[EntryHandler|EntryHandler]]:


![](media/customize/entry-handler-net9.png)



![](media/customize/entry-handler-net10.png)


The [[Entry (Controls)|Entry]] property mapper, in the [[EntryHandler|EntryHandler]] class, maps the cross-platform control properties to the native view API. This ensures that when a property is set on an [[Entry (Controls)|Entry]], the underlying native view is updated as required.

The property mapper can be modified to customize [[Entry (Controls)|Entry]] on each platform:
...
In this example, the [[Entry (Controls)|Entry]] customization occurs in a page class. Therefore, all [[Entry (Controls)|Entry]] controls on Android, iOS, and Windows will be customized once an instance of the `CustomizeEntryPage` is created. Customization is performed by accessing the handlers [[IElementHandler.PlatformView|PlatformView]] property, which provides access to the native view that maps to the cross-platform control on each platform. Native code then customizes the handler by selecting all of the text in the [[Entry (Controls)|Entry]] when it gains focus.

For more information about mappers, see [[index#mappers|Mappers]].

## Customize a specific control instance

Handlers are global, and customizing a handler for a control will result in all controls of the same type being customized in your app. However, handlers for specific control instances can be customized by subclassing the control, and then by modifying the handler for the base control type only when the control is of the subclassed type. For example, to customize a specific [[Entry (Controls)|Entry]] control on a page that contains multiple [[Entry (Controls)|Entry]] controls, you should first subclass the [[Entry (Controls)|Entry]] control:
...
All handler-based .NET MAUI controls support [[Element.HandlerChanging|HandlerChanging]] and [[Element.HandlerChanged|HandlerChanged]] events. The [[Element.HandlerChanged|HandlerChanged]] event is raised when the native view that implements the cross-platform control is available and initialized. The [[Element.HandlerChanging|HandlerChanging]] event is raised when the control's handler is about to be removed from the cross-platform control. For more information about handler lifecycle events, see [[index#handler-lifecycle|Handler lifecycle]].

The handler lifecycle can be used to perform handler customization. For example, to subscribe to, and unsubscribe from, native view events you must register event handlers for the [[Element.HandlerChanged|HandlerChanged]] and [[Element.HandlerChanging|HandlerChanging]] events on the cross-platform control being customized:
...
Handlers can be customized per platform by using conditional compilation, or by using partial classes to organize your code into platform-specific folders and files. Each approach will be discussed in turn, by customizing an [[Entry (Controls)|Entry]] so that all of its text is selected when it gains focus.

### Conditional compilation

The code-behind file containing the event handlers for the [[Element.HandlerChanged|HandlerChanged]] and [[Element.HandlerChanging|HandlerChanging]] events is shown in the following example, which uses conditional compilation:
...
The [[Element.HandlerChanged|HandlerChanged]] event is raised after the native view that implements the cross-platform control has been created and initialized. Therefore, its event handler is where native event subscriptions should be performed. This requires casting the [[IElementHandler.PlatformView|PlatformView]] property of the handler to the type, or base type, of the native view so that native events can be accessed. In this example, on iOS, Mac Catalyst, and Windows, the `OnEntryHandlerChanged` event subscribes to native view events that are raised when the native views that implement the [[Entry (Controls)|Entry]] gain focus.

The `OnEditingDidBegin` and `OnGotFocus` event handlers access the native view for the [[Entry (Controls)|Entry]] on their respective platforms, and select all text that's in the [[Entry (Controls)|Entry]].

The [[Element.HandlerChanging|HandlerChanging]] event is raised before the existing handler is removed from the cross-platform control, and before the new handler for the cross-platform control is created. Therefore, its event handler is where native event subscriptions should be removed, and other cleanup should be performed. The [[HandlerChangingEventArgs|HandlerChangingEventArgs]] object that accompanies this event has [[HandlerChangingEventArgs.OldHandler|OldHandler]] and [[HandlerChangingEventArgs.NewHandler|NewHandler]] properties, which will be set to the old and new handlers respectively. In this example, the `OnEntryHandlerChanging` event removes the subscription to the native view events on iOS, Mac Catalyst, and Windows.
...
For information about the organization of the *Platforms* folder in a .NET MAUI project, see [[invoke-platform-code#partial-classes-and-methods|Partial classes and methods]]. For information about how to configure multi-targeting so that you don't have to place platform code into sub-folders of the *Platforms* folder, see [[configure-multi-targeting|Configure multi-targeting]].
