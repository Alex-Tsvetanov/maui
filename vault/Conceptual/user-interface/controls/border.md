...
tags:
  ...
  - area/user-interface
ms_date: "08/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/border?view=net-maui-10.0"
---

# Border

The .NET Multi-platform App UI (.NET MAUI) [[Border|Border]] is a container control that draws a border, background, or both, around another control. A [[Border|Border]] can only contain one child object. If you want to put a border around multiple objects, wrap them in a container object such as a layout.  For more information about layouts, see [[layouts|Layouts]].

[[Border|Border]] defines the following properties:

- `Content`, of type `IView`, represents the content to display in the border. This property is the [[ContentPropertyAttribute|`ContentProperty`]] of the [[Border|Border]] class, and therefore does not need to be explicitly set from XAML.
- `Padding`, of type `Thickness`, represents the distance between the border and its child element.
- `StrokeShape`, of type `IShape`, describes the shape of the border. This property has a type converter applied to it that can convert a string to its equivalent `IShape`. Its default value is [[Rectangle|Rectangle]]. Therefore, a [[Border|Border]] will be rectangular by default.
- `Stroke`, of type [[Brush|Brush]], indicates the brush used to paint the border.
...
> [!IMPORTANT]
> When creating a border using a shape, such as a [[Rectangle|Rectangle]] or [[Polygon (Shapes)|Polygon]], only closed shapes should be used. Therefore, open shapes such as [[Line|Line]] are unsupported.

For more information about the properties that control the shape and stroke of the border, see [[shapes|Shapes]].

## Create a Border

To draw a border, create a [[Border|Border]] object and set its properties to define its appearance. Then, set its child to the control to which the border should be added.

The following XAML example shows how to draw a border around a [[Label (Controls)|Label]]:
...
In this example, a border with rounded top-left and bottom-right corners is drawn around a [[Label (Controls)|Label]]. The border shape is defined as a [[RoundRectangle|RoundRectangle]] object, whose `CornerRadius` property is set to a `Thickness` value that enables independent control of each corner of the rectangle:

![](media/border/border.png)

Because the `Stroke` property is of type [[Brush|Brush]], borders can also be drawn using gradients:
...
In this example, a border that uses a linear gradient is drawn around a [[Label (Controls)|Label]]:

![](media/border/linear-gradient-border.png)
...
- `Path`, followed by path markup syntax data. For example, `Path M 10,100 L 100,100 100,50Z` will draw a triangular border. For more information about path markup syntax, see [[path-markup-syntax|Path markup syntax]].
