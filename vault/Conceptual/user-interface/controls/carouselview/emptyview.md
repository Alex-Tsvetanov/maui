...
tags:
  ...
  - area/user-interface
ms_date: "09/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/carouselview/emptyview?view=net-maui-10.0"
---
...
The .NET Multi-platform App UI (.NET MAUI) [[CarouselView|CarouselView]] defines the following properties that can be used to provide user feedback when there's no data to display:
...
The main usage scenarios for setting the `EmptyView` property are displaying user feedback when a filtering operation on a [[CarouselView|CarouselView]] yields no data, and displaying user feedback while data is being retrieved from a web service.
...
When the [[SearchBar (Controls)|SearchBar]] executes the `FilterCommand`, the collection displayed by the [[CarouselView|CarouselView]] is filtered for the search term stored in the `SearchBar.Text` property. If the filtering operation yields no data, the [[StackLayout (Controls)|StackLayout]] set as the `EmptyView` property value is displayed.
...
The `ToggleEmptyView` method sets the `EmptyView` property of the [[CarouselView|CarouselView]] object to one of the two [[ContentView (Controls)|ContentView]] objects stored in the [[ResourceDictionary|ResourceDictionary]], based on the value of the `Switch.IsToggled` property. When the [[SearchBar (Controls)|SearchBar]] executes the `FilterCommand`, the collection displayed by the [[CarouselView|CarouselView]] is filtered for the search term stored in the `SearchBar.Text` property. If the filtering operation yields no data, the [[ContentView (Controls)|ContentView]] object set as the `EmptyView` property is displayed.
...
The appearance of the `EmptyView` can be chosen at runtime, based on its value, by setting the `CarouselView.EmptyViewTemplate` property to a [[DataTemplateSelector|DataTemplateSelector]] object:
...
When the [[SearchBar (Controls)|SearchBar]] executes the `FilterCommand`, the collection displayed by the [[CarouselView|CarouselView]] is filtered for the search term stored in the `SearchBar.Text` property. If the filtering operation yields no data, the [[DataTemplate|DataTemplate]] chosen by the `SearchTermDataTemplateSelector` object is set as the `EmptyViewTemplate` property and displayed.
