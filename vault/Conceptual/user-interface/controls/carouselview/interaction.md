...
tags:
  ...
  - area/user-interface
ms_date: "09/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/carouselview/interaction?view=net-maui-10.0"
---
...
The .NET Multi-platform App UI (.NET MAUI) [[CarouselView|CarouselView]] defines the following properties that control user interaction:
...
- `IsBounceEnabled`, of type `bool`, which specifies whether the [[CarouselView|CarouselView]] will bounce at a content boundary. The default value is `true`.
...
- `Loop`, of type `bool`, which determines whether the [[CarouselView|CarouselView]] provides looped access to its collection of items. The default value is `true`.
...
[[CarouselView|CarouselView]] defines a `CurrentItemChanged` event that's fired when the `CurrentItem` property changes, either due to user scrolling, or when an application sets the property. The `CurrentItemChangedEventArgs` object that accompanies the `CurrentItemChanged` event has two properties, both of type `object`:

- `PreviousItem` – the previous item, after the property change.
- `CurrentItem` – the current item, after the property change.

[[CarouselView|CarouselView]] also defines a `PositionChanged` event that's fired when the `Position` property changes, either due to user scrolling, or when an application sets the property. The `PositionChangedEventArgs` object that accompanies the `PositionChanged` event has two properties, both of type `int`:
...
The following XAML example shows a [[CarouselView|CarouselView]] that uses an event handler to respond to the current item changing:
...
![](media/interaction/current-item-events.png)

### Command

The following XAML example shows a [[CarouselView|CarouselView]] that uses a command to respond to the current item changing:
...
The following XAML example shows a [[CarouselView|CarouselView]] that uses an event handler to respond to the `Position` property changing:
...
![](media/interaction/current-position-events.png)

### Command

The following XAML example shows a [[CarouselView|CarouselView]] that uses a command to respond to the `Position` property changing:
...
The current item in a [[CarouselView|CarouselView]] can be programmatically set by setting the `CurrentItem` property to the item. The following XAML example shows a [[CarouselView|CarouselView]] that pre-chooses the current item:
...
![](media/interaction/preset-item.png)

## Preset the position

The displayed item in a [[CarouselView|CarouselView]] can be programmatically set by setting the `Position` property to the index of the item in the underlying collection. The following XAML example shows a [[CarouselView|CarouselView]] that sets the displayed item:
...
The `CarouselView.Position` property data binds to the `Position` property of the connected view model, which is of type `int`. By default, a `TwoWay` binding is used so that if the user scrolls through the [[CarouselView|CarouselView]], the value of the `Position` property will be set to the index of the displayed item. The `Position` property is defined in the `MonkeysViewModel` class:
...
![](media/interaction/preset-position.png)

## Define visual states

[[CarouselView|CarouselView]] defines four visual states:
...
These visual states can be used to initiate visual changes to the items displayed by the [[CarouselView|CarouselView]].
...
In this example, the `CurrentItem` visual state specifies that the current item displayed by the [[CarouselView|CarouselView]] will have its `Scale` property changed from its default value of 1 to 1.1. The `PreviousItem` and `NextItem` visual states specify that the items surrounding the current item will be displayed with an `Opacity` value of 0.5. The `DefaultItem` visual state specifies that the remainder of the items displayed by the [[CarouselView|CarouselView]] will be displayed with an `Opacity` value of 0.25.

> [!NOTE]
> Alternatively, the visual states can be defined in a [[Style|Style]] that has a `TargetType` property value that's the type of the root element of the [[DataTemplate|DataTemplate]], which is set as the `ItemTemplate` property value.

The following screenshot shows the `CurrentItem`, `PreviousItem`, and `NextItem` visual states:

![](media/interaction/visual-states.png)
...
By default, [[CarouselView|CarouselView]] bounces items at content boundaries. This can be disabled by setting the `IsBounceEnabled` property to `false`.
...
By default, [[CarouselView|CarouselView]] allows users to move through items using a swipe gesture. This swipe interaction can be disabled by setting the `IsSwipeEnabled` property to `false`.
