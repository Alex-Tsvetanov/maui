...
tags:
  ...
  - area/user-interface
ms_date: "08/19/2025"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/datepicker?view=net-maui-10.0"
---

# DatePicker

The .NET Multi-platform App UI (.NET MAUI) [[DatePicker (Controls)|DatePicker]] invokes the platform's date-picker control and allows you to select a date.

[[DatePicker (Controls)|DatePicker]] defines eight properties:
...
- `TextColor` of type [[Color|Color]], the color used to display the selected date.
...
- `CharacterSpacing`, of type `double`, is the spacing between characters of the [[DatePicker (Controls)|DatePicker]] text.

All eight properties are backed by [[BindableProperty|BindableProperty]] objects, which means that they can be styled, and the properties can be targets of data bindings. The `Date` property has a default binding mode of `BindingMode.TwoWay`, which means that it can be a target of a data binding in an application that uses the Model-View-ViewModel (MVVM) pattern.


[[DatePicker (Controls)|DatePicker]] defines the following properties:
...
All of these properties are backed by [[BindableProperty|BindableProperty]] objects. The `Date` property has a default binding mode of `BindingMode.TwoWay`.
...
> [!WARNING]
> When setting `MinimumDate` and `MaximumDate`, make sure that `MinimumDate` is always less than or equal to `MaximumDate`. Otherwise, [[DatePicker (Controls)|DatePicker]] will raise an exception.

The [[DatePicker (Controls)|DatePicker]] ensures that `Date` is between `MinimumDate` and `MaximumDate`, inclusive. If `MinimumDate` or `MaximumDate` is set so that `Date` is not between them, [[DatePicker (Controls)|DatePicker]] will adjust the value of `Date`.

The [[DatePicker (Controls)|DatePicker]] raises a `DateSelected` event when the user selects a date.
...
The following screenshot shows the resulting [[DatePicker (Controls)|DatePicker]] on iOS:

![](media/datepicker/date-picker.png)

If the `BindingContext` property of [[DatePicker (Controls)|DatePicker]] is set to an instance of a viewmodel containing properties of type `DateTime` named `MinDate`, `MaxDate`, and `SelectedDate` (for example), you can instantiate the [[DatePicker (Controls)|DatePicker]] like this:
...
If the [[DatePicker (Controls)|DatePicker]] does not contain a binding on its `Date` property, your app should attach a handler to the `DateSelected` event to be informed when the user selects a new date.
...
For information about setting font properties, see [[fonts|Fonts]].

## DatePicker and layout

It's possible to use an unconstrained horizontal layout option such as `Center`, `Start`, or `End` with [[DatePicker (Controls)|DatePicker]]:
...
> [!TIP]
> It's best to use the default `HorizontalOptions` setting of `Fill` with [[DatePicker (Controls)|DatePicker]], and not to use a width of `Auto` when putting [[DatePicker (Controls)|DatePicker]] in a [[Grid (Controls)|Grid]] cell.

<!--
> [!TIP]
> On Android, the [[DatePicker (Controls)|DatePicker]] dialog can be customized by overriding the `CreateDatePickerDialog` method in a custom renderer. This allows, for example, additional buttons to be added to the dialog. -->

## Localize a DatePicker on Windows

For apps targeting Windows, ensuring that the [[DatePicker (Controls)|DatePicker]] displays dates in a format that's localized to the user's settings, including the names of months and days in the picker's dialog, requires specific configuration in your project's *Package.appxmanifest* file. Localizing the elements in the package manifest improves the user experience by adhering to the cultural norms of the user's locale.

Localizing the date formats and strings in the `[[DatePicker (Controls)|DatePicker]]` requires declaring the supported languages within your *Package.appxmanifest* file.

Follow these steps to configure your [[DatePicker (Controls)|DatePicker]] for localization on Windows:
...
This configuration ensures that the [[DatePicker (Controls)|DatePicker]] will display date formats, months, and days according to the user's locale, significantly enhancing the app's usability and accessibility across different regions.

For more information about localization in .NET MAUI apps, see [[localization|Localization]].

## Nullable selected date (.NET 10)


In .NET 10, [[DatePicker (Controls)|DatePicker]] supports nullable values so you can represent “no date selected” and clear selection in bindings:
