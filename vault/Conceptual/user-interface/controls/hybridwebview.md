...
tags:
  ...
  - area/user-interface
ms_date: "12/08/2025"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/hybridwebview?view=net-maui-10.0"
---
...
The .NET Multi-platform App UI (.NET MAUI) [[HybridWebView|HybridWebView]] enables hosting arbitrary HTML/JS/CSS content in a web view, and enables communication between the code in the web view (JavaScript) and the code that hosts the web view (C#/.NET). For example, if you have an existing React JS app, you could host it in a cross-platform .NET MAUI native app, and build the back-end of the app using C# and .NET.

[[HybridWebView|HybridWebView]] defines the following properties:

- [[HybridWebView.DefaultFile|DefaultFile]], of type `string?`, which specifies the file within the [[HybridWebView.HybridRoot|HybridRoot]] that should be served as the default file. The default value is *index.html*.
- [[HybridWebView.HybridRoot|HybridRoot]], of type `string?`, which is the path within the app's raw asset resources that contain the web app's contents. The default value is *wwwroot*, which maps to *Resources/Raw/wwwroot*.

In addition, [[HybridWebView|HybridWebView]] defines a [[HybridWebView.RawMessageReceived|RawMessageReceived]] event that's raised when a raw message is received. The [[HybridWebViewRawMessageReceivedEventArgs|HybridWebViewRawMessageReceivedEventArgs]] object that accompanies the event defines a [[HybridWebViewRawMessageReceivedEventArgs.Message|Message]] property that contains the message.

Your app's C# code can invoke synchronous and asynchronous JavaScript methods within the [[HybridWebView|HybridWebView]] with the `InvokeJavaScriptAsync%2A` and `EvaluateJavaScriptAsync%2A` methods. Your app's JavaScript code can also invoke synchronous and asynchronous C# methods. For more information, see [Invoke JavaScript from C#](#invoke-javascript-from-c) and [Invoke C# from JavaScript](#invoke-c-from-javascript).

To create a .NET MAUI app with [[HybridWebView|HybridWebView]] you need:
...
> [!IMPORTANT]
> By default, the [[HybridWebView|HybridWebView]] control won't be available when full trimming or Native AOT is enabled. To change this behavior, see [[trimming#trimming-feature-switches|Trimming feature switches]].

![[webview2-program-files-warning]]

![[browser-engines]]

## Create a .NET MAUI HybridWebView app

To create a .NET MAUI app with a [[HybridWebView|HybridWebView]]:
...
1. Add your web content to the .NET MAUI app project.
    ...
    Your app's web content should be included as part of a .NET MAUI project as raw assets. A raw asset is any file in the app's *Resources\Raw* folder, and includes sub-folders. For a default [[HybridWebView|HybridWebView]], web content should be placed in the *Resources\Raw\wwwroot* folder, with the main file named *index.html*.
    ...
    - *Resources\Raw\wwwroot\scripts\HybridWebView.js* with the standard [[HybridWebView|HybridWebView]] JavaScript library:
...
1. Add the [[HybridWebView|HybridWebView]] control to your app:
...
1. Modify the `CreateMauiApp` method of your `MauiProgram` class to enable developer tools on the underlying WebView controls when your app is running in debug configuration. To do this, call the `AddHybridWebViewDeveloperTools%2A` method on the `IServiceCollection` object:
...
1. Use the [[HybridWebView|HybridWebView]] APIs to send messages between the JavaScript and C# code:
...
Your app's C# code can synchronously and asynchronously invoke JavaScript methods within the [[HybridWebView|HybridWebView]], with optional parameters and an optional return value. This can be achieved with the `InvokeJavaScriptAsync%2A` and `EvaluateJavaScriptAsync%2A` methods:

- The `EvaluateJavaScriptAsync%2A` method runs the JavaScript code provided via a parameter and returns the result as a string.
- The `InvokeJavaScriptAsync%2A` method invokes a specified JavaScript method, optionally passing in parameter values, and specifies a generic argument that indicates the type of the return value. It returns an object of the generic argument type that contains the return value of the called JavaScript method. Internally, parameters and return values are JSON encoded.


> [!NOTE]
> .NET 10 includes an `InvokeJavaScriptAsync%2A` overload that invokes a specified JavaScript method without specifying any information about the return type. For more information, see [Invoke JavaScript methods that don't return a value](#invoke-javascript-methods-that-dont-return-a-value).
x


### Invoke synchronous JavaScript

Synchronous JavaScript methods can be invoked with the `EvaluateJavaScriptAsync%2A` and `InvokeJavaScriptAsync%2A` methods. In the following example the `InvokeJavaScriptAsync%2A` method is used to demonstrate invoking JavaScript that's embedded in an app's web content. For example, a simple Javascript method to add two numbers could be defined in your web content:
...
The `AddNumbers` JavaScript method can be invoked from C# with the `InvokeJavaScriptAsync%2A` method:
...
Asynchronous JavaScript methods can be invoked with the `EvaluateJavaScriptAsync%2A` and `InvokeJavaScriptAsync%2A` methods. In the following example the `InvokeJavaScriptAsync%2A` method is used to demonstrate invoking JavaScript that's embedded in an app's web content. For example, a Javascript method that asynchronously retrieves data could be defined in your web content:
...
The `EvaluateMeWithParamsAndAsyncReturn` JavaScript method can be invoked from C# with the `InvokeJavaScriptAsync%2A` method:
...
The `InvokeJavaScriptAsync%2A` method can also be used to invoke JavaScript methods that don't return a value. There are alternative approaches to doing this:


- Invoke the `InvokeJavaScriptAsync%2A`, specifying the JavaScript method name and any optional parameters:
...
- Invoke the `InvokeJavaScriptAsync%2A` method without specifying the generic argument:
...
- Invoke the `InvokeJavaScriptAsync%2A` method while specifying the generic argument:
...
By default, invocation of JavaScript methods in a [[HybridWebView|HybridWebView]] can hide exceptions thrown by your JavaScript code. To opt into JavaScript exceptions being sent to .NET, where they're re-thrown as .NET exceptions, add the following code to your `MauiProgram` class:
...
Your app's JavaScript code within the [[HybridWebView|HybridWebView]] can synchronously and asynchronously invoke C# methods, with optional parameters and an optional return value. This can be achieved by:

- Defining public C# methods that will be invoked from JavaScript.
- Calling the `SetInvokeJavaScriptTarget%2A` method to set the object that will be the target of JavaScript calls from the [[HybridWebView|HybridWebView]].
...
You must then call the `SetInvokeJavaScriptTarget%2A` method to set the object that will be the target of JavaScript calls from the [[HybridWebView|HybridWebView]]:
...
The public methods on the object set via the `SetInvokeJavaScriptTarget%2A` method can then be invoked from JavaScript with the `window.HybridWebView.InvokeDotNet` function:
...
The JavaScript object is automatically serialized to JSON when sent to C#, and the [[HybridWebView|HybridWebView]] automatically deserializes it to the expected C# type. Nested objects, arrays, and other complex structures are supported, as long as the JavaScript object structure matches the C# class structure.
...
> [!NOTE]
> .NET 10 includes events to directly handle the initialization start ([[HybridWebView.WebViewInitializing|WebViewInitializing]]) and the initialization end ([[HybridWebView.WebViewInitialized|WebViewInitialized]]).

While [[HybridWebView|HybridWebView]] doesn’t expose app-facing initializing/initialized events like [[BlazorWebView (Maui)|BlazorWebView]], you can still customize the underlying platform web views and run code after they’re ready:

- Windows (WebView2): the platform view is [[HybridWebView|HybridWebView]], which inherits `WebView2` and adds `RunAfterInitialize(Action)` so you can safely access `CoreWebView2` once it’s ready.
...
The [[HybridWebView|HybridWebView]] exposes app-facing initializing/initialized events so that you can customize the underlying platform web views and run code after they’re ready:

In most cases, you can just add an event handler for the [[HybridWebView.WebViewInitialized|WebViewInitialized]] event:

```xaml
<HybridWebView WebViewInitialized="HybridWebViewInitialized" />
```

In your code-behind, you can get access to the platform features using the [[WebViewInitializedEventArgs.PlatformArgs|PlatformArgs]] property. This property will provide an instance of [[PlatformWebViewInitializedEventArgs|PlatformWebViewInitializedEventArgs]] which gives you access to all the available settings and configuration needed to control the underlying web view:
...
In most cases, you can just add an event handler for the [[HybridWebView.WebViewInitializing|WebViewInitializing]] event:

```xaml
<HybridWebView WebViewInitializing="HybridWebViewInitializing" />
```

In your code-behind, you can get access to the platform features using the [[WebViewInitializingEventArgs.PlatformArgs|PlatformArgs]] property. This property will provide an instance of [[PlatformWebViewInitializingEventArgs|PlatformWebViewInitializingEventArgs]] which gives you access to all the available settings and configuration needed to control the underlying web view:
...
[[HybridWebView|HybridWebView]] can intercept and respond to web requests that originate from within the hosted web content. This enables scenarios such as modifying headers, redirecting requests, or supplying local responses.
