...
tags:
  ...
  - area/user-interface
ms_date: "05/13/2025"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/entry?view=net-maui-10.0"
---

# Entry

The .NET Multi-platform App UI (.NET MAUI) [[Entry (Controls)|Entry]] allows you to enter and edit a single line of text. In addition, the [[Entry (Controls)|Entry]] can be used as a password field.

[[Entry (Controls)|Entry]] defines the following properties:
...
- `HorizontalTextAlignment`, of type [[TextAlignment|TextAlignment]], defines the horizontal alignment of the text.
...
- `VerticalTextAlignment`, of type [[TextAlignment|TextAlignment]], defines the vertical alignment of the text.
...
In addition, [[Entry (Controls)|Entry]] defines a `Completed` event, which is raised when the user finalizes text in the [[Entry (Controls)|Entry]] with the return key.

[[Entry (Controls)|Entry]] derives from the [[InputView (Controls)|InputView]] class, from which it inherits the following properties:
...
The following example shows how to create an [[Entry (Controls)|Entry]]:
...
The following screenshot shows the resulting [[Entry (Controls)|Entry]] on Android:

![](media/entry/entry.png)
...
The `TextChanged` event is raised when the text in the [[Entry (Controls)|Entry]] changes, and the `TextChangedEventArgs` provide the text before and after the change via the `OldTextValue` and `NewTextValue` properties:
...
> [!NOTE]
> The [[VisualElement (Controls)|VisualElement]] class, which is in the [[Entry (Controls)|Entry]] inheritance hierarchy, also has `Focused` and `Unfocused` events.

## Set character spacing

Character spacing can be applied to an [[Entry (Controls)|Entry]] by setting the `CharacterSpacing` property to a `double` value:
...
The result is that characters in the text displayed by the [[Entry (Controls)|Entry]] are spaced `CharacterSpacing` device-independent units apart.
...
The `MaxLength` property can be used to limit the input length that's permitted for the [[Entry (Controls)|Entry]]. This property should be set to a positive integer:
...
A `MaxLength` property value of 0 indicates that no input will be allowed, and a value of `int.MaxValue`, which is the default value for an [[Entry (Controls)|Entry]], indicates that there is no effective limit on the number of characters that may be entered.
...
The default value of the `CursorPosition` property is 0, which indicates that text will be inserted at the start of the [[Entry (Controls)|Entry]].

In addition, the `SelectionLength` property can be used to return or set the length of text selection within the [[Entry (Controls)|Entry]]:
...
The `ClearButtonVisibility` property can be used to control whether an [[Entry (Controls)|Entry]] displays a clear button, which enables the user to clear the text. This property should be set to a `ClearButtonVisibility` enumeration member:
...
- `WhileEditing` indicates that a clear button will be displayed in the [[Entry (Controls)|Entry]], while it has focus and text.
...
The following screenshot shows an [[Entry (Controls)|Entry]] on Android with the clear button enabled:

![](media/entry/entry-clearbutton.png)

## Transform text

An [[Entry (Controls)|Entry]] can transform the casing of its text, stored in the `Text` property, by setting the `TextTransform` property to a value of the `TextTransform` enumeration. This enumeration has four values:
...
[[Entry (Controls)|Entry]] provides the `IsPassword` property which visually obscures entered text when it's set to `true`:

```xaml
<Entry IsPassword="true" />
```

The following screenshot shows an [[Entry (Controls)|Entry]] whose input has been obscured:

![](media/entry/entry-password.png)

## Customize the keyboard

The soft input keyboard that's presented when users interact with an [[Entry (Controls)|Entry]] can be set programmatically via the `Keyboard` property, to one of the following properties from the `Keyboard` class:
...
The appearance of the return key on the soft input keyboard, which is displayed when an [[Entry (Controls)|Entry]] has focus, can be customized by setting the `ReturnType` property to a value of the `ReturnType` enumeration:
...
When the <kbd>Return</kbd> key is pressed, the `Completed` event fires and any `ICommand` specified by the `ReturnCommand` property is executed. In addition, any `object` specified by the `ReturnCommandParameter` property will be passed to the `ICommand` as a parameter. For more information about commands, see [[commanding|Commanding]].

![[soft-input-extensions]]

The following example shows how to hide the soft input keyboard on an [[Entry (Controls)|Entry]] named `entry`, if it's currently showing:
...
Users can be prevented from modifying the text in an [[Entry (Controls)|Entry]] by setting the `IsReadOnly` property to `true`:
...
> [!NOTE]
> The `IsReadonly` property does not alter the visual appearance of an [[Entry (Controls)|Entry]], unlike the `IsEnabled` property that also changes the visual appearance of the [[Entry (Controls)|Entry]] to gray.
