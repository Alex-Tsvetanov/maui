...
tags:
  ...
  - area/user-interface
ms_date: "05/12/2026"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/editor?view=net-maui-10.0"
---

# Editor

The .NET Multi-platform App UI (.NET MAUI) [[Editor|Editor]] allows you to enter and edit multiple lines of text.

[[Editor|Editor]] defines the following properties:
...
In addition, [[Editor|Editor]] defines a `Completed` event, which is raised when the user finalizes text in the [[Editor|Editor]] with the return key.


[[Editor|Editor]] also defines `ReturnCommand` and `ReturnCommandParameter` properties, both of which are backed by [[BindableProperty|BindableProperty]] objects. `ReturnCommand`, of type `ICommand`, is executed whenever the `Completed` event is raised. `ReturnCommandParameter`, of type `object`, is passed to `ReturnCommand` when it's executed. This lets you bind a view model command directly to the editor's completion gesture without subscribing to the `Completed` event in code-behind.


[[Editor|Editor]] derives from the [[InputView (Controls)|InputView]] class, from which it inherits the following properties:
...
In addition, [[InputView (Controls)|InputView]] defines a `TextChanged` event, which is raised when the text in the [[Editor|Editor]] changes. The `TextChangedEventArgs` object that accompanies the `TextChanged` event has `NewTextValue` and `OldTextValue` properties, which specify the new and old text, respectively.

For information about specifying fonts on an [[Editor|Editor]], see [[fonts|Fonts]].

> [!TIP]
> On Android, .NET 10 switches the native view for [[Editor|Editor]] to `MauiAppCompatEditText`, which enables selection change tracking. As the selection changes, [[InputView (Controls).CursorPosition|CursorPosition]] and [[InputView (Controls).SelectionLength|SelectionLength]] are updated so you can react to selection moves.
...
The following example shows how to create an [[Editor|Editor]]:
...
The following screenshot shows the resulting [[Editor|Editor]] on Android:

![](media/editor/editor.png)
...
The `TextChanged` event is raised when the text in the [[Editor|Editor]] changes, and the `TextChangedEventArgs` provide the text before and after the change via the `OldTextValue` and `NewTextValue` properties:
...
Character spacing can be applied to an [[Editor|Editor]] by setting the `CharacterSpacing` property to a `double` value:
...
The result is that characters in the text displayed by the [[Editor|Editor]] are spaced `CharacterSpacing` device-independent units apart.
...
The `MaxLength` property can be used to limit the input length that's permitted for the [[Editor|Editor]]. This property should be set to a positive integer:

```xaml
<Editor ... MaxLength="10" />
```

A `MaxLength` property value of 0 indicates that no input will be allowed, and a value of `int.MaxValue`, which is the default value for an [[Editor|Editor]], indicates that there is no effective limit on the number of characters that may be entered.

## Auto-size an Editor

An [[Editor|Editor]] can be made to auto-size to its content by setting the `Editor.AutoSize` property to `TextChanges`, which is a value of the `EditorAutoSizeOption` enumeration. This enumeration has two values:
...
When auto-resizing is enabled, the height of the [[Editor|Editor]] will increase when the user fills it with text, and the height will decrease as the user deletes text. This can be used to ensure that [[Editor|Editor]] objects in a [[DataTemplate|DataTemplate]] in a [[CollectionView|CollectionView]] size correctly.

> [!IMPORTANT]
> An [[Editor|Editor]] will not auto-size if the [[VisualElement (Controls).HeightRequest|HeightRequest]] property has been set.

## Transform text

An [[Editor|Editor]] can transform the casing of its text, stored in the `Text` property, by setting the `TextTransform` property to a value of the `TextTransform` enumeration. This enumeration has four values:
...
The keyboard that's presented when users interact with an [[Editor|Editor]] can be set programmatically via the `Keyboard` property, to one of the following properties from the `Keyboard` class:
...
The following example shows how to hide the soft input keyboard on an [[Editor|Editor]] named `editor`, if it's currently showing:
...
Users can be prevented from modifying the text in an [[Editor|Editor]] by setting the `IsReadOnly` property, which has a default value of `false`, to `true`:
...
> [!NOTE]
> The `IsReadonly` property does not alter the visual appearance of an [[Editor|Editor]], unlike the `IsEnabled` property that also changes the visual appearance of the [[Editor|Editor]] to gray.
