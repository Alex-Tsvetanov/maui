...
tags:
  ...
  - area/user-interface
ms_date: "09/19/2025"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/webview?view=net-maui-10.0"
---

# WebView

The .NET Multi-platform App UI (.NET MAUI) [[WebView (Controls)|WebView]] displays remote web pages, local HTML files, and HTML strings, in an app. The content displayed a [[WebView (Controls)|WebView]] includes support for Cascading Style Sheets (CSS), and JavaScript. By default, .NET MAUI projects include the platform permissions required for a [[WebView (Controls)|WebView]] to display a remote web page.

[[WebView (Controls)|WebView]] defines the following properties:

- [[WebView (Controls).Cookies|Cookies]], of type `CookieContainer`, provides storage for a collection of cookies.
- [[WebView (Controls).CanGoBack|CanGoBack]], of type `bool`, indicates whether the user can navigate to previous pages. This is a read-only property.
- [[WebView (Controls).CanGoForward|CanGoForward]], of type `bool`, indicates whether the user can navigate forward. This is a read-only property.
- [[WebView (Controls).Source|Source]], of type `WebViewSource`, represents the location that the [[WebView (Controls)|WebView]] displays.
- [[WebView (Controls).UserAgent|UserAgent]], of type `string`, represents the user agent. The default value is the user agent of the underlying platform browser, or `null` if it can't be determined.
...
- `ProcessTerminated`, that's raised when a [[WebView (Controls)|WebView]] process ends unexpectedly. The `WebViewProcessTerminatedEventArgs` object that accompanies this event defines platform-specific properties that indicate why the process failed.


> [!IMPORTANT]
> A [[WebView (Controls)|WebView]] must specify its [[VisualElement (Controls).HeightRequest|HeightRequest]] and [[VisualElement (Controls).WidthRequest|WidthRequest]] properties when contained in a [[HorizontalStackLayout|HorizontalStackLayout]], [[StackLayout (Controls)|StackLayout]], or [[VerticalStackLayout|VerticalStackLayout]]. If you fail to specify these properties, the [[WebView (Controls)|WebView]] will not render.
...
<!-- For more information, see [[ats|App Transport Security]]. -->
...
For more information about raw assets, see [[single-project#raw-assets|Raw assets]].

## Reload content

[[WebView (Controls)|WebView]] has a `Reload` method that can be called to reload its source:
...
[[WebView (Controls)|WebView]] supports programmatic navigation with the `GoBack` and `GoForward` methods. These methods enable navigation through the [[WebView (Controls)|WebView]] page stack, and should only be called after inspecting the values of the `CanGoBack` and `CanGoForward` properties:
...
When page navigation occurs in a [[WebView (Controls)|WebView]], either initiated programmatically or by the user, the following events occur:
...
On Android, JavaScript execution is enabled by default for [[WebView (Controls)|WebView]]. You can toggle this behavior at runtime using the Android-specific platform configuration APIs:
...
> [!NOTE]
> Disabling JavaScript may break functionality on sites that rely on it. Additionally, calls to `EvaluateJavaScriptAsync%2A` won't execute scripts while JavaScript is disabled.

## Play video full screen on Android

When videos are hosted in a [[WebView (Controls)|WebView]] on Android, they can be played fullscreen by including `allowfullscreen` in the `iframe`:
...
On Android, navigation won't occur in a [[WebView (Controls)|WebView]] when a hyperlink that specifies `target="_blank"` (to open the content in a new window) is pressed. This is because opening a hyperlink in a new window requires `OnCreateWindow%2A` to be implemented, which .NET MAUI doesn't. Therefore, for this scenario you should decide whether to implement `OnCreateWindow%2A` yourself, open the URL in the system browser, or do something else.

Alternatively, to force all hyperlinks to open in the same [[WebView (Controls)|WebView]], modify the `WebViewHandler` in your app so that the native `WebView` disables support for multiple windows:
...
This code customizes the property mapper for the `WebViewHandler` on Android by calling the `SetSupportMultipleWindows%2A` method with a `false` argument, and should be executed before a user can navigate to a hyperlink that specifies `target="_blank"`. For more information about handlers, see [[handlers|Handlers]].

## Handle permissions on Android

When browsing to a page that requests access to the device's recording hardware, such as the camera or microphone, permission must be granted by the [[WebView (Controls)|WebView]] control. The `WebView` control uses the `WebChromeClient` type on Android to react to permission requests. However, the `WebChromeClient` implementation provided by .NET MAUI ignores permission requests. You must create a new type that inherits from `MauiWebChromeClient` and approves the permission requests.
...
01. Use the `SetWebChromeClient%2A` method on the Android's `WebView` control to set the chrome client to `MyWebChromeClient`. The following two items demonstrate how you can set the chrome client:
    ...
    - You can also use handler property mapping to force all `WebView` controls to use your chrome client. For more information, see [[handlers|Handlers]].
...
Cookies can be set on a [[WebView (Controls)|WebView]] so that they are sent with the web request to the specified URL. Set the cookies by adding `Cookie` objects to a `CookieContainer`, and then set the container as the value of the `WebView.Cookies` bindable property. The following code shows an example:
...
In this example, a single `Cookie` is added to the `CookieContainer` object, which is then set as the value of the `WebView.Cookies` property. When the  [[WebView (Controls)|WebView]] sends a web request to the specified URL, the cookie is sent with the request.

## Invoke JavaScript

[[WebView (Controls)|WebView]] includes the ability to invoke a JavaScript function from C# and return any result to the calling C# code. This interop is accomplished with the `EvaluateJavaScriptAsync` method, which is shown in the following example:
...
Since the configuration of the native [[WebView (Controls)|WebView]] can't be changed on iOS and Mac Catalyst once the handler's platform view is created, you should create a custom handler factory delegate to modify it:
...
> [!NOTE]
> You should configure `MauiWKWebView` with a `WKWebViewConfiguration` object before a [[WebView (Controls)|WebView]] is displayed in your app. Suitable locations to do this are in your app's startup path, such as in *MauiProgram.cs* or *App.xaml.cs*. <!-- For more information about configuring a native .NET MAUI control, see [[customize|Customize controls]]. -->



## Set media playback preferences on iOS and Mac Catalyst

Inline media playback of HTML5 video, including autoplay and picture in picture, is enabled by default for the [[WebView (Controls)|WebView]] on iOS and Mac Catalyst. To change this default, or set other media playback preferences, you should create a custom handler factory delegate since media playback preferences can't be changed once the handler's platform view is created. The following code shows an example of doing this:
...
For more information about configuring a [[WebView (Controls)|WebView]] on iOS, see [Configure the native WebView on iOS and Mac Catalyst](#configure-the-native-webview-on-ios-and-mac-catalyst).



## Inspect a WebView on Mac Catalyst

To use Safari developer tools to inspect the content of a [[WebView (Controls)|WebView]] on Mac Catalyst, add the following code to your app:
...
This code customizes the property mapper for the `WebViewHandler` on Mac Catalyst, to make [[WebView (Controls)|WebView]] content inspectable by Safari developer tools. For more information about handlers, see [[handlers|Handlers]].
...
For more information, see [[launcher|Launcher]].
