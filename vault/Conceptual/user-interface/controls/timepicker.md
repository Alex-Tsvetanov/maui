...
tags:
  ...
  - area/user-interface
ms_date: "08/19/2025"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/timepicker?view=net-maui-10.0"
---

# TimePicker

The .NET Multi-platform App UI (.NET MAUI) [[TimePicker (Controls)|TimePicker]] invokes the platform's time-picker control and allows you to select a time.

[[TimePicker (Controls)|TimePicker]] defines the following properties:
...
- `TextColor` of type [[Color|Color]], the color used to display the selected time.
...
- `CharacterSpacing`, of type `double`, is the spacing between characters of the [[TimePicker (Controls)|TimePicker]] text.

All of these properties are backed by [[BindableProperty|BindableProperty]] objects, which means that they can be styled, and the properties can be targets of data bindings. The `Time` property has a default binding mode of `BindingMode.TwoWay`, which means that it can be a target of a data binding in an application that uses the Model-View-ViewModel (MVVM) pattern.
...
All of these properties are backed by [[BindableProperty|BindableProperty]] objects. The `Time` property has a default binding mode of `BindingMode.TwoWay`.
...
> [!NOTE]
> The [[TimePicker (Controls)|TimePicker]] doesn't include an event to indicate a new selected `Time` value. If you need to be notified of this, you can add an event handler for the `PropertyChanged` event.



In addition, [[TimePicker (Controls)|TimePicker]] defines a [[TimePicker (Controls).TimeSelected|TimeSelected]] event, which is raised when the selected time changes. The [[TimeChangedEventArgs|TimeChangedEventArgs]] object that accompanies the `TimeSelected` event has `NewTime` and `OldTime` properties, which specify the new and old time, respectively.
...
The following screenshot shows the resulting [[TimePicker (Controls)|TimePicker]] on iOS:

![](media/timepicker/time-picker.png)

If the `BindingContext` property of [[TimePicker (Controls)|TimePicker]] is set to an instance of a viewmodel containing a property of type `TimeSpan` named `SelectedTime` (for example), you can instantiate the [[TimePicker (Controls)|TimePicker]] like this:
...
It's possible to use an unconstrained horizontal layout option such as `Center`, `Start`, or `End` with [[TimePicker (Controls)|TimePicker]]:
...
However, this is not recommended. Depending on the setting of the `Format` property, selected times might require different display widths. For example, the "T" format string causes the [[TimePicker (Controls)|TimePicker]] view to display times in a long format, and "4:15:26 AM" requires a greater display width than the short time format ("t") of "4:15 AM". Depending on the platform, this difference might cause the [[TimePicker (Controls)|TimePicker]] view to change width in layout, or for the display to be truncated.

> [!TIP]
> It's best to use the default `HorizontalOptions` setting of `Fill` with [[TimePicker (Controls)|TimePicker]], and not to use a width of `Auto` when putting [[TimePicker (Controls)|TimePicker]] in a [[Grid (Controls)|Grid]] cell.

<!--
> [!TIP]
> On Android, the [[TimePicker (Controls)|TimePicker]] dialog can be customized by overriding the `CreateTimePickerDialog` method in a custom renderer. This allows, for example, additional buttons to be added to the dialog. -->

## Platform differences

This section describes the platform-specific differences with the [[TimePicker (Controls)|TimePicker]] control.
...
In .NET 10, [[TimePicker (Controls)|TimePicker]] supports a nullable selected time so you can represent “no time selected” and clear selection in bindings. The `Time` property is `TimeSpan?` with validation that any non-null value has hours < 24 and milliseconds >= 0.
