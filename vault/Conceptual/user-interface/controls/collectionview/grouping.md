...
tags:
  ...
  - area/user-interface
ms_date: "09/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/collectionview/grouping?view=net-maui-10.0"
---
...
The .NET Multi-platform App UI (.NET MAUI) [[CollectionView|CollectionView]] supports displaying grouped data, and defines the following properties that control how it will be presented:
...
- `GroupHeaderTemplate`, of type [[DataTemplate|DataTemplate]], the template to use for the header of each group.
- `GroupFooterTemplate`, of type [[DataTemplate|DataTemplate]], the template to use for the footer of each group.
...
The following screenshot shows a [[CollectionView|CollectionView]] displaying grouped data:

![](media/grouping/grouped-data.png)
...
[[CollectionView|CollectionView]] will display grouped data, provided that the data has been grouped correctly, by setting the `IsGrouped` property to `true`:
...
The appearance of each item in the [[CollectionView|CollectionView]] is defined by setting the `CollectionView.ItemTemplate` property to a [[DataTemplate|DataTemplate]]. For more information, see [[populate-data#define-item-appearance|Define item appearance]].

> [!NOTE]
> By default, [[CollectionView|CollectionView]] will display the group name in the group header and footer. This behavior can be changed by customizing the group header and group footer.

## Customize the group header

The appearance of each group header can be customized by setting the `CollectionView.GroupHeaderTemplate` property to a [[DataTemplate|DataTemplate]]:
...
![](media/grouping/customized-header.png)

## Customize the group footer

The appearance of each group footer can be customized by setting the `CollectionView.GroupFooterTemplate` property to a [[DataTemplate|DataTemplate]]:
...
In this example, each group footer is set to a [[Label (Controls)|Label]] that displays the number of items in the group. The following screenshot shows the customized group footer:

![](media/grouping/customized-footer.png)

## Empty groups

When a [[CollectionView|CollectionView]] displays grouped data, it will display any groups that are empty. Such groups will be displayed with a group header and footer, indicating that the group is empty. The following screenshot shows an empty group:

![](media/grouping/empty-group.png)

> [!NOTE]
> On iOS 10, group headers and footers for empty groups may all be displayed at the top of the [[CollectionView|CollectionView]].

## Group without templates

[[CollectionView|CollectionView]] can display correctly grouped data without setting the `CollectionView.ItemTemplate` property to a [[DataTemplate|DataTemplate]]:
