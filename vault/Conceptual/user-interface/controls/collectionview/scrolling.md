...
tags:
  ...
  - area/user-interface
ms_date: "09/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/collectionview/scrolling?view=net-maui-10.0"
---
...
[[CollectionView|CollectionView]] defines a [[ItemsView (Controls).ScrollToRequested|ScrollToRequested]] event that is fired when one of the `ScrollTo%2A` methods is invoked. The [[ScrollToRequestedEventArgs|ScrollToRequestedEventArgs]] object that accompanies the [[ItemsView (Controls).ScrollToRequested|ScrollToRequested]] event has many properties, including `IsAnimated`, `Index`, `Item`, and `ScrollToPosition`. These properties are set from the arguments specified in the `ScrollTo%2A` method calls.

In addition, [[CollectionView|CollectionView]] defines a [[ItemsView (Controls).Scrolled|Scrolled]] event that is fired to indicate that scrolling occurred. The [[ItemsViewScrolledEventArgs|ItemsViewScrolledEventArgs]] object that accompanies the [[ItemsView (Controls).Scrolled|Scrolled]] event has many properties. For more information, see [Detect scrolling](#detect-scrolling).

[[CollectionView|CollectionView]] also defines a [[ItemsView (Controls).ItemsUpdatingScrollMode|ItemsUpdatingScrollMode]] property that represents the scrolling behavior of the [[CollectionView|CollectionView]] when new items are added to it. For more information about this property, see [Control scroll position when new items are added](#control-scroll-position-when-new-items-are-added).
...
[[CollectionView|CollectionView]] can also load data incrementally as the user scrolls. For more information, see [[populate-data#load-data-incrementally|Load data incrementally]].

![[scrolling-tip]]

## Detect scrolling

[[CollectionView|CollectionView]] defines a [[ItemsView (Controls).Scrolled|Scrolled]] event which is fired to indicate that scrolling occurred. The [[ItemsViewScrolledEventArgs|ItemsViewScrolledEventArgs]] class, which represents the object that accompanies the [[ItemsView (Controls).Scrolled|Scrolled]] event, defines the following properties:
...
The following XAML example shows a [[CollectionView|CollectionView]] that sets an event handler for the [[ItemsView (Controls).Scrolled|Scrolled]] event:
...
In this code example, the `OnCollectionViewScrolled` event handler is executed when the [[ItemsView (Controls).Scrolled|Scrolled]] event fires:
...
> [!IMPORTANT]
> The [[ItemsView (Controls).Scrolled|Scrolled]] event is fired for user initiated scrolls, and for programmatic scrolls.

## Scroll an item at an index into view

One `ScrollTo%2A` method overload scrolls the item at the specified index into view. Given a [[CollectionView|CollectionView]] object named [[CollectionView|CollectionView]], the following example shows how to scroll the item at index 12 into view:
...
> [!NOTE]
> The [[ItemsView (Controls).ScrollToRequested|ScrollToRequested]] event is fired when the `ScrollTo%2A` method is invoked.

## Scroll an item into view

Another `ScrollTo%2A` method overload scrolls the specified item into view. Given a [[CollectionView|CollectionView]] object named [[CollectionView|CollectionView]], the following example shows how to scroll the Proboscis Monkey item into view:
...
A scrolling animation is displayed when scrolling an item into view. However, this animation can be disabled by setting the `animate` argument of the `ScrollTo%2A` method to `false`:
...
When scrolling an item into view, the exact position of the item after the scroll has completed can be specified with the `position` argument of the `ScrollTo%2A` methods. This argument accepts a [[ScrollToPosition|ScrollToPosition]] enumeration member.
...
![](media/scrolling/scrolltoposition-makevisible.png)

> [!NOTE]
> The `ScrollToPosition.MakeVisible` member is used by default, if the `position` argument is not specified when calling the `ScrollTo%2A` method.
...
![](media/scrolling/scrolltoposition-start.png)
...
![](media/scrolling/scrolltoposition-center.png)
...
![](media/scrolling/scrolltoposition-end.png)

## Control scroll position when new items are added

[[CollectionView|CollectionView]] defines a [[ItemsView (Controls).ItemsUpdatingScrollMode|ItemsUpdatingScrollMode]] property, which is backed by a bindable property. This property gets or sets a [[ItemsUpdatingScrollMode|ItemsUpdatingScrollMode]] enumeration value that represents the scrolling behavior of the [[CollectionView|CollectionView]] when new items are added to it. The [[ItemsUpdatingScrollMode|ItemsUpdatingScrollMode]] enumeration defines the following members:
...
The default value of the [[ItemsView (Controls).ItemsUpdatingScrollMode|ItemsUpdatingScrollMode]] property is `KeepItemsInView`. Therefore, when new items are added to a [[CollectionView|CollectionView]] the first item in the list will remain displayed. To ensure that the last item in the list is displayed when new items are added, set the [[ItemsView (Controls).ItemsUpdatingScrollMode|ItemsUpdatingScrollMode]] property to `KeepLastItemInView`:
...
[[CollectionView|CollectionView]] defines `HorizontalScrollBarVisibility` and `VerticalScrollBarVisibility` properties, which are backed by bindable properties. These properties get or set a [[ScrollBarVisibility|ScrollBarVisibility]] enumeration value that represents when the horizontal, or vertical, scroll bar is visible. The [[ScrollBarVisibility|ScrollBarVisibility]] enumeration defines the following members:
...
- `SnapPointsType`, of type [[SnapPointsType|SnapPointsType]], specifies the behavior of snap points when scrolling.
- `SnapPointsAlignment`, of type [[SnapPointsAlignment|SnapPointsAlignment]], specifies how snap points are aligned with items.
...
The [[SnapPointsType|SnapPointsType]] enumeration defines the following members:
...
![](media/scrolling/snappoints-none.png)

### Snap points alignment

The [[SnapPointsAlignment|SnapPointsAlignment]] enumeration defines `Start`, `Center`, and `End` members.
...
![](media/scrolling/snappoints-start.png)
...
![](media/scrolling/snappoints-center.png)
...
![](media/scrolling/snappoints-end.png)
