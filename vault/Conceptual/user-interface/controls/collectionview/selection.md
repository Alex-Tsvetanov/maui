...
tags:
  ...
  - area/user-interface
ms_date: "09/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/user-interface/controls/collectionview/selection?view=net-maui-10.0"
---
...
The .NET Multi-platform App UI (.NET MAUI) [[CollectionView|CollectionView]] defines the following properties that control item selection:
...
All of these properties are backed by [[BindableProperty|BindableProperty]] objects, which means that the properties can be targets of data bindings.

By default, [[CollectionView|CollectionView]] selection is disabled. However, this behavior can be changed by setting the `SelectionMode` property value to one of the `SelectionMode` enumeration members:
...
In addition, [[CollectionView|CollectionView]] has a `UpdateSelectedItems` method that updates the `SelectedItems` property with a list of selected items, while only firing a single change notification.

## Single selection

When the `SelectionMode` property is set to `Single`, a single item in the [[CollectionView|CollectionView]] can be selected. When an item is selected, the `SelectedItem` property is set to the value of the selected item. When this property changes, the `SelectionChangedCommand` is executed (with the value of the `SelectionChangedCommandParameter` being passed to the `ICommand`), and the `SelectionChanged` event fires.

The following XAML example shows a [[CollectionView|CollectionView]] that can respond to single item selection:
...
The following screenshot shows single item selection in a [[CollectionView|CollectionView]]:

![](media/selection/single-selection.png)

## Multiple selection

When the `SelectionMode` property is set to `Multiple`, multiple items in the [[CollectionView|CollectionView]] can be selected. When items are selected, the `SelectedItems` property is set to the selected items. When this property changes, the `SelectionChangedCommand` is executed (with the value of the `SelectionChangedCommandParameter` being passed to the `ICommand`, and the `SelectionChanged` event fires.

The following XAML example shows a [[CollectionView|CollectionView]] that can respond to multiple item selection:
...
The following screenshot shows multiple item selection in a [[CollectionView|CollectionView]]:

![](media/selection/multiple-selection.png)

## Single preselection

When the `SelectionMode` property is set to `Single`, a single item in the [[CollectionView|CollectionView]] can be preselected by setting the `SelectedItem` property to the item. The following XAML example shows a [[CollectionView|CollectionView]] that preselects a single item:
...
Therefore, when the [[CollectionView|CollectionView]] appears, the fourth item in the list is preselected:

![](media/selection/single-pre-selection.png)

## Multiple preselection

When the `SelectionMode` property is set to `Multiple`, multiple items in the [[CollectionView|CollectionView]] can be preselected. The following XAML example shows a [[CollectionView|CollectionView]] that enables the preselection of multiple items:
...
Therefore, when the [[CollectionView|CollectionView]] appears, the second, fourth, and fifth items in the list are preselected:

![](media/selection/multiple-pre-selection.png)
...
[[CollectionView|CollectionView]] has a `Selected` [[VisualState|VisualState]] that can be used to initiate a visual change to the selected item in the [[CollectionView|CollectionView]]. A common use case for this [[VisualState|VisualState]] is to change the background color of the selected item, which is shown in the following XAML example:
...
> [!IMPORTANT]
> The [[Style|Style]] that contains the `Selected` [[VisualState|VisualState]] must have a `TargetType` property value that's the type of the root element of the [[DataTemplate|DataTemplate]], which is set as the `ItemTemplate` property value.
...
In this example, the `Style.TargetType` property value is set to [[Grid (Controls)|Grid]] because the root element of the `ItemTemplate` is a [[Grid (Controls)|Grid]]. The `Selected` [[VisualState|VisualState]] specifies that when an item in the [[CollectionView|CollectionView]] is selected, the `BackgroundColor` of the item is set to `LightSkyBlue`:

![](media/selection/single-selection-color.png)

For more information about visual states, see [[visual-states|Visual states]].

## Disable selection

[[CollectionView|CollectionView]] selection is disabled by default. However, if a [[CollectionView|CollectionView]] has selection enabled, it can be disabled by setting the `SelectionMode` property to `None`:
...
When the `SelectionMode` property is set to `None`, items in the [[CollectionView|CollectionView]] can't be selected, the `SelectedItem` property remains `null`, and the `SelectionChanged` event isn't fired.
