...
tags:
  ...
  - area/windows
ms_date: "04/06/2022"
source: "https://learn.microsoft.com/dotnet/maui/windows/platform-specifics/visualelement-access-keys?view=net-maui-10.0"
---
...
- `Top` – indicates that the access key tip will appear above the top edge of the [[VisualElement (Controls)|VisualElement]].
- `Bottom` – indicates that the access key tip will appear below the lower edge of the [[VisualElement (Controls)|VisualElement]].
- `Right` – indicates that the access key tip will appear to the right of the right edge of the [[VisualElement (Controls)|VisualElement]].
- `Left` – indicates that the access key tip will appear to the left of the left edge of the [[VisualElement (Controls)|VisualElement]].
- `Center` – indicates that the access key tip will appear overlaid on the center of the [[VisualElement (Controls)|VisualElement]].
...
The result is that access key tips can be displayed next to any [[VisualElement (Controls)|VisualElement]] instances that define access keys, by pressing the Alt key:

![](media/visualelement-access-keys/visualelement-accesskeys.png )

When a user activates an access key, by pressing the Alt key followed by the access key, the default action for the [[VisualElement (Controls)|VisualElement]] will be executed. For example, when a user activates the access key on a [[Switch (Controls)|Switch]], the [[Switch (Controls)|Switch]] is toggled. When a user activates the access key on an [[Entry (Controls)|Entry]], the [[Entry (Controls)|Entry]] gains focus. When a user activates the access key on a [[Button (Controls)|Button]], the event handler for the `Clicked` event is executed.
