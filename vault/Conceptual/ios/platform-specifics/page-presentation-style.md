...
tags:
  ...
  - area/ios
ms_date: "03/14/2025"
source: "https://learn.microsoft.com/dotnet/maui/ios/platform-specifics/page-presentation-style?view=net-maui-10.0"
---
...
In addition, the `GetModalPresentationStyle` method can be used to retrieve the current value of the `UIModalPresentationStyle` enumeration that's applied to the [[Page (Controls)|Page]].

The result is that the modal presentation style on a [[Page (Controls)|Page]] can be set:

![](media/page-presentation-style/modal-presentation-style.png)

> [!NOTE]
> Pages that use this platform-specific to set the modal presentation style must use modal navigation. For more information, see [[navigationpage#perform-modal-navigation|Perform modal navigation]].
