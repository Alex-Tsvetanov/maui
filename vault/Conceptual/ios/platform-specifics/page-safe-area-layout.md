...
tags:
  ...
  - area/ios
ms_date: "08/18/2025"
source: "https://learn.microsoft.com/dotnet/maui/ios/platform-specifics/page-safe-area-layout?view=net-maui-10.0"
---
...
> [!IMPORTANT]
> In .NET 10, use the [[ContentPage.SafeAreaEdges|SafeAreaEdges]] property (also available on [[Layout (Controls)|Layout]], [[ScrollView (Controls)|ScrollView]], [[ContentView (Controls)|ContentView]], and [[Border|Border]]) to precisely control safe area behavior. The iOS-specific `Page.UseSafeArea` remains for existing apps, but new code should prefer `SafeAreaEdges`.

## Modern safe area guidance (.NET 10+)

### Choose the safe area behavior with SafeAreaEdges

`SafeAreaEdges` accepts the following values from [[SafeAreaEdges|SafeAreaEdges]]: `Default`, `None`, `Container`, `SoftInput`, and `All`.
...
> [!NOTE]
> The [[Layout (Controls)|Layout]] class defines a [[Layout (Controls).IgnoreSafeArea|IgnoreSafeArea]] property that ensures that content is positioned on an area of the screen that is safe for all iOS devices. This property can be set to `true` on any layout class, such as a [[Grid (Controls)|Grid]] or [[StackLayout (Controls)|StackLayout]], to perform the equivalent of this platform-specific.
