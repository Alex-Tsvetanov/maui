...
Provided that large titles are enabled on the [[NavigationPage (Controls)|NavigationPage]], all pages in the navigation stack will display large titles. This behavior can be overridden on pages by setting the `Page.LargeTitleDisplay` attached property to a value of the `LargeTitleDisplayMode` enumeration:
...
The result is that a specified `LargeTitleDisplayMode` is applied to the [[Page (Controls)|Page]], which controls the large title behavior:

![](media/page-large-title/large-title.png)
