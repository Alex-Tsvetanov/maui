...
> [!IMPORTANT]
> The private key and certificate that make up your signing identity will also be exported to **Keychain Access** on your Mac build host, provided that the IDE is paired to it. For more information, see [[pair-to-mac|Pair to Mac for iOS development]].
...
The recommended approach is to create a *wildcard App ID*, unless your app uses app-specific capabilities. For more information about capabilities, see [[capabilities|Capabilities]].
...
1. In the **Register an App ID** page, provide a **Description** and set the **Bundle ID** to **Wildcard**. Then, enter an App ID in the format `com.domainname.*` and click the **Continue** button:
    ...
    ![](media/manual-provisioning/register-app-id.png)
...
1. In the **Register a New Provisioning Profile** page, in the **Development** section, select **iOS App Development** and click the **Continue** button:
    ...
    ![](media/manual-provisioning/provisioning-profile-ios.png)

1. In the **Generate a Provisioning Profile** page, select the **App ID** to use and then click the **Continue** button.

    ![](media/manual-provisioning/provisioning-profile-appid.png)
...
1. In the **Generate a Provisioning Profile** page, select the certificates to include in the provisioning profile and then click the **Continue** button.
    ...
    ![](media/manual-provisioning/provisioning-profile-certificates.png)
...
The provisioning profiles will be downloaded on Windows, and exported to your Mac build host if the IDE is paired to it. For more information, see [[pair-to-mac|Pair to Mac for iOS development]].
...
1. In the project properties, navigate to the **iOS Bundle Signing** tab and ensure that **Manual Provisioning** is selected in the **Scheme** drop-down:
    ...
    ![](media/manual-provisioning/vs/bundle-signing.png)
...
Apple provides a selection of application services, known as capabilities, that can be activated for a .NET MAUI iOS app. These capabilities must be configured in both your provisioning profile, when the **App ID** is created, and in the *Entitlements.plist* file that should be added to the .NET MAUI app project. For more information about capabilities, see [[entitlements|Entitlements]] and [[capabilities|Capabilities]].
