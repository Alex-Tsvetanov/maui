...
tags:
  ...
  - area/migration
ms_date: "10/31/2024"
source: "https://learn.microsoft.com/dotnet/maui/migration/renderer-to-handler?view=net-maui-10.0"
---
...
Alternatively, custom renderers that customize Xamarin.Forms controls can be converted so that they modify .NET MAUI handlers. For more information, see [[customize|Customize controls with handlers]].
...
The following code example shows the `CustomEntryHandler` class extended with the [[PropertyMapper{TVirtualView}|PropertyMapper]] definition:
...
- `DisconnectHandler%2A`, which should perform any native view cleanup, such as unsubscribing from events and disposing objects. This method is intentionally not invoked by .NET MAUI. Instead, you must invoke it yourself from a suitable location in your app's lifecycle. For more information, see [Native view cleanup](#native-view-cleanup).



- `CreatePlatformView%2A`, which should create and return the native view that implements the cross-platform control.
- `ConnectHandler%2A`, which should perform any native view setup, such as initializing the native view and performing event subscriptions.
- `DisconnectHandler%2A`, which should perform any native view cleanup, such as unsubscribing from events and disposing objects. This method is automatically invoked by .NET MAUI by default, although this behavior can be changed. For more information, see [Control handler disconnection](#control-handler-disconnection).


> [!NOTE]
> The `CreatePlatformView%2A`, `ConnectHandler%2A`, and `DisconnectHandler%2A` overrides are the replacements for the `OnElementChanged` method in a Xamarin.Forms custom renderer.
...
`CustomEntryHandler` derives from the `ViewHandler`2` class, with the generic `CustomEntry` argument specifying the cross-platform control type, and the `AppCompatEditText` argument specifying the type of native control.

The `CreatePlatformView%2A` override creates and returns an `AppCompatEditText` object. The `ConnectHandler%2A` override is the location to perform any required native view setup. The `DisconnectHandler%2A` override is the location to perform any native view cleanup, and so calls the `Dispose` method on the `AppCompatEditText` instance.
...
`CustomEntryHandler` derives from the `ViewHandler`2` class, with the generic `CustomEntry` argument specifying the cross-platform control type, and the `MauiAppCompatEditText` argument specifying the type of native control.

The `CreatePlatformView%2A` override creates and returns an `MauiAppCompatEditText` object. The `ConnectHandler%2A` override is the location to perform any required native view setup. The `DisconnectHandler%2A` override is the location to perform any native view cleanup, and so calls the `Dispose` method on the `MauiAppCompatEditText` instance.
...
- [[create|Create a custom control using handler]]
