...
tags:
  ...
  - area/migration
ms_date: "02/15/2023"
source: "https://learn.microsoft.com/dotnet/maui/migration/effects?view=net-maui-10.0"
---
...
If you're using a .NET MAUI multi-targeted project, your effect code should be combined into a single file and placed outside the *Platforms* folder. This requires you to combine your [[RoutingEffect|RoutingEffect]] implementation and [[PlatformEffect|PlatformEffect]] implementations into a single file, using conditional compilation around platform code. However, if your solution has separate projects per-platform, then you should move the platform-specific effect files into the corresponding projects.

In .NET MAUI, the [[RoutingEffect|RoutingEffect]] class is in the `Microsoft.Maui.Controls` namespace. This namespace is one of .NET MAUI's implicit `global using` directives, and so you don't need to add a `using` directive for it. However, the [[PlatformEffect|PlatformEffect]] class is in the `Microsoft.Maui.Controls.Platform` namespace, for which you must add a `using` directive.
...
In your .NET MAUI app project, open *MauiProgram.cs* and call the `ConfigureEffects%2A` method on the [[MauiAppBuilder|MauiAppBuilder]] object in the `CreateMauiApp` method:
...
The effect is registered with the `ConfigureEffects%2A` method, whose `configureDelegate` registers the [[PlatformEffect|PlatformEffect]] implementation against its [[RoutingEffect|RoutingEffect]] implementation.

## Consume the effect

The effect can be consumed in a .NET MAUI app by adding it to the [[Element.Effects|Effects]] collection of a control:
