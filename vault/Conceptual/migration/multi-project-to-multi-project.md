...
tags:
  ...
  - area/migration
ms_date: "3/02/2023"
source: "https://learn.microsoft.com/dotnet/maui/migration/multi-project-to-multi-project?view=net-maui-10.0"
---
...
This article describes how to manually migrate a Xamarin.Forms library project to a .NET MAUI library project. Before you do this, you must update your Xamarin.Forms platform projects to be SDK-style projects. SDK-style projects are the same project format used by all .NET workloads, and compared to many Xamarin projects are much less verbose. For information about updating your app projects, see [[native-projects|Upgrade Xamarin.Android, Xamarin.iOS, and Xamarin.Mac projects to .NET]], [[android-projects|Xamarin.Android project migration]], [[apple-projects|Xamarin Apple project migration]], and [[uwp-projects|Xamarin.Forms UWP project migration]].
...
.NET MAUI apps have a single cross-platform app entry point. Each platform entry point calls a `CreateMauiApp` method on the static `MauiProgram` class, and returns a [[MauiApp|MauiApp]].
...
If there are platform specific services that need to be migrated to .NET MAUI, use the `Type)` method to add a transient service of the specified type to the specified `IServiceCollection`.
