...
tags:
  ...
  - area/migration
ms_date: "04/13/2023"
source: "https://learn.microsoft.com/dotnet/maui/migration/custom-renderers?view=net-maui-10.0"
---
...
> [!NOTE]
> An alternative to using a Xamarin.Forms custom renderer in .NET MAUI is to migrate the custom renderer to a .NET MAUI handler. For more information, see [[renderer-to-handler|Migrate a Xamarin.Forms custom renderer to a .NET MAUI handler]].
...
The renderers are registered with the `ConfigureMauiHandlers%2A` and `AddHandler%2A` method. This first argument to the `AddHandler%2A` method is the cross-platform control type, with the second argument being its renderer type.

> [!IMPORTANT]
> Only renderers that derive from `FrameRenderer`, `ListViewRenderer`, `NavigationRenderer` on iOS, `ShellRenderer` on iOS and Android, `TabbedRenderer` on iOS, `TableViewRenderer`, and `VisualElementRenderer` can be registered with the `AddHandler%2A` method.
