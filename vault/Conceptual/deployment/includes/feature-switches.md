...
tags:
  ...
  - area/deployment
ms_date: "11/14/2024"
source: "https://learn.microsoft.com/dotnet/maui/deployment/includes/feature-switches?view=net-maui-10.0"
---
...
| `MauiShellSearchResultsRendererDisplayMemberNameSupported` | When set to `false`, the value of `SearchHandler.DisplayMemberName` will be ignored. Instead, you should provide an [[ItemsView{TVisual}.ItemTemplate|ItemTemplate]] to define the appearance of [[SearchHandler|SearchHandler]] results. By default, this build property is set to `false` when full trimming or Native AOT is enabled.|
| `MauiQueryPropertyAttributeSupport` | When set to `false`, `[QueryProperty(...)]` attributes won't be used to set property values when navigating. Instead, you should implement the [[IQueryAttributable|IQueryAttributable]] interface to accept query parameters. By default, this build property is set to `false` when full trimming or Native AOT is enabled. |
...
| `MauiHybridWebViewSupported` | When set to `false`, the [[HybridWebView|HybridWebView]] control won't be available. By default, this build property is set to `false` when full trimming or Native AOT is enabled. |
