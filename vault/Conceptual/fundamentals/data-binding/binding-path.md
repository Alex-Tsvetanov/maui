...
tags:
  ...
  - area/fundamentals
ms_date: "02/27/2025"
source: "https://learn.microsoft.com/dotnet/maui/fundamentals/data-binding/binding-path?view=net-maui-10.0"
---
...
For example, suppose a page contains a [[TimePicker (Controls)|TimePicker]]:

```xaml
<TimePicker x:Name="timePicker">
```

The `Time` property of [[TimePicker (Controls)|TimePicker]] is of type `TimeSpan`, and it has a `TotalSeconds` property. A data binding can be created that references the `TotalSeconds` property of that `TimeSpan` value:
...
In the second [[Label (Controls)|Label]], the binding source is the page itself. The `Content` property is of type [[StackLayout (Controls)|StackLayout]], which has a `Children` property of type `IList<View>`, which has a `Count` property indicating the number of children.

## Paths with indexers

In the example above, the binding in the third [[Label (Controls)|Label]] references the `CultureInfo` class in the `System.Globalization` namespace:
...
The fourth [[Label (Controls)|Label]] does something similar but for the culture associated with France. The `Source` property of the binding is set to `CultureInfo` object with a constructor:
...
For more information about specifying constructor arguments in XAML, see [[pass-arguments#pass-constructor-arguments|Pass constructor arguments]].

The last [[Label (Controls)|Label]] references the second [[Label (Controls)|Label]] on the page:
...
[[Label (Controls)|Label]] has a `Text` property of type `String`, which has a `Length` property. The second [[Label (Controls)|Label]] reports the `TimeSpan` set in the [[TimePicker (Controls)|TimePicker]], so when that text changes, the final [[Label (Controls)|Label]] changes as well:

![](media/binding-path/pathvariations.png)
...
That displays the type of the binding source, or `DataBindingDemos.PathVariationsPage`. You know `PathVariationsPage` derives from [[ContentPage|ContentPage]], so it has a `Content` property:
