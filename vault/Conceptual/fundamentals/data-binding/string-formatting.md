...
tags:
  ...
  - area/fundamentals
ms_date: "02/27/2025"
source: "https://learn.microsoft.com/dotnet/maui/fundamentals/data-binding/string-formatting?view=net-maui-10.0"
---
...
In a .NET Multi-platform App UI (.NET MAUI) app, it's sometimes convenient to use data bindings to display the string representation of an object or value. For example, you might want to use a [[Label (Controls)|Label]] to display the current value of a [[Slider (Controls)|Slider]]. In this data binding, the [[Slider (Controls)|Slider]] is the source, and the target is the `Text` property of the [[Label (Controls)|Label]].
...
In this example, the bindings on the [[Slider (Controls)|Slider]] and [[TimePicker (Controls)|TimePicker]] show the use of format specifications particular to `double` and `TimeSpan` data types. The `StringFormat` that displays the text from the [[Entry (Controls)|Entry]] view demonstrates how to specify double quotation marks in the formatting string with the use of the `&quot;` HTML entity.

The next section in the XAML file is a [[StackLayout (Controls)|StackLayout]] with a `BindingContext` set to an `x:Static` markup extension that references the static `DateTime.Now` property. The first binding has no properties:
...
![](media/string-formatting/stringformatting.png)

## ViewModels and string formatting

When you're using [[Label (Controls)|Label]] and `StringFormat` to display the value of a view that is also the target of a viewmodel, you can either define the binding from the view to the [[Label (Controls)|Label]] or from the viewmodel to the [[Label (Controls)|Label]]. In general, the second approach is best because it verifies that the bindings between the view and viewmodel are working.
...
In this example, there are three pairs of [[Slider (Controls)|Slider]] and [[Label (Controls)|Label]] elements that are bound to the same source property in the `HslColorViewModel` object. Each [[Label (Controls)|Label]] that accompanies a [[Slider (Controls)|Slider]] has a `StringFormat` property to display each [[Slider (Controls)|Slider]] value:

![](media/string-formatting/bettercolorselector.png)
