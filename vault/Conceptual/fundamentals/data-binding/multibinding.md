...
tags:
  ...
  - area/fundamentals
ms_date: "02/27/2025"
source: "https://learn.microsoft.com/dotnet/maui/fundamentals/data-binding/multibinding?view=net-maui-10.0"
---
...
A `MultiBinding` must use a `IMultiValueConverter` to produce a value for the binding target, based on the value of the bindings in the `Bindings` collection. For example, a [[Color|Color]] might be computed from red, blue, and green values, which can be values from the same or different binding source objects. When a value moves from the target to the sources, the target property value is translated to a set of values that are fed back into the bindings.
...
A `IMultiValueConverter` is typically consumed by instantiating it in a resource dictionary, and then referencing it using the [[StaticResourceExtension|`StaticResource`]] markup extension to set the `MultiBinding.Converter` property:
...
By default, the `CheckBox.IsChecked` property uses a `TwoWay` binding. Therefore, the `ConvertBack` method of the `AllTrueMultiConverter` instance is executed when the [[CheckBox|CheckBox]] is unchecked by the user, which sets the source binding values to the value of the `CheckBox.IsChecked` property.
...
In this example, the `StringFormat` property combines the three bound values into a single string that's displayed by the [[Label (Controls)|Label]].
...
For more information about string formatting in .NET MAUI, see [[string-formatting|String formatting]].
...
For more information about binding fallbacks, see [[binding-fallbacks|Binding fallbacks]].
