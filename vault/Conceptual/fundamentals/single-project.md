...
tags:
  ...
  - area/fundamentals
ms_date: "05/17/2022"
source: "https://learn.microsoft.com/dotnet/maui/fundamentals/single-project?view=net-maui-10.0"
---
...
![](media/single-project/resources.png)
...
For more information, see [[images|Add images to a .NET MAUI app project]].
...
At build time, raw assets are copied to your app package. For information about disabling asset packaging, see [[troubleshooting#disable-asset-file-packaging|Disable asset file packaging]].
...
CSS files must be loaded by the `StyleSheet` class before being added to a [[ResourceDictionary|ResourceDictionary]]:
...
For more information, see [[css|Style apps with CSS]].
...
![](media/single-project/manifest.png)
...
![](media/single-project/platform-folders.png)

The folders for each platform contain platform-specific resources, and code that starts the app on each platform:

![](media/single-project/platform-code.png)

At build time, the build system only includes the code from each folder when building for that specific platform. For example, when you build for Android the files in the *Platforms\Android* folder will be built into the app package, but the files in the other *Platforms* folders won't be. This approach uses multi-targeting to target multiple platforms from a single project. Multi-targeting can be combined with partial classes and partial methods to invoke native platform functionality from cross-platform code. For more information, see [[invoke-platform-code|Invoke platform code]].
...
In the preceding example, the `MainPage` property is set to the `AppShell` object. `AppShell` is a subclassed [[Shell|Shell]] class that describes the visual hierarchy of the app. For more information, see [[create|Create a .NET MAUI Shell app]].
...
In the preceding example, a new [[Window|Window]] is created whose initial content is set to the `AppShell` object. `AppShell` is a subclassed [[Shell|Shell]] class that describes the visual hierarchy of the app. For more information, see [[create|Create a .NET MAUI Shell app]].
