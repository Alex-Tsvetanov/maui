...
tags:
  ...
  - area/fundamentals
ms_date: "11/29/2024"
source: "https://learn.microsoft.com/dotnet/maui/fundamentals/behaviors?view=net-maui-10.0"
---
...
.NET Multi-platform App UI (.NET MAUI) behaviors let you add extra functionality to user interface controls, such as [[Button (Controls)|Button]] or [[Entry (Controls)|Entry]], without having to subclass the controls. Instead, the extra functionality is implemented in a [[Behavior{T}|Behavior]] class and attached to the control.

Behaviors can be used to add different functionality to UI controls, such as:

- Input validations. Behaviors can be used with input controls to ensure the data is valid. For example, you might write a behavior for an [[Entry (Controls)|Entry]] to ensure that a phone number of email address is in the correct format.
- Enhancing the UI. Behaviors can be used to enhance UI controls in your app. For example, you might write a behavior for a [[Button (Controls)|Button]] to make it shake when it's clicked, or change color.
- Runtime responses. Behaviors can be used to make controls react to different situations. For example, you might automatically save user input in an [[Entry (Controls)|Entry]] when a user finishes typing.
...
- .NET MAUI behaviors are classes that derive from the [[Behavior{T}|Behavior]] or `Behavior`1` class, where `T` is the type of the control to which the behavior should apply. For more information, see [.NET MAUI Behaviors](#net-maui-behaviors).
- Platform behaviors are classes that derive from the `PlatformBehavior`1` or `PlatformBehavior`2` class. These behaviors can respond to arbitrary conditions and events on a native control. For more information, see [Platform behaviors](#platform-behaviors).

## Attached behaviors

Attached behaviors are static classes with one or more attached properties. An attached property is a special type of bindable property. They are defined in one class but attached to other objects, and they are recognizable in XAML as attributes that contain a class and a property name separated by a period. For more information about attached properties, see [[attached-properties|Attached properties]].
...
1. The `propertyChanged` delegate casts the control reference, which is received as a [[BindableObject|BindableObject]], to the control type that the behavior is designed to enhance.
...
The following example shows the `AttachedNumericValidationBehavior` class, which highlights the value entered by the user into an [[Entry (Controls)|Entry]] control in red if it's not a `double`:
...
The equivalent [[Entry (Controls)|Entry]] in C# is shown in the following code example:
...
.NET MAUI behaviors are created by deriving from the [[Behavior{T}|Behavior]] or `Behavior`1` class.

The process for creating a .NET MAUI behavior is as follows:

1. Create a class that inherits from the [[Behavior{T}|Behavior]] or `Behavior`1` class, where `T` is the type of the control to which the behavior should apply.
1. Override the `OnAttachedTo%2A` method to perform any required setup.
1. Override the `OnDetachingFrom%2A` method to perform any required cleanup.
...
The `OnAttachedTo%2A` method is called immediately after the behavior is attached to a control. This method receives a reference to the control to which it is attached, and can be used to register event handlers or perform other setup that's required to support the behavior functionality. For example, you could subscribe to an event on a control. The behavior functionality would then be implemented in the event handler for the event.

The `OnDetachingFrom%2A` method is called when the behavior is removed from the control. This method receives a reference to the control to which it is attached, and is used to perform any required cleanup. For example, you could unsubscribe from an event on a control to prevent memory leaks.

The behavior can then be consumed by attaching it to the [[VisualElement (Controls).Behaviors|Behaviors]] collection of the control.

### Create a .NET MAUI Behavior

A .NET MAUI behavior can be implemented by creating a class that derives from the [[Behavior{T}|Behavior]] or `Behavior`1` class, and overriding the `OnAttachedTo%2A` and `OnDetachingFrom%2A` methods.

The following example shows the `NumericValidationBehavior` class, which highlights the value entered by the user into an [[Entry (Controls)|Entry]] control in red if it's not a `double`:
...
In this example, the `NumericValidationBehavior` class derives from the `Behavior`1` class, where `T` is an [[Entry (Controls)|Entry]]. The `OnAttachedTo%2A` method registers an event handler for the `TextChanged` event, with the `OnDetachingFrom%2A` method de-registering the `TextChanged` event to prevent memory leaks. The core functionality of the behavior is provided by the `OnEntryTextChanged` method, which parses the value entered in the [[Entry (Controls)|Entry]] and sets the `TextColor` property to red if the value isn't a `double`.
...
Every .NET MAUI control has a [[VisualElement (Controls).Behaviors|Behaviors]] collection, to which one or more behaviors can be added:
...
The following code example shows an *explicit* style for the `NumericValidationStyleBehavior` that uses the `AttachBehavior` attached property, and which can be applied to [[Entry (Controls)|Entry]] controls:
...
The [[Style|Style]] can be applied to an [[Entry (Controls)|Entry]] by setting its `Style` property to the style using the [[StaticResourceExtension|`StaticResource`]] markup extension:
...
> [!NOTE]
> While you can add bindable properties to a behavior that is set or queried in XAML, if you do create behaviors that have state they should not be shared between controls in a [[Style|Style]] in a [[ResourceDictionary|ResourceDictionary]].

### Remove a .NET MAUI behavior

The `OnDetachingFrom%2A` method is called when a behavior is removed from a control, and is used to perform any required cleanup such as unsubscribing from an event to prevent a memory leak. However, behaviors are not implicitly removed from controls unless the control's [[VisualElement (Controls).Behaviors|Behaviors]] collection is modified by the `Remove` or `Clear` method:
...
Alternatively, the control's [[VisualElement (Controls).Behaviors|Behaviors]] collection can be cleared:
...
Platform behaviors are created by deriving from the `PlatformBehavior`1` or `PlatformBehavior`2` class. They respond to arbitrary conditions and events on a native control.
...
1. In each native partial class that you require to implement the platform behavior you should:
    1. Override the `OnAttachedTo%2A` method to perform any setup.
    1. Override the `OnDetachedFrom%2A` method to perform any cleanup.
...
![](media/behaviors/platform-behavior-native-partial-classes.png)

Alternatively you could configure your project to support filename-based multi-targeting, or folder-based multi-targeting, or both. For more information about multi-targeting, see [[configure-multi-targeting|Configure multi-targeting]].

The native partial classes should inherit from the `PlatformBehavior`1` class or the `PlatformBehavior`2` class, where `TView` is the cross-platform control to which the behavior should apply, and `TPlatformView` is the native view that implements the cross-platform control on a particular platform. In each native partial class that you require to implement the platform behavior, you should override the `OnAttachedTo%2A` method and the `OnDetachedFrom%2A` method, and implement the core functionality of the platform behavior.
...
In this example, the `TintColorBehavior` class derives from the `PlatformBehavior`2` class, where `TView` is an [[Image (Controls)|Image]] and `TPlatformView` is an `ImageView`. The `OnAttachedTo%2A` applies the tint color to the image, provided that the `TintColor` property has a value. The `OnDetachedFrom%2A` method removes the tint color from the image.
...
Every .NET MAUI control has a [[VisualElement (Controls).Behaviors|Behaviors]] collection, to which one or more platform behaviors can be added:
...
The equivalent [[Image (Controls)|Image]] in C# is shown in the following example:
...
![](media/behaviors/platform-behavior.png)
