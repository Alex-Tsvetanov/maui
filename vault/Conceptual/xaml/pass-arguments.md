...
tags:
  ...
  - area/xaml
ms_date: "01/24/2022"
source: "https://learn.microsoft.com/dotnet/maui/xaml/pass-arguments?view=net-maui-10.0"
---
...
The following example demonstrates using the `x:Arguments` attribute with three different [[Color|Color]] constructors:
...
The number of elements within the `x:Arguments` tag, and the types of these elements, must match one of the [[Color|Color]] constructors. The [[Color|Color]] constructor with a single parameter requires a grayscale `float` value from 0 (black) to 1 (white). The [[Color|Color]] constructor with three parameters requires `float` red, green, and blue values ranging from 0 to 1. The [[Color|Color]] constructor with four parameters adds a `float` alpha channel as the fourth parameter.
...
The [[Color|Color]] class defines a number of factory methods, and the following example demonstrates calling three of them:
...
For more information about generic type arguments, see [[generics|Generics in XAML]].
