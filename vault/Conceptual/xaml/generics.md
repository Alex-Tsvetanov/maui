...
tags:
  ...
  - area/xaml
ms_date: "01/27/2025"
source: "https://learn.microsoft.com/dotnet/maui/xaml/generics?view=net-maui-10.0"
---
...
> [!NOTE]
> The `x:Type` markup extension supplies a Common Language Runtime (CLR) type reference for a generic type, and has a similar function to the `typeof` operator in C#. For more information, see [[consume#xtype-markup-extension|x:Type markup extension]].

For information about specifying generic types in .NET MAUI XAML, with the `x:DataType` and `x:Type` directives, see [[compiled-bindings#compile-bindings-that-specify-a-generic-type|Compile bindings that specify a generic type]] and [[consume#xtype-markup-extension|x:Type markup extension]].
...
In this example, `GenericsDemo.Models` is defined as the `models` XAML namespace, and `System.Collections.Generic` is defined as the `scg` XAML namespace. The `CollectionView.ItemsSource` property is set to a `List<T>` that's instantiated with a `Monkey` type argument. The `List<Monkey>` collection is initialized with multiple `Monkey` items, and a [[DataTemplate|DataTemplate]] that defines the appearance of each `Monkey` object is set as the `ItemTemplate` of the [[CollectionView|CollectionView]].
