...
tags:
  ...
  - area/xaml
ms_date: "07/15/2025"
source: "https://learn.microsoft.com/dotnet/maui/xaml/namespaces?view=net-maui-10.0"
---
...
The default namespace specifies that elements defined within the XAML file with no prefix refer to .NET Multi-platform App UI (.NET MAUI) classes, such as [[ContentPage|ContentPage]], [[Label (Controls)|Label]], and [[Button (Controls)|Button]].
...
|`x:Key`|Specifies a unique user-defined key for each resource in a [[ResourceDictionary|ResourceDictionary]]. The key's value is used to retrieve the XAML resource, and is typically used as the argument for the [[StaticResourceExtension|`StaticResource`]] markup extension.|
...
For more information about the `x:ClassModifier` attribute, see [[class-modifiers|Class modifiers]]. For more information about the `x:DataType` attribute, see [[compiled-bindings|Compiled bindings]]. For more information about the `x:FieldModifier` attribute, see [[field-modifiers|Field modifiers]]. For more information about the `x:Arguments` and `x:FactoryMethod` attributes, see [[pass-arguments|Pass arguments]]. For more information about the `x:TypeArguments` attribute, see [[generics|Generics]].

> [!NOTE]
> In addition to the constructs listed above, .NET MAUI also includes markup extensions that can be consumed through the `x` namespace prefix. For more information, see [[consume|Consume XAML Markup Extensions]].
...
For information about defining a custom namespace schema, see [[custom-namespace-schemas|Custom namespace schemas]].
