...
tags:
  ...
  - area/xaml
ms_date: "08/19/2025"
source: "https://learn.microsoft.com/dotnet/maui/xaml/runtime-load?view=net-maui-10.0"
---

# Load XAML at runtime

When a .NET Multi-platform App UI (.NET MAUI) XAML class is constructed, a `LoadFromXaml%2A` method is indirectly called. This occurs because the code-behind file for a XAML class calls the `InitializeComponent` method from its constructor:
...
> [!WARNING]
> Loading XAML at runtime isn't trim safe and shouldn't be used with full trimming or NativeAOT. It can be made trim safe by annotating all types that could be loaded at runtime with the `DynamicallyAccessedMembers` attribute or the `DynamicDependency` attribute. However, this is very error prone and isn't recommended. In addition, loading XAML at runtime has a significant performance cost. For more information, see [[trimming|Trim a .NET MAUI app]] and [[nativeaot|Native AOT deployment]].
...
In this example, a [[Button (Controls)|Button]] instance is created, with its `Text` property value being set from the XAML defined in the `string`. The [[Button (Controls)|Button]] is then added to a [[StackLayout (Controls)|StackLayout]] that has been defined in the XAML for the page.

> [!NOTE]
> The `LoadFromXaml%2A` extension methods allow a generic type argument to be specified. However, it's rarely necessary to specify the type argument, as it will be inferred from the type of the instance it's operating on.

The `LoadFromXaml%2A` method can be used to inflate any XAML, with the following example inflating a [[ContentPage|ContentPage]] and then navigating to it:
...
Loading XAML at runtime with the `LoadFromXaml%2A` method does not allow strongly-typed access to the XAML elements that have specified runtime object names (using `x:Name`). However, these XAML elements can be retrieved using the `FindByName` method, and then accessed as required:
...
In this example, the XAML for a [[ContentPage|ContentPage]] is inflated. This XAML includes a [[Label (Controls)|Label]] named `monkeyName`, which is retrieved using the `FindByName` method, before its `Text` property is set.
