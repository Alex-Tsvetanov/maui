...
tags:
  ...
  - area/xaml
ms_date: "08/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/xaml/fundamentals/markup-extensions?view=net-maui-10.0"
---
...
XAML markup extensions are so named because they are backed by code in classes that implement [[IMarkupExtension{T}|IMarkupExtension]]. It's also possible to write your own custom markup extensions.
...
Some XAML pages contain several views with properties set to the same values. For example, many of the property settings for these [[Button (Controls)|Button]] objects are the same:
...
In XAML, one popular solution is to store such values or objects in a *resource dictionary*. The [[VisualElement (Controls)|VisualElement]] class defines a property named `Resources` of type [[ResourceDictionary|ResourceDictionary]], which is a dictionary with keys of type `string` and values of type `object`. You can put objects into this dictionary and then reference them from markup, all in XAML.
...
> [!NOTE]
> Optional [[ResourceDictionary|ResourceDictionary]] tags can be included as the child of the `Resources` tags.

The resources can then be consumed by the [[Button (Controls)|Button]] objects, by using the [[StaticResourceExtension|`StaticResource`]] XAML markup extension to set their `HorizontalOptions` and `VerticalOptions` properties:
...
The [[StaticResourceExtension|`StaticResource`]] markup extension is always delimited with curly braces, and includes the dictionary key. The name [[StaticResourceExtension|`StaticResource`]] distinguishes it from [[DynamicResourceExtension|`DynamicResource`]], which .NET MAUI also supports. [[DynamicResourceExtension|`DynamicResource`]] is for dictionary keys associated with values that might change at runtime, while [[StaticResourceExtension|`StaticResource`]] accesses elements from the dictionary just once when the elements on the page are constructed. Whenever the XAML parser encounters a [[StaticResourceExtension|`StaticResource`]] markup extension, it searches up the visual tree and uses the first [[ResourceDictionary|ResourceDictionary]] it encounters containing that key.
...
The `OnPlatform` resource gets an `x:Key` attribute because it’s an object in the dictionary, and an `x:TypeArguments` attribute because it’s a generic class. The `iOS`, and `Android` attributes are converted to [[Color|Color]] values when the object is initialized.
...
![](media/markup-extensions/sharedresources.png)

Although it's common to define the `Resources` collection at the top of the page, you can have `Resources` collections on other elements on the page. For example, the following example shows resources added to a [[StackLayout (Controls)|StackLayout]]:
...
One of the most common types of objects stored in resource dictionaries is the .NET MAUI [[Style|Style]], which defines a collection of property settings. For more information about styles, see [[xaml|Style apps using XAML]].

> [!NOTE]
> The purpose of a resource dictionary is to share objects. Therefore, it doesn't make sense to put controls such as a [[Label (Controls)|Label]] or [[Button (Controls)|Button]] in a resource dictionary. Visual elements can't be shared because the same instance can't appear twice on a page.

## x:Static Markup Extension

In addition to the [[StaticResourceExtension|`StaticResource`]] markup extension, there's also an `x:Static` markup extension. However, while [[StaticResourceExtension|`StaticResource`]] returns an object from a resource dictionary,`x:Static` accesses a public static field, a public static property, a public constant field, or an enumeration member.

> [!NOTE]
> The [[StaticResourceExtension|`StaticResource`]] markup extension is supported by XAML implementations that define a resource dictionary, while `x:Static` is an intrinsic part of XAML, as the `x` prefix reveals.
...
In this example, the [[BoxView (Controls)|BoxView]] dimensions are set to `Math.PI` and `Math.E`, but scaled by a factor of 100:

![Screenshot of controls using the x:Static markup extension.](media/markup-extensions/staticconstants.png)
