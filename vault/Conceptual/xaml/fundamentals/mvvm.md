...
tags:
  ...
  - area/xaml
ms_date: "08/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/xaml/fundamentals/mvvm?view=net-maui-10.0"
---
...
In this example, the retrieved `DateTime` value is set as the `BindingContext` on a [[StackLayout (Controls)|StackLayout]]. When you set the `BindingContext` on an element, it is inherited by all the children of that element. This means that all the children of the [[StackLayout (Controls)|StackLayout]] have the same `BindingContext`, and they can contain bindings to properties of that object:

![](media/mvvm/oneshotdatetime.png)

However, the problem is that the date and time are set once when the page is constructed and initialized, and never change.

> [!WARNING]
> In a class that derives from [[BindableObject|BindableObject]], only properties of type [[BindableProperty|BindableProperty]] are bindable. For example, [[VisualElement (Controls).IsLoaded|VisualElement.IsLoaded]] and [[Element.Parent|Element.Parent]] aren't bindable.
...
In this example, `ClockViewModel` is set to the `BindingContext` of the [[ContentPage|ContentPage]] using property element tags. Alternatively, the code-behind file could instantiate the viewmodel.

The `Binding` markup extension on the `Text` property of the [[Label (Controls)|Label]] formats the `DateTime` property. The following screenshot shows the result:

![](media/mvvm/clock.png)
...
The following example shows the `HslViewModel` that converts a [[Color|Color]] value into `Hue`, `Saturation`, and `Luminosity` values, and back again:
...
The following XAML example contains a [[BoxView (Controls)|BoxView]] whose `Color` property is bound to the `Color` property of the viewmodel, and three [[Slider (Controls)|Slider]] and three [[Label (Controls)|Label]] views bound to the `Hue`, `Saturation`, and `Luminosity` properties:
...
The binding on each [[Label (Controls)|Label]] is the default `OneWay`. It only needs to display the value. However, the default binding on each [[Slider (Controls)|Slider]] is `TwoWay`. This allows the [[Slider (Controls)|Slider]] to be initialized from the viewmodel. When the viewmodel is instantiated it's `Color` property is set to `Aqua`. A change in a [[Slider (Controls)|Slider]] sets a new value for the property in the viewmodel, which then calculates a new color:

![](media/mvvm/hslcolorscroll.png)
...
The viewmodel can define properties of type `ICommand`. You can then bind these properties to the `Command` property of each [[Button (Controls)|Button]] or other element, or perhaps a custom view that implements this interface. You can optionally set the `CommandParameter` property to identify individual [[Button (Controls)|Button]] objects (or other elements) that are bound to this viewmodel property. Internally, the [[Button (Controls)|Button]] calls the `Execute` method whenever the user taps the [[Button (Controls)|Button]], passing to the `Execute` method its `CommandParameter`.

The `CanExecute` method and `CanExecuteChanged` event are used for cases where a [[Button (Controls)|Button]] tap might be currently invalid, in which case the [[Button (Controls)|Button]] should disable itself. The [[Button (Controls)|Button]] calls `CanExecute` when the `Command` property is first set and whenever the `CanExecuteChanged` event is raised. If `CanExecute` returns `false`, the [[Button (Controls)|Button]] disables itself and doesn’t generate `Execute` calls.
...
In this example, the `Command` property of the first [[Button (Controls)|Button]] that is bound to the `DeleteCharCommand`. The other buttons are bound to the `AddCharCommand` with a `CommandParameter` that's the same as the character that appears on the [[Button (Controls)|Button]]:

![](media/mvvm/keypad.png)
