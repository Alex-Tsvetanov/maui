...
tags:
  ...
  - area/xaml
ms_date: "08/30/2024"
source: "https://learn.microsoft.com/dotnet/maui/xaml/fundamentals/data-binding-basics?view=net-maui-10.0"
---
...
The target property must be a bindable property, which means that the target object must derive from [[BindableObject|BindableObject]]. A property of [[Label (Controls)|Label]], such as `Text`, is associated with the bindable property `TextProperty`.

In XAML, you must also perform the same two steps that are required in code, except that the `Binding` markup extension takes the place of the `SetBinding` call and the `Binding` class. However, when you define data bindings in XAML, there are multiple ways to set the `BindingContext` of the target object. Sometimes it’s set from the code-behind file, sometimes using a [[StaticResourceExtension|`StaticResource`]] or `x:Static` markup extension, and sometimes as the content of `BindingContext` property-element tags.

> [!NOTE]
> The data binding expressions on this page use compiled bindings. For more information about compiled bindings, see [[compiled-bindings|Compiled bindings]].
...
The following example contains a [[Slider (Controls)|Slider]] and two [[Label (Controls)|Label]] views, one of which is rotated by the [[Slider (Controls)|Slider]] value and another which displays the [[Slider (Controls)|Slider]] value:
...
The [[Slider (Controls)|Slider]] contains an `x:Name` attribute that is referenced by the two [[Label (Controls)|Label]] views using the `x:Reference` markup extension. The `x:Reference` binding extension defines a property named `Name` to set to the name of the referenced element, in this case `slider`. However, the `ReferenceExtension` class that defines the `x:Reference` markup extension also defines a [[ContentPropertyAttribute|`ContentProperty`]] attribute for `Name`, which means that it isn’t explicitly required.

The `Binding` markup extension itself can have several properties, just like the `BindingBase` and `Binding` class. The [[ContentPropertyAttribute|`ContentProperty`]] for `Binding` is `Path`, but the “Path=” part of the markup extension can be omitted if the path is the first item in the `Binding` markup extension.
...
In this example, four [[Slider (Controls)|Slider]] views are intended to control the `Scale`, `Rotate`, `RotateX`, and `RotateY` properties of a [[Label (Controls)|Label]]. At first, it seems as if these four properties of the [[Label (Controls)|Label]] should be data-binding targets because each is being set by a [[Slider (Controls)|Slider]]. However, the `BindingContext` of [[Label (Controls)|Label]] can be only one object, and there are four different sliders. For that reason, the `BindingContext` of each of the four sliders is set to the [[Label (Controls)|Label]], and the bindings are set on the `Value` properties of the sliders. By using the `OneWayToSource` and `TwoWay` modes, these `Value` properties can set the source properties, which are the `Scale`, `Rotate`, `RotateX`, and `RotateY` properties of the [[Label (Controls)|Label]].

The bindings on three of the [[Slider (Controls)|Slider]] views are `OneWayToSource`, meaning that the [[Slider (Controls)|Slider]] value causes a change in the property of its `BindingContext`, which is the [[Label (Controls)|Label]] named `label`. These three [[Slider (Controls)|Slider]] views cause changes to the `Rotate`, `RotateX`, and `RotateY` properties of the [[Label (Controls)|Label]]:

![](media/data-binding-basics/slidertransforms.png)

However, the binding for the `Scale` property is `TwoWay`. This is because the `Scale` property has a default value of 1, and using a `TwoWay` binding causes the [[Slider (Controls)|Slider]] initial value to be set at 1 rather than 0. If that binding were `OneWayToSource`, the `Scale` property would initially be set to 0 from the [[Slider (Controls)|Slider]] default value. The [[Label (Controls)|Label]] would not be visible.

> [!NOTE]
> The [[VisualElement (Controls)|VisualElement]] class also has `ScaleX` and `ScaleY` properties, which scale the [[VisualElement (Controls)|VisualElement]] on the x-axis and y-axis respectively.

## Bindings and collections

[[ListView (Controls)|ListView]] defines an `ItemsSource` property of type `IEnumerable`, and it displays the items in that collection. These items can be objects of any type. By default, [[ListView (Controls)|ListView]] uses the `ToString` method of each item to display that item. Sometimes this is just what you want, but in many cases, `ToString` returns only the fully-qualified class name of the object.

However, the items in the [[ListView (Controls)|ListView]] collection can be displayed any way you want through the use of a *template*, which involves a class that derives from [[Cell (Controls)|Cell]]. The template is cloned for every item in the [[ListView (Controls)|ListView]], and data bindings that have been set on the template are transferred to the individual clones. Custom cells can be created for items using the [[ViewCell (Controls)|ViewCell]] class.

[[ListView (Controls)|ListView]] can display a list of every named color that's available in .NET MAUI, with the help of the `NamedColor` class:
...
Each `NamedColor` object has `Name` and `FriendlyName` properties of type `string`, a `Color` property of type [[Color|Color]], and `Red`, `Green`, and `Blue` properties. In addition, the `NamedColor` static constructor creates an `IEnumerable<NamedColor>` collection that contains `NamedColor` objects corresponding to the fields of type [[Color|Color]] in the [[Colors|Colors]] class, and assigns it to its public static `All` property.

Setting the static `NamedColor.All` property to the `ItemsSource` of a [[ListView (Controls)|ListView]] can be achieved using the `x:Static` markup extension:
...
![](media/data-binding-basics/listview1.png)

To define a template for the items, the `ItemTemplate` should be set to a [[DataTemplate|DataTemplate]] that references a [[ViewCell (Controls)|ViewCell]]. The [[ViewCell (Controls)|ViewCell]] should define a layout of one or more views to display each item:
...
In this example, the [[Label (Controls)|Label]] element is set to the [[View|View]] property of the [[ViewCell (Controls)|ViewCell]]. The `ViewCell.View` tags are not needed because the [[View|View]] property is the content property of [[ViewCell (Controls)|ViewCell]]. This XAML displays the `FriendlyName` property of each `NamedColor` object:

![](media/data-binding-basics/listview2.png)
...
![](media/data-binding-basics/listview3.png)

The [[ListView (Controls)|ListView]] can handle changes that dynamically occur in the underlying data, but only if you take certain steps. If the collection of items assigned to the `ItemsSource` property of the [[ListView (Controls)|ListView]] changes during runtime, use an `ObservableCollection`1` class for these items. `ObservableCollection`1` implements the `INotifyCollectionChanged` interface, and [[ListView (Controls)|ListView]] will install a handler for the `CollectionChanged` event.
