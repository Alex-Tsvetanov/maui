---
title: "ViewHandler.MapUnfocus"
tags:
  - api
  - member/method
  - ns/Microsoft-Maui-Handlers
aliases:
  - "Microsoft.Maui.Handlers.ViewHandler.MapUnfocus"
declaring_type: "ViewHandler"
member_kind: method
---

# ViewHandler.MapUnfocus

> [!abstract] Method of [[ViewHandler|ViewHandler]]
> Namespace: `Microsoft.Maui.Handlers`

Maps the abstract `Unfocus` method to the platform-specific implementations.

## Signature

```csharp
void static MapUnfocus(Microsoft.Maui.IViewHandler! handler, Microsoft.Maui.IView! view, object? args)
```

## Parameters

| Parameter | Description |
|---|---|
| `handler` | The associated handler. |
| `view` | The associated `IView` instance. |
| `args` | The arguments passed associated to this event. |

## See also

- Declaring type: [[ViewHandler|ViewHandler]]
- [[_Microsoft.Maui.Handlers|Microsoft.Maui.Handlers namespace]]
